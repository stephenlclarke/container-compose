...
public extension ComposeOrchestrator {
    ...
    func commit(project: ComposeProject, serviceName: String, options commit: ComposeCommitOptions = ComposeCommitOptions()) async throws {
        guard let service = project.services[serviceName] else {
            throw ComposeError.invalidProject("unknown service '\(serviceName)'")
        }
        ...
        if options.dryRun {
            ...
            emitComposeRuntimeOperation(commitArchiveDryRunArguments(rootfsArchive: rootfsArchive, imageArchive: imageArchive, service: service, options: commit))
        ...
        try ComposeCommitImageArchive.write(rootfsArchive: rootfs, output: archive, service: service, options: commit, baseImageMetadata: baseImageMetadata)
    ...
    func commitArchiveDryRunArguments(rootfsArchive: String, imageArchive: String, service: ComposeService, options commit: ComposeCommitOptions) -> [String] {
        var args = ["commit-archive", "--rootfs", rootfsArchive, "--output", imageArchive, "--service", service.name]
    ...
    func validateCommitContainerStatus(_ container: ComposeContainerSummary, service: ComposeService, pause: Bool) throws {
