...
extension ComposeOrchestrator {
    ...
    func validateRuntimeResourceSupport(service: ComposeService) throws {
        ...
        _ = try runtimeSecurityOptionArguments(service: service)
    ...
    func validateProjectNetworks(_ project: ComposeProject) throws {
        for (name, network) in project.networks.sorted(by: { $0.key < $1.key }) {
            guard let fields = network.unsupportedFields, !fields.isEmpty else {
                ...
                try validateNetworkIPv6Gateway(network, name: name)
                continue
            }
            let fieldList = fields.joined(separator: ", ")
            throw ComposeError.unsupported("network '\(name)' uses unsupported fields \(fieldList); supported project network fields are name, external, internal, attachable, enable_ipv4, enable_ipv6, labels, driver_opts, the default bridge driver, and one IPv4 IPAM subnet with optional gateway, allocation range, and reserved addresses plus one IPv6 IPAM subnet with an optional gateway")
        }
    }
