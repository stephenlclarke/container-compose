...
private struct ContainerPackageRuntimeProfileError: LocalizedError {
    let value: String

    var errorDescription: String? {
        "CONTAINER_COMPOSE_RUNTIME_PROFILE must be stock or enhanced, not \(value)"
    }
}
...
extension ContainerPackageCompatibility {
    ...
    static func compatibilityFailure(
        ...
        lane: String,
        runtimeProfile: RuntimeProfile = .enhanced,
        expectedContainerRef: String? = nil,
        expectedContainerizationRef: String? = nil,
    ) -> String? {
        ...
        if runtimeProfile == .stock {
            return stockCompatibilityFailure(
                container: container,
    ...
    private static func selectedRuntimeProfile() throws -> RuntimeProfile {
        let value = ProcessInfo.processInfo.environment[runtimeProfileEnvironmentKey]
            ?? compiledRuntimeProfile.rawValue
        guard let profile = RuntimeProfile(rawValue: value) else {
            throw ContainerPackageRuntimeProfileError(value: value)
        }
        return profile
    }

    private static func stockCompatibilityFailure(
        container: ContainerSystemVersionComponent,
        apiserver: ContainerSystemVersionComponent?,
        lane: String,
    ) -> String? {
        let stockSource = container.source == nil || container.source == "apple/container"
        let stockDistribution = container.distribution == nil || container.distribution == "apple"
        var detected: [String] = []
        if !stockSource || !stockDistribution {
            detected.append(
                "container: \(container.source ?? "unknown") (distribution: \(container.distribution ?? "unknown"))",
            )
        }
        if let apiserver {
            if apiserver.commit != container.commit {
                detected.append(
                    "container-apiserver: \(apiserver.commit ?? "unknown") (CLI commit \(container.commit ?? "unknown"))",
                )
            }
        } else {
        ...
        return stockInstallGuidance(lane: lane, detected: detected)
    }
    ...
    private static func serviceGuidance(
        ...
        if runtimeProfile == .stock {
            return """
            container-compose requires the selected stock Apple container system service to be running.

            Start the exact stock runtime selected by CONTAINER_COMPOSE_CONTAINER, then run this command again:
