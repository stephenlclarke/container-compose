...
| Runtime resources and security | ⚠️ Partial | `cpus`, `cpu_shares`, `mem_limit`, `pids_limit`, `pid: host`, `blkio_config`, `sysctls`, `ulimits`, `shm_size`, `oom_score_adj`, `privileged`, `cap_add`, `cap_drop`, `read_only`, `restart`, `stop_signal`, `stop_grace_period`, and numeric or named `group_add` values are implemented. `cpu_shares` sets a relative CPU weight; zero leaves the runtime default unchanged and non-zero weights start at `2`. `oom_score_adj` accepts Linux scores from `-1000` through `1000` and is applied to service, healthcheck, and one-off processes. Named groups resolve from the selected guest image's `/etc/group`; numeric values are supplied as GIDs. `cpu_count`, `cpu_percent`, `cpu_period`, `cpu_quota`, `cpu_rt_runtime`, `cpu_rt_period`, `cpuset`, `mem_reservation`, `memswap_limit`, `mem_swappiness`, `oom_kill_disable`, `cgroup`, `cgroup_parent`, `ipc`, `isolation`, `security_opt`, `storage_opt`, `userns_mode`, and `uts` are parsed but remain fully or partly limited by runtime primitives; non-host `pid` namespace joins remain unsupported. |
