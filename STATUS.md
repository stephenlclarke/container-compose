...
| Volumes, mounts, configs, and secrets | ⚠️ Partial | `volumes`, `volumes_from`, `volume_driver`, `tmpfs`, `configs`, and `secrets` are implemented for named, bind, anonymous, tmpfs, read-only local OCI image, file-backed, environment-backed, inline `configs.content`, external-config, and external-secret local mode. Named volumes support long-form `volume.subpath`, and image mounts support `image.subpath`; each selected directory must already exist in its source. Image-mount sources must already be available locally. Generated config/secret `uid`, `gid`, and `mode` are honored through private creation-time file snapshots; file-backed config/secret grant metadata remains Docker-compatible source-file bind behavior. Mount `consistency`, SELinux, recursive bind, `npipe`, `cluster`, and `use_api_socket` remain gaps. |
| Runtime resources and security | ⚠️ Partial | `cpus`, `cpu_shares`, `mem_limit`, `mem_reservation`, `memswap_limit`, `pids_limit`, `pid: host`, `blkio_config`, `sysctls`, `ulimits`, `shm_size`, `oom_score_adj`, `privileged`, `cap_add`, `cap_drop`, `read_only`, `restart`, `stop_signal`, `stop_grace_period`, and numeric or named `group_add` values are implemented. `cpu_shares` sets a relative CPU weight; zero leaves the runtime default unchanged and non-zero weights start at `2`. `mem_reservation` sets a soft memory reservation; zero leaves it unset, and an explicit `mem_limit` must be higher. `memswap_limit` limits total memory plus swap: zero is unset, `-1` keeps unlimited swap, and a positive value must be at least `mem_limit`; when `mem_limit` is set without an explicit non-zero swap limit, Compose uses Docker's two-times-memory default. `make docker-compose-memory-swap-limit-parity` keeps the Docker/Colima config, Engine, and local dry-run contract covered. `oom_score_adj` accepts Linux scores from `-1000` through `1000` and is applied to service, healthcheck, and one-off processes. Named groups resolve from the selected guest image's `/etc/group`; numeric values are supplied as GIDs. `cpu_count`, `cpu_percent`, `cpu_period`, `cpu_quota`, `cpu_rt_runtime`, `cpu_rt_period`, `cpuset`, `mem_swappiness`, `oom_kill_disable`, `cgroup`, `cgroup_parent`, `ipc`, `isolation`, `security_opt`, `storage_opt`, `userns_mode`, and `uts` are parsed but remain fully or partly limited by runtime primitives; non-host `pid` namespace joins remain unsupported. |
