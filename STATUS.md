...
| Compose Deploy Specification | ⚠️ Partial | `mode`, `replicas`, `labels`, `endpoint_mode`, CPU/memory `resources` limits and reservations, generic GPU device reservations, `restart_policy`, stop-first `update_config` delay, `rollback_config`, and `placement` are implemented locally or preserved as metadata. Start-first `update_config`, pids reservations, non-GPU device reservations, generic reservations, pids/device/generic limits, and Swarm scheduler or rollback orchestration remain gaps. |
...
| Devices, GPU, and credentials | ⚠️ Partial | `devices`, `device_cgroup_rules`, service `gpus`, and Deploy generic GPU reservations are implemented through the `stephenlclarke` runtime. GPU support is the single Apple virtio-gpu exposed as `/dev/dri/card0` and `/dev/dri/renderD128`; vendor drivers such as NVIDIA/CUDA, multiple GPUs, driver options, non-GPU device reservations, `credential_spec`, and arbitrary macOS hardware passthrough remain runtime gaps. |
...
| Deploy Specification attributes | ⚠️ Partial | `deploy` fields `mode`, `replicas`, `labels`, `endpoint_mode`, `resources.limits`, CPU/memory `resources.reservations`, generic GPU device reservations, restart policy metadata, `rollback_config`, and `placement` are implemented for local behavior or preserved as scheduler metadata. `update_config` start-first behavior, pids reservations, non-GPU device reservations, generic reservations, pids/device/generic limits, and Swarm scheduler or rollback orchestration semantics remain gaps. |
