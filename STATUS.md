...
| Compose Deploy Specification | ⚠️ Partial | `mode`, `replicas`, `labels`, `endpoint_mode`, CPU/memory `resources` limits and reservations, `restart_policy`, stop-first `update_config` delay, `rollback_config`, and `placement` are implemented locally or preserved as metadata. Start-first `update_config`, pids/device/generic reservations and limits, and Swarm scheduler or rollback orchestration remain gaps. |
...
| Deploy Specification attributes | ⚠️ Partial | `deploy` fields `mode`, `replicas`, `labels`, `endpoint_mode`, `resources.limits`, CPU/memory `resources.reservations`, restart policy metadata, `rollback_config`, and `placement` are implemented for local behavior or preserved as scheduler metadata. `update_config` start-first behavior, pids/device/generic reservations, pids/device/generic limits, and Swarm scheduler or rollback orchestration semantics remain gaps. |
