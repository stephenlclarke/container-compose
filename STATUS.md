...
- `stephenlclarke/container`: `f34a51f000b9291c6c60dbee1e092bacbb98b347`
...
`compose build --ssh` and service `build.ssh` now report as supported and flow through the normal build path and `build --print` bake output. Repeated CLI values are accepted, compose-file SSH values normalize as build metadata, and CLI SSH entries replace file entries with the same id for local socket overrides. Default SSH agent forwarding, explicit `id=/path/to/socket` host Unix sockets, and multiple distinct host socket paths are live-tested through compose.yml/Dockerfile builds. Runtime build execution requires the matching `stephenlclarke/container` build backend and SSH-capable builder image.
