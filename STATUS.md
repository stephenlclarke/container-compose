...
Last updated: 2026-06-24 13:10 BST.
...
| --- | --- | --- | --- |
| `stephenlclarke/container-compose` | `develop` | Current Compose-side integration branch | Carries the former log/lifecycle/config/network/runtime-control proving work from the deleted broad integration branches, including fork-backed logs, health/dependencies, restart policies, configs/secrets, network identity, blkio, sysctls, pause/copy/top, and events mappings. |
| `stephenlclarke/container` | `develop` | Current fork-backed runtime integration branch | Carries the former broad runtime proving work after squash and cleanup, including logs, local log policy/rotation, exit metadata, health checks, restart policies, image healthcheck metadata, identity/network primitives, blkio, sysctls, pause, copy, process identifiers, and events. |
...
21. The image healthcheck inheritance plugin slab reads that direct API metadata so Dockerfile `HEALTHCHECK` defaults and timing-only Compose overrides can become runtime `--health-*` flags. Its handoff files are `docs/upstream/container-compose/ISSUE-image-healthcheck-inheritance.md` and `docs/upstream/container-compose/PR-image-healthcheck-inheritance.md`.
22. The config/secret materialization plugin slab follows Docker's Compose config and secret source model by materializing `configs.content`, `configs.environment`, and `secrets.environment` into deterministic project-scoped files under the per-user `container-compose` state root, then mounts them read-only through existing `apple/container` bind-mount primitives. Its handoff files are `docs/upstream/container-compose/ISSUE-config-secret-materialization.md` and `docs/upstream/container-compose/PR-config-secret-materialization.md`.
23. The config/secret grant mode plugin slab applies service-level `mode` to generated config/secret grants, ignores writable bits per Compose semantics, preserves Docker Compose bind-mount behavior for file-backed grants, and keeps `uid`/`gid` ownership remapping as an apple/container runtime gap. Its handoff files are `docs/upstream/container-compose/ISSUE-config-secret-grant-mode.md` and `docs/upstream/container-compose/PR-config-secret-grant-mode.md`.
...
25. The Compose `extra_hosts` plugin slab maps compose-go normalized static host entries to `container run/create --add-host` and validates IP literals before side effects. Its handoff files are `docs/upstream/container-compose/ISSUE-extra-hosts.md` and `docs/upstream/container-compose/PR-extra-hosts.md`.
26. The hostname runtime/plugin slab adds fork-side `container run/create -h, --hostname`, preserves existing default hostname derivation, and maps Compose service `hostname` to the runtime. Its handoff files are `ISSUE-hostname.md` and `PR-hostname.md` in the container fork, plus `docs/upstream/container-compose/ISSUE-service-hostname.md` and `docs/upstream/container-compose/PR-service-hostname.md` in this repository.
27. The host-gateway runtime/plugin slab adds fork-side `container run/create --add-host host:host-gateway` resolution to the first network IPv4 gateway and maps Compose `extra_hosts` `host-gateway` entries to that runtime primitive. Its handoff files are `docs/upstream/container-compose/ISSUE-host-gateway.md` and `docs/upstream/container-compose/PR-host-gateway.md` in both repositories.
28. The network-alias runtime/plugin slab adds fork-side `AttachmentOptions.aliases` plus `container run/create --network name,alias=...`, then maps Compose `services.*.networks.*.aliases` for the current single-network subset. Its handoff files are `docs/upstream/container-compose/ISSUE-network-aliases.md` and `docs/upstream/container-compose/PR-network-aliases.md` in both repositories.
29. The legacy-links plugin slab maps Compose `links` to implicit dependency ordering plus target-service aliases for services that share exactly one Compose network, including the compose-go normalized implicit `default` network. Its handoff files are `docs/upstream/container-compose/ISSUE-links.md` and `docs/upstream/container-compose/PR-links.md` in this repository.
30. The domainname runtime/plugin slab adds fork-side `ContainerConfiguration.domainname`, `container run/create --domainname`, runtime `kernel.domainname` mapping, and Compose service `domainname` translation. Its handoff files are `ISSUE-domainname.md` and `PR-domainname.md` in the container fork, plus `docs/upstream/container-compose/ISSUE-service-domainname.md` and `docs/upstream/container-compose/PR-service-domainname.md` in this repository.
31. The legacy external-links plugin slab maps Compose `external_links` to generated `--add-host` entries when the source service has exactly one Compose network and the referenced existing apple/container container has exactly one matching runtime attachment. Its handoff files are `docs/upstream/container-compose/ISSUE-external-links.md` and `docs/upstream/container-compose/PR-external-links.md` in this repository.
32. The deploy job-mode plugin slab preserves compose-go normalized `deploy.mode: replicated-job` / `global-job`, starts local job replicas detached, waits for successful completion through the direct lifecycle adapter, and treats job `condition: any` as `on-failure`. Its handoff files are `docs/upstream/container-compose/ISSUE-deploy-job-modes.md` and `docs/upstream/container-compose/PR-deploy-job-modes.md` in this repository.
33. The block I/O plugin slab preserves Compose `blkio_config` fields and maps them to the repeatable `container run/create --blkio` key-value contract proposed by Chris George in [apple/container#1595](https://github.com/apple/container/pull/1595), which closes [apple/container#1512](https://github.com/apple/container/issues/1512). Its handoff files are `docs/upstream/container-compose/ISSUE-blkio-config.md` and `docs/upstream/container-compose/PR-blkio-config.md` in this repository.
34. The sysctl runtime/plugin slab exposes the existing fork-side `ContainerConfiguration.sysctls` through repeatable `container run/create --sysctl name=value`, then maps Compose service `sysctls` to that runtime bridge. Its handoff files are `ISSUE-sysctl-cli.md` and `PR-sysctl-cli.md` in the container fork, plus `docs/upstream/container-compose/ISSUE-sysctls.md` and `docs/upstream/container-compose/PR-sysctls.md` in this repository.
...
- Added the first fork-side container event-stream primitive in `stephenlclarke/container` as code commits `b71e4bb323e3` (`feat(events): stream container lifecycle events`) and `0da7890b2632` (`fix(events): avoid blocking slow event subscribers`), plus handoff-doc commits `48b763c` and `24dcfbc`. The slice adds `ContainerEvent`, `ContainerClient.events()`, API-service lifecycle event emission, non-blocking event subscribers, and `container events` JSON Lines output without Compose filtering.
- Added plugin-side `container compose events --json [SERVICE...]` support in commit `113be38063ea` by filtering the fork-backed `ContainerClient.events()` stream to Compose project/service containers, skipping one-off containers, stripping Compose-private attributes, and rendering Docker Compose compatible JSON Lines output. Handoff files are `docs/upstream/events/ISSUE-compose-events.md` and `docs/upstream/events/PR-compose-events.md`.
- Added fork-side event replay/time filtering in commit `d0977b5a99ec7dfd4fdc9a3b5e50b36869451270` and plugin-side `container compose events --json --since/--until [SERVICE...]` mapping in commit `3a3387d7dbea301eec3a7f1fcc3f954dec80276c`. Handoff files are `docs/upstream/events/ISSUE-container-event-time-filters.md`, `docs/upstream/events/PR-container-event-time-filters.md`, `docs/upstream/events/ISSUE-compose-event-time-filters.md`, and `docs/upstream/events/PR-compose-event-time-filters.md`, with Apple runtime mirrors under `docs/upstream/apple-container/`.
- Added plugin-side default text `container compose events [--since/--until] [SERVICE...]` support in commits `fd3d94824f23cd3255a812faed9e3972906b4ab5` and `4cfb39e9531a84b496e1dcc76a84ac7654df943f`, preserving the prior `--json` renderer and extending the optional local-only Docker Compose parity script with text replay validation. Handoff files are `docs/upstream/events/ISSUE-compose-events-text-format.md` and `docs/upstream/events/PR-compose-events-text-format.md`.

Next:

- Completed the first five slices from the runtime event-streaming slab: the `apple/container` fork event primitive, the `container-compose` `events --json [SERVICE...]` mapping, the `apple/container` bounded replay/time-filter primitive, the `container-compose` `events --json --since/--until [SERVICE...]` mapping, and the `container-compose` default text event formatting slice. Keep these as separate PR-shaped slices: Apple PRs expose generic runtime events and time filtering, while Compose PRs own project/service filtering and Docker Compose output policy.
- Next selected lifecycle/control slice should be chosen after a fresh issue/PR check. The current high-value candidate is interactive attach / signal proxying against [apple/container#378](https://github.com/apple/container/issues/378), [apple/container#1747](https://github.com/apple/container/issues/1747), [apple/container#1778](https://github.com/apple/container/pull/1778), and [apple/containerization#735](https://github.com/apple/containerization/issues/735).
- Final completion gate: once the intended `container-compose` functionality and supporting fork code are all implemented, review every potential Apple PR independently for fit, narrowness, code-owner suitability, source issue/PR references, commit-ID accuracy, validation evidence, and leakage of Compose-specific policy into Apple runtime code. Fix findings before raising or refreshing upstream PRs; the detailed checklist is in `docs/upstream/README.md`.
