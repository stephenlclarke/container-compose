...
| Isolation and security | Cgroup mode/parent, non-host IPC/PID/UTS/user namespaces, complete `security_opt`, and Docker-complete privileged behavior (devices and masked/read-only paths as well as capabilities). | Preserve/apply `stop_signal` and `stop_grace_period` as container configuration rather than only at Compose stop time. | `cpu_count`, `cpu_percent`, `isolation`, and `credential_spec` are Windows-only. |
