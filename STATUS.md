...
| Dependencies and links | ⚠️ Partial | `depends_on` and `external_links` ordering/discovery are implemented, including `condition: service_completed_successfully`, `condition: service_healthy`, and dependency restart/required metadata where local mode can honor it. Legacy service `links` resolves each linked service's current IPv4 attachment into a source-container static host entry after dependency creation; source and target must share exactly one normalized Compose network, and aliases colliding with source `extra_hosts` or another linked service are rejected before side effects. Dynamic source-scoped DNS aliases, direct network `aliases`, links with zero or multiple shared networks, address-change propagation outside Compose reconciliation, and richer external-service discovery remain runtime gaps. |
...
| Network attachments and discovery | ⚠️ Partial | `networks`, including multiple attachments at container creation on macOS 26+, `network_mode` values `none` and `host`, service MTU `driver_opts`, `mac_address`, `dns`, `dns_opt`, `dns_search`, and `extra_hosts` are implemented where the runtime exposes matching primitives. Network `aliases` are rejected before side effects because the runtime has no container-facing DNS listener. Endpoint `ipv4_address`, `ipv6_address`, `link_local_ips`, `interface_name`, `gw_priority`, `priority`, arbitrary endpoint `driver_opts`, `network_mode: service:NAME`, and `network_mode: container:NAME` remain runtime gaps. |
