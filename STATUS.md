...
| Runtime resources and security | ⚠️ Partial | `cpu_shares`, `mem_reservation`, `deploy.resources.reservations.memory`, `memswap_limit`, `pids_limit`, host `pid`, `blkio_config`, `sysctls`, `ulimits`, `shm_size`, `oom_score_adj`, `cap_add`, `cap_drop`, `read_only`, restart, numeric/named `group_add`, and `security_opt` values `no-new-privileges:true` and `no-new-privileges:false` are mapped. Docker Compose's local Deploy memory reservation reuses the generic soft-memory runtime primitive. `cpus` accepts fractional Compose values but the runtime parser is integral, so Docker quota semantics for values such as `0.25` are not implemented; `mem_limit` is rounded/truncated to MiB. Advanced `cpu_period`, `cpu_quota`, `cpu_rt_period`, `cpu_rt_runtime`, and `cpuset` controls; `mem_swappiness`, `oom_kill_disable`, `cgroup`/`cgroup_parent`, non-host `ipc`/`pid`/`uts`/`userns_mode`, other `security_opt` forms, and `storage_opt` are unavailable. `privileged` currently grants capabilities only and is not Docker's full privileged isolation/device behavior. Service `stop_signal` and `stop_grace_period` are applied by Compose stop operations but are not persisted as per-container runtime configuration. `cpu_count`, `cpu_percent`, `isolation`, and `credential_spec` are Windows-specific. |
...
| --- | --- | --- | --- |
| CPU and memory controls | Fractional cgroup CPU quota (`cpus: 0.25`), byte-accurate hard memory limits, `cpu_period`, `cpu_quota`, `cpu_rt_period`, `cpu_rt_runtime`, `cpuset`, `mem_swappiness`, and `oom_kill_disable`. | Pass fractional CPU limits only after the runtime accepts a non-integral quota. | Integral CPU counts, CPU shares, service/Deploy memory reservation, memory swap, and pids limits already work. |
| Isolation and security | Cgroup mode/parent, non-host IPC/PID/UTS/user namespaces, SELinux/AppArmor/seccomp and other `security_opt` forms, and Docker-complete privileged behavior (devices and masked/read-only paths as well as capabilities). | Preserve/apply `stop_signal` and `stop_grace_period` as container configuration rather than only at Compose stop time. | `security_opt` values `no-new-privileges:true` and `no-new-privileges:false` use the generic Linux guest primitive. `cpu_count`, `cpu_percent`, `isolation`, and `credential_spec` are Windows-only. |
