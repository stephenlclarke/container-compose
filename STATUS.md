...
- Completed the first four slices from the runtime event-streaming slab: the `apple/container` fork event primitive, the `container-compose` `events --json [SERVICE...]` mapping, the `apple/container` bounded replay/time-filter primitive, and the `container-compose` `events --json --since/--until [SERVICE...]` mapping. Keep these as separate PR-shaped slices: Apple PRs expose generic runtime events and time filtering, while Compose PRs own project/service filtering and Docker Compose output policy.
- Next selected lifecycle slice should be chosen from the remaining lifecycle/control gaps after a fresh issue/PR check. The current high-value candidates are non-JSON `container compose events` formatting as a plugin-only follow-up, or interactive attach / signal proxying against [apple/container#378](https://github.com/apple/container/issues/378), [apple/container#1747](https://github.com/apple/container/issues/1747), [apple/container#1778](https://github.com/apple/container/pull/1778), and [apple/containerization#735](https://github.com/apple/containerization/issues/735).
