...
| Runtime resources and security | ⚠️ Partial | Positive fractional `cpus`, `cpu_period`, `cpu_quota`, `cpu_shares`, byte-accurate `mem_limit`, `mem_reservation`, `deploy.resources.reservations.memory`, `memswap_limit`, `pids_limit`, host `pid`, `blkio_config`, `sysctls`, `ulimits`, `shm_size`, `oom_score_adj`, `cap_add`, `cap_drop`, `read_only`, restart, numeric/named `group_add`, and `security_opt` values `no-new-privileges:true` and `no-new-privileges:false` are mapped. Fractional and explicit CFS settings are persisted and applied through the macOS Linux guest's cgroup v2 `cpu.max`; `stop_signal` and `stop_grace_period` are also persisted as per-container runtime defaults. Docker Compose's local Deploy memory reservation reuses the generic soft-memory runtime primitive. `cpus: 0` / `0.000` (Docker's no-limit form), `cpu_rt_period`, `cpu_rt_runtime`, and `cpuset`; `mem_swappiness`, `oom_kill_disable`, `cgroup`/`cgroup_parent`, non-host `ipc`/`pid`/`uts`/`userns_mode`, other `security_opt` forms, and `storage_opt` are unavailable. `privileged` currently grants capabilities only and is not Docker's full privileged isolation/device behavior. `cpu_count`, `cpu_percent`, `isolation`, and `credential_spec` are Windows-specific. |
...
| --- | --- | --- | --- |
| CPU and memory controls | Docker's zero/no-limit `cpus` form, `cpu_rt_period`, `cpu_rt_runtime`, `cpuset`, `mem_swappiness`, and `oom_kill_disable`. | Expose a generic unlimited-CPU configuration before accepting `cpus: 0` / `0.000`; no additional adapter work remains for positive fractional `cpus`, `cpu_period`, or `cpu_quota`. | Positive fractional `cpus`, explicit cgroup v2 CPU quota/period, integral CPU counts, byte-accurate hard memory limits, CPU shares, service/Deploy memory reservation, memory swap, and pids limits work. |
