...
- Completed the first two slices from the runtime event-streaming slab: the `apple/container` fork event primitive and the `container-compose` `events --json [SERVICE...]` mapping. Keep both as separate PR-shaped slices: the Apple PR exposes generic runtime events, while the Compose PR owns project/service filtering and Docker Compose JSON output policy.
- Next selected slice from the same slab: add runtime event replay/time filtering for `--since` and `--until` as a separate `apple/container` PR-shaped primitive before enabling those flags in `container-compose`. A targeted live search on 2026-06-22 found no matching open Apple issue or PR for `since` / `until` / replay events, so use [apple/container#484](https://github.com/apple/container/issues/484), Docker `system events`, and Docker Compose `events` behavior as the source references. Keep non-JSON `container compose events` formatting as a later plugin-only follow-up.
