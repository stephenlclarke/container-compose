...
| Lifecycle hooks | ⚠️ Partial | `post_start` and `pre_stop` hooks run for detached and managed lifecycle paths, plus regular foreground `up`: services start detached, run `post_start`, then aggregate logs; interruption uses the standard stop path so `pre_stop` runs. Foreground one-off `run` still cannot place hooks around its original process, and Docker Compose `pre_start` hook metadata is preserved and explicitly rejected before side effects until the runtime exposes ephemeral init-container lifecycle support. |
