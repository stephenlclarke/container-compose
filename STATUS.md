...
| CLI commands | ⚠️ Partial | 42 commands are ✅, 4 are ⚠️, and 0 are ❌. These HELP markers cover command invocation and visible command behavior; the full runtime gap register is below. Every command is listed in [CLI Command Surface](#cli-command-surface). |
...
| `run` | ⚠️ Partial | One-off containers and Docker Compose run options are implemented except `--use-aliases`/container-facing DNS. Foreground lifecycle hooks cover non-interactive log/wait and interactive reattach, signal, detach-key, cleanup, and exit-status paths. |
...
| `up` | ⚠️ Partial | Create/start/attach/watch/menu/build/pull/recreate/log-output/scaling behavior, health-aware `--wait`/`--wait-timeout`, selected-service `--exit-code-from` status propagation, and service-level `pre_start` helpers are implemented. Container-facing DNS aliases remain unavailable. |
...
| Networks and names | Container-facing, network-scoped embedded DNS that resolves service/container names and aliases; namespace sharing for `network_mode: service:NAME`/`container:NAME`; arbitrary custom-name network modes; custom network drivers; custom IPAM drivers. | Source-scoped `networks.aliases`, `run --use-aliases`, complete `links`/`external_links`, dynamic address propagation, legacy link environment semantics, and endpoint `driver_opts` beyond MTU. | Existing runtime attachment, static IPAM, `extra_hosts`, host/none modes, the built-in `bridge` mode, MTU, priorities, interface naming, and local-network `attachable` metadata remain usable. `network_mode: bridge` maps to Container's built-in `default` network and intentionally retains Docker's default-bridge no-service-DNS boundary. |
