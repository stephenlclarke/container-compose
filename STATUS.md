...
| Top-level `networks` | ⚠️ Partial | `name`, `external`, `internal`, `labels`, top-level `driver_opts`, the default bridge `driver`, and `ipam` with one IPv4 `config.subnet` plus optional IPv4 `gateway` and `ip_range` and one IPv6 `config.subnet` are implemented. An IPv4 `ip_range` limits only dynamic address allocation; valid explicit static addresses remain available across the containing subnet. Custom drivers, `attachable` set true, `enable_ipv4` set false, `enable_ipv6` without a mapped subnet, IPAM `driver`/`options`, IPv6 gateway or allocation range, `aux_addresses`, and multiple subnets of the same address family remain runtime gaps and fail before resource creation. |
