...
| Non-local volumes, advanced mounts, and `use_api_socket` | Arbitrary volume drivers currently degrade silently to local ext4, existing writable ext4 volumes are not safe Docker-style multi-VM shared storage, bind propagation is rendered without live lower-runtime proof, and there is no authoritative Docker API socket/credential boundary. | Implement the cross-stack [non-local volume, advanced mount, and API socket parity design](docs/non-local-volumes-advanced-mounts-api-socket-design.md): provider-backed shareable storage, typed guest-kernel mount semantics, devcontainer-derived shared Engine transport, exact credential transformation, migration, and behavior/performance oracles. |
...
| CLI commands | ⚠️ Partial | 44 commands are ✅, 2 are ⚠️, and 0 are ❌. These HELP markers cover command invocation and visible command behavior; the full runtime gap register is below. Every command is listed in [CLI Command Surface](#cli-command-surface). |
...
| Logging | ⚠️ Partial | `logging` drivers `none`, `json-file`, and `local` are accepted, with `max-size`/`max-file` validation, but `json-file` and `local` both use the same Apple local logger. The matched runtime preserves complete long records across backward-read tail chunk boundaries and isolates failed attached-client sinks so persistent logging continues. Docker driver behavior and plugins (`syslog`, `journald`, `fluentd`, `gelf`, `awslogs`, `splunk`, and others), `mode`, `max-buffer-size`, and driver-specific options are unavailable. |
...
| `attach` | ✅ Yes | Interactive init-process stream reattachment, `--sig-proxy`, and Docker-compatible `--detach-keys` are implemented through the forked runtime primitive tracked by [apple/container#378](https://github.com/apple/container/issues/378); the matched client encodes proxied signals by Linux-resolvable name, and output-only attach continues to follow persisted logs. |
...
| `run` | ✅ Yes | One-off containers and Docker Compose run options are implemented, including opt-in service and explicit network aliases through `--use-aliases`. Foreground lifecycle hooks cover non-interactive log/wait and interactive reattach, signal, detach-key, cleanup, and exit-status paths. |
...
| `up` | ✅ Yes | Create/start/attach/watch/menu/build/pull/recreate/log-output/scaling behavior, health-aware `--wait`/`--wait-timeout`, selected-service `--exit-code-from` status propagation, service-level `pre_start` helpers, and network-scoped service discovery are implemented. |
...
| Docker API socket | One authoritative Docker-compatible Engine service, exact credential handoff, and a typed per-container inbound socket grant. | Implement the [non-local volume, advanced mount, and API socket parity design](docs/non-local-volumes-advanced-mounts-api-socket-design.md) before enabling `use_api_socket`. | A raw host socket bind or the current bounded devcontainer router is not equivalent to Docker Compose's API socket behavior. |
