...
| --- | --- | --- | --- |
| CPU and memory controls | Fractional cgroup CPU quota (`cpus: 0.25`), `cpu_period`, `cpu_quota`, `cpu_rt_period`, `cpu_rt_runtime`, `cpuset`, `mem_swappiness`, and `oom_kill_disable`. | Pass fractional CPU limits only after the runtime accepts a non-integral quota. | Integral CPU counts, byte-accurate hard memory limits, CPU shares, service/Deploy memory reservation, memory swap, and pids limits already work. |
