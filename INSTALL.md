...
| Tagged Compose release | `container-compose-release-v0-3-0` style | release | You want a specific Compose `release-VERSION-TAG` branch. |
...
Tagged Compose release branch formulae use the same pattern. For example, branch `release-v0.3.0` publishes `container-compose-release-v0-3-0`. Tagged Compose formulae currently pair with the moving `container-release` runtime formula unless a matching versioned runtime formula is published for the same stack release.
