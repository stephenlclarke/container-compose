...
[`check-docker-rest-gelf-contract.sh`](../../Tools/parity/check-docker-rest-gelf-contract.sh) drives the same Docker CLI lifecycle against Docker Engine 29.2.1 and a fresh Container public socket. The reference needs `--gelf-address-host host.docker.internal` because its UDP receiver is outside Colima; the candidate receiver is local to the isolated runtime. The fixture exercises a cache-disabled UDP GELF logger, default gzip framing, selected environment/label metadata, tag expansion, ordered stdout/stderr/binary records, inspect projection, unreadable remote logs, native authority visibility, and cleanup.

`LOGGING-GELF-REST-01` passed a fresh Docker reference in `0.680846750s` and three fresh exact candidates in `1.567608333s`, `1.526945791s`, and `1.461681958s`. The source, local/transitive dependency graph, release archive, signed binaries, guest/init archive, bootstrap image, script, wrapper, Docker oracle, and result files are retained under `/private/tmp/container-gelf-rest-v4.AR1wW6`; the complete scope and non-goals are in the [slice ledger](slice-ledger.md#logging-gelf-rest-01). The candidate timings are 2.15–2.30× Docker, so they satisfy the fixture's 10× functional guard but do not close the programme-wide comparable-or-better performance gap. The public create response's name-as-ID mismatch is outside this logging contract and is tracked in [Container issue 74](https://github.com/stephenlclarke/container/issues/74).
...
The oracle does not provision external logging services or installed logging plugins. Cloud-driver delivery, a blocking slow-sink backpressure case, dual-cache rotation under sustained load, and plugin lifecycle behavior still require separate focused oracles. GELF's direct Engine UDP/TCP wire, configuration/validation, selected metadata/tag behavior, and narrow cache-disabled UDP public-socket lifecycle are captured here; the component and public-socket proof is retained in [LOGGING-GELF-REST-01](slice-ledger.md#logging-gelf-rest-01). The `local` Docker-CLI/public-socket candidate lane is verified separately in [LOGGING-LOCAL-REST-01](slice-ledger.md#logging-local-rest-01); all other candidate-runtime and external-client certification remains separate work.
