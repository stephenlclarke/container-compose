...
`--transport tcp --scenario tcp-retry-delay` adds two reset peers, a positive `gelf-tcp-max-reconnect=2` budget, and a one-second `gelf-tcp-reconnect-delay`. The latest retained Docker reference, `/private/tmp/container-rest-gelf.default-host.wG4HQt`, passed in `22.447226208` seconds (result SHA-256 `ecb6fc023506a605c360f4032a59a26138daa0898d3dfd2db3e1c7ac1f363abe`) and proves two one-frame resets, two configured retry intervals (`10.019107917` and `9.010752833` seconds), and Docker's observed terminal recovery disposition. The matching Container path is `Active`: two retained candidates reached distinct bounded functional failures, and signed Container `54c5b332a204c01da34029c7fadabb560520c887` now preserves a redacted bootstrap phase through the protected service, GELF provider, and Docker diagnostic. `GELFTCPServiceWireTests` (10), `EngineLinuxSandboxGELFTCPServiceTests` (17), and `ContainerLoggingStartErrorTests` (5) pass on the exact local graph; a new marker-protected exact-fingerprint candidate is required. [RUNTIME-ISOLATED-PUBLIC-SOCKET-01](handoffs/RUNTIME-ISOLATED-PUBLIC-SOCKET-01.md) permits that namespace-aware run without stopping the user-owned devcontainer engine. Performance comparison remains post-functional unless execution hangs or exceeds its liveness bound. The precise evidence and acceptance boundary are in the [handoff](handoffs/LOGGING-GELF-TCP-RETRY-DELAY-01.md).
