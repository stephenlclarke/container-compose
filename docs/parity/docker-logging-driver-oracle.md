...
The matching fork-only native correction maps the Docker VM alias to loopback only when a macOS Fluentd TCP client dials; it leaves stored configuration and TLS requested identity unchanged. The focused 12-test loopback suite and 92.28% line-coverage report pass. Its public candidate and timing comparison are `Handed off`, not passed: the current runner calls global `container system stop` and would interrupt the user-owned `devcontainer-engine`. The exact boundary, retained roots, and completion criteria are in [LOGGING-FLUENTD-TCP-ACK-REST-01](handoffs/LOGGING-FLUENTD-TCP-ACK-REST-01.md).
