...
`LOGGING-GELF-REST-01` passed a fresh UDP Docker reference in `0.680846750s` and three fresh exact candidates in `1.567608333s`, `1.526945791s`, and `1.461681958s`. `LOGGING-GELF-TCP-REST-01` passed a fresh TCP reference in `0.931894667s` and three exact candidates in `1.603390667s`, `1.700857709s`, and `1.975864916s`. `LOGGING-GELF-TCP-RECONNECT-REST-01` passed its fresh forced-peer-loss Docker reference in `5.524841000s` and two exact candidates in `6.508964500s` and `6.340717583s` (1.18× and 1.15×). `LOGGING-GELF-TCP-FAILURE-REST-01` passed Docker's unavailable-endpoint reference in `0.072112500s` and two exact candidates in `0.505147750s` and `0.460362292s` (7.00× and 6.38×); its retry-exhaustion reference passed in `12.445434875s` and candidates in `13.568986042s` and `13.622602750s` (both 1.09×). The source, local dependency graph, release archive, signed binaries, guest/init archives, script, wrapper, Docker oracle, and result files are retained under `/private/tmp/container-gelf-rest-v4.AR1wW6`, `/private/tmp/container-gelf-tcp-rest-v3.Zd0bfm`, `/private/tmp/container-gelf-tcp-reconnect-candidate-v2.nmMgi1`, and `/private/tmp/container-gelf-tcp-failure-candidate-v6.bF5VQ7`; the complete scope and non-goals are in the [slice ledger](slice-ledger.md#logging-gelf-rest-01), [TCP slice ledger](slice-ledger.md#logging-gelf-tcp-rest-01), [TCP reconnect slice ledger](slice-ledger.md#logging-gelf-tcp-reconnect-rest-01), and [TCP failure slice ledger](slice-ledger.md#logging-gelf-tcp-failure-rest-01). The older UDP/TCP candidates are 1.72–2.30× Docker, reconnect and retry exhaustion are near the reference, and unavailable endpoint remains 6.38–7.00×; all remain below the fixture's 10× functional guard, but the programme-wide comparable-or-better performance gap is still open. The public create response's name-as-ID mismatch is outside this logging contract and is tracked in [Container issue 74](https://github.com/stephenlclarke/container/issues/74).
