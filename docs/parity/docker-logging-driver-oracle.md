...
The 0.14.0 graph can resolve the coordinated Container, Containerization, and Engine API revisions from their owned remotes. A post-release candidate that tests uncommitted or unpublished revisions must still use identity-preserving local package roots. The target records the selected graph before runtime validation and restores `Package.resolved` afterwards.
