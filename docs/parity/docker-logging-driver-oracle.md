...
The matching fork-only native correction maps the Docker VM alias to loopback only when a macOS Fluentd TCP client dials; it leaves stored configuration and TLS requested identity unchanged. The focused 12-test loopback suite and 92.28% line-coverage report pass. Its public candidate and timing comparison are `Queued`, not passed: [RUNTIME-ISOLATED-PUBLIC-SOCKET-01](handoffs/RUNTIME-ISOLATED-PUBLIC-SOCKET-01.md) proves the candidate runner now scopes `container system stop` to a unique namespace, but the Fluentd contract itself has not run or produced performance evidence. The exact boundary, retained roots, and completion criteria are in [LOGGING-FLUENTD-TCP-ACK-REST-01](handoffs/LOGGING-FLUENTD-TCP-ACK-REST-01.md).
