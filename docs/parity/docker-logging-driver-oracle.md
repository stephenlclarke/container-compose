...
`LOGGING-GELF-REST-01` passed a fresh UDP Docker reference in `0.680846750s` and three fresh exact candidates in `1.567608333s`, `1.526945791s`, and `1.461681958s`. `LOGGING-GELF-TCP-REST-01` passed a fresh TCP reference in `0.931894667s` and three exact candidates in `1.603390667s`, `1.700857709s`, and `1.975864916s`. The source, local/transitive dependency graph, release archive, signed binaries, guest/init archive, bootstrap image, script, wrapper, Docker oracle, and result files are retained under `/private/tmp/container-gelf-rest-v4.AR1wW6` and `/private/tmp/container-gelf-tcp-rest-v3.Zd0bfm`; the complete scope and non-goals are in the [slice ledger](slice-ledger.md#logging-gelf-rest-01) and [TCP slice ledger](slice-ledger.md#logging-gelf-tcp-rest-01). The candidate timings are 1.72–2.30× Docker, so they satisfy the fixture's 10× functional guard but do not close the programme-wide comparable-or-better performance gap. The public create response's name-as-ID mismatch is outside this logging contract and is tracked in [Container issue 74](https://github.com/stephenlclarke/container/issues/74).
...
The oracle does not provision external logging services or installed logging plugins. Cloud-driver delivery, a blocking slow-sink backpressure case, dual-cache rotation under sustained load, and plugin lifecycle behavior still require separate focused oracles. GELF's direct Engine UDP/TCP wire, configuration/validation, selected metadata/tag behavior, and narrow cache-disabled UDP/TCP public-socket lifecycles are captured here; the component and public-socket proof is retained in [LOGGING-GELF-REST-01](slice-ledger.md#logging-gelf-rest-01) and [LOGGING-GELF-TCP-REST-01](slice-ledger.md#logging-gelf-tcp-rest-01). The `local` Docker-CLI/public-socket candidate lane is verified separately in [LOGGING-LOCAL-REST-01](slice-ledger.md#logging-local-rest-01); all other candidate-runtime and external-client certification remains separate work.
