...
Docker accepts a `tls://` Fluentd address, but its Fluentd option grammar exposes no CA path or skip-verification option. The versioned `tlsLocalTrustFailure` probe freezes the boundary: against its bounded self-signed receiver, Engine 29.2.1 accepts container creation with HTTP 201 and rejects start with HTTP 500: `tls: failed to verify certificate: x509: certificate signed by unknown authority`; the receiver independently observes the resulting TLS bad-certificate alert. Fresh `/private/tmp/container-rest-fluentd-tls.docker.m7QDss/result.json` passes in `0.31440287502482533` seconds. The first exact Container candidate built from `66e0cac3c7e86147d3cb5e26c5dc68fe2a987d4f` now matches Docker's public start diagnostic and `created` state, but strict fixture evidence at `/private/tmp/container-rest-fluentd-tls.candidate-66e-1.tBR6n1` records `SSLV3_ALERT_CERTIFICATE_UNKNOWN`. The active source mapper is therefore not a Fluentd TLS parity certificate: the remaining Darwin SwiftNIO SSL custom-verifier bridge must set BoringSSL's `outAlert` to `bad_certificate` without changing trust policy, then two fresh candidates must pass. Capturing decrypted Fluentd TLS wire bytes would require a deliberate Colima trust-store mutation or a publicly trusted endpoint. This harness does neither.
