...
| Compose `cp -` archive metadata | Host-staged stdin/stdout content copies match Docker Compose v5.3.1 for content, mode, symlink, sparse allocation, long-path, and large-file fixtures. Arbitrary UID/GID and timestamps are not preserved and hard-link entries are rejected. Compose now reports partial support; complete parity remains blocked on reviewed direct-stream work in [apple/containerization#812](https://github.com/apple/containerization/pull/812) and [apple/container#1947](https://github.com/apple/container/pull/1947). |
...
- Track third-party direct-stream drafts `apple/containerization#812` and
  `apple/container#1947` without importing them. Reassess only after both
  proposals are ready for review and their metadata, cancellation,
  backpressure, and path-safety contracts are complete.
