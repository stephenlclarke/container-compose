...
A second pass moved create-time Docker/Compose projection into this repository without changing live execution yet. `container-compose` now builds typed values for local logging policy, healthcheck inheritance and overrides, restart policies, host entries, sysctls, and block I/O resource data. The existing `container run/create` command-vector path remains the live execution path until the typed service-creation adapter is ready, but the Compose-shaped parsing and validation no longer needs to live in Apple's CLI parser layer.

No other local `container` source changes can be moved directly without replacing the create/run execution path. The remaining broad-branch deltas are either non-emulatable lower runtime behavior, typed Apple client/resource primitives, or runtime schedulers/observers. The next code-heavy migration in this repository is a typed service-creation adapter that constructs `ContainerConfiguration` and `ContainerCreateOptions` directly, while keeping Docker-shaped command vectors as dry-run presentation.
...
2. Add a typed service-creation adapter in `container-compose` that builds `ContainerConfiguration` and `ContainerCreateOptions` directly, with the existing command-vector path retained only for dry-run rendering and temporary compatibility.
3. Connect the typed healthcheck, restart, hostname/domainname, host entry, sysctl, blkio, and logging policy projections to that adapter so live create no longer depends on Docker-shaped `container run/create` flags.
