...
`--change VOLUME` targets. Three focused archive tests pass, including
duplicate and multi-target cases. The strict live probe builds matched base
images with `VOLUME ["/image-data", "/shared-data"]`, adds `/added-data` and
`["/logs", "/shared-data"]`, and confirms Docker Compose v5.3.1 and the
installed Container-backed Compose binary both retain the same four targets.
No Apple runtime fork or stack pin changes.
