...
- Map Compose `extra_hosts` `host-gateway` entries to runtime `--add-host`.
...
The local container fork now accepts `--add-host host:host-gateway` and resolves it against the first runtime network gateway. This plugin change passes the Compose value through to that generic runtime primitive. Compose-specific behavior stays in `container-compose`; gateway resolution stays in `apple/container`.
...
- Added orchestration coverage proving `up` emits `--add-host host.docker.internal:host-gateway`.
