...
- Recover an interrupted multi-entry publication only when a private host transaction identifies the exact guest journal; prepared names never authorize deletion, publishing records bind every entry to its staged device/inode plus complete-tree identity, and exclusive renames cannot overwrite a concurrently created destination. Before recursive rollback, atomically quarantine a matching published tree back into the private transaction stage and authenticate the complete tree again; concurrent changes are restored and fail closed.
