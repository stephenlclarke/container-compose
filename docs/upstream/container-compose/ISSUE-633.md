...
- Recover an interrupted multi-entry publication only when a private host transaction identifies the exact guest journal. Prepared names never authorize deletion. Once publishing can have started, the journal binds every entry to its staged device/inode plus complete-tree identity and records the final root metadata; recovery completes that authenticated transaction forward through exclusive renames. It never recursively deletes published or staged data, so writes through retained external handles cannot be destroyed; changed, missing, duplicated, or unjournaled trees fail closed and remain available for diagnosis.
...
- Restore the original mount-root owner, group, mode, and extended attributes when a prepared transaction has no published data. Apply the recorded source-root metadata after a publishing transaction is completed forward, then synchronize every published path and journal removal before accepting completion.
...
- Deterministic Unix-socket component tests cover the Docker-free build context, OCI platform variants, platform/name/bytes-derived helper identity, Homebrew symlink resolution, verified local-image snapshots, copy-up, entry and root extended attributes, off-volume recovery state, pre-mutation ext4 metadata capture, crash recovery including complete mount-root metadata restoration, durable forward-only publication, transaction hardening, user-data preservation, managed-volume service launch, native exec, ownership-preserving request projection, concurrent serialization, request routing, content-addressed helper placement, and helper cleanup.
