...
- A generated `links` or `external_links` host entry cannot reuse a source
  `extra_hosts` hostname or an alias generated by another link, using
  case-insensitive hostname matching; ambiguous static-host mappings are
  rejected before side effects.
- `external_links` uses the same safe static-host approach when the source and
  external container share exactly one runtime network; the source can retain
  other network attachments.
...
- `external_links` uses the same direct-runtime-inspection and generated-host
  approach when the source and external container share exactly one runtime
  network, even if the source has other attachments.
