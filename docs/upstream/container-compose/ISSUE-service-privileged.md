...
- Runtime dry-run and live guest-path smokes cover a compose.yml with service-level `privileged: true`.
...
The current runtime mapping grants the service init process all Linux capabilities and restores the generic Linux guest paths that Containerization masks or mounts read-only by default. It does not implement Docker's full privileged behavior for arbitrary host devices, device cgroups, seccomp, AppArmor, vendor/native GPU passthrough, or other host-isolation boundaries.
