...
- `domainname` is handled by the separate
...
- `links` and `external_links` have current safe single-network subsets.
  Multi-network links, shared aliases, and source-scoped DNS remain separate
  runtime gaps.
