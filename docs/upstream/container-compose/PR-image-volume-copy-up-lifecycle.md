...
- `ComposeOrchestratorImageVolumes` chooses implicit or explicit volumes only for Dockerfile-declared targets, skips `nocopy` and non-volume masks, and invokes initialization before the `container run` handoff.
...
- This change covers only Dockerfile-declared image volume targets. Docker's generic copy-up for other volume mounts and `volume.subpath` initialization remain intentionally rejected/unsupported rather than silently degraded.
