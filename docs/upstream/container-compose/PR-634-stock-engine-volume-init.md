...
- `Sources/ComposeEngineRuntime/ComposeEngineRuntime.swift`
  ...
  - hashes the source digest and static initializer to identify a reusable local helper image;
  ...
  - stages the complete tree and rolls back every published entry if publication fails;
...
- `Tests/ComposeEngineRuntimeTests/ComposeEngineRuntimeTests.swift`
  - proves build-context projection, first-use copy-up, managed-volume launch projection, native exec projection, ownership/mode request projection, cross-provider serialization, existing-data preservation, platform projection, lock-file hardening, non-overlapping helper mounts, and helper cleanup over a real local Unix-socket test server.
