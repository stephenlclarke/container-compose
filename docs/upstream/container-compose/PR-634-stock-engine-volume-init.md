...
- Package native static initializer binaries for Linux arm64 and amd64, preserve extended attributes, and make publication durable before removing its recovery journal.
...
- `Sources/ComposeEngineRuntime/ComposeEngineRuntime.swift`
  ...
  - resolves the installed executable through Homebrew symlinks before locating packaged helpers;
  - parses the requested OCI platform, selects the static Linux arm64 or amd64 helper for both base architectures and valid variants, preserves the complete platform for Engine requests, and rejects unsupported operating systems or architectures;
  - hashes the source image identity, platform, helper name, helper bytes, and in-image path to identify a reusable local helper image;
  ...
  - derives an exact helper executable path from the immutable source-image identity, platform, helper identity, and copied subtree, making a collision with a pre-existing source-image directory cryptographically self-invalidating, while selecting a separate volume-mount path that cannot overlap or obscure the requested image subtree;
  - overrides the source image's user and entrypoint, so scratch and distroless source images need no shell or utilities;
  - copies the selected image directory inside the Linux guest through the project-owned static helper, preserving files, directories, ownership, modes, timestamps, root and entry extended attributes such as `security.capability` and ACLs, symlinks, hard links, and named pipes;
  ...
  - performs authenticated recovery before checking the current source path, restores the original mount-root owner, group, mode, and extended attributes from the versioned journal, and synchronizes rollback deletions before the volume can be accepted as empty;
