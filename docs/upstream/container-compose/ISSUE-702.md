...
Cleanup must never claim an unmarked root, follow a symbolic link, cross into retained storage, create a cleanup target before acquiring the build lock, or run beside an active build; partial mutation must retain a failure receipt whenever its destination remains writable. GitHub deletion must reject a stale supplied default branch, revalidate the exact SHA, absence of an open pull request, and exact merged-pull-request proof immediately before changing the ref, then atomically require that same SHA at the server and journal the deletion before fallible reconciliation. It restores the exact head if a new or reopened pull request became active during deletion or post-delete evidence is unavailable. If current or later branch processing fails, the already-completed decisions and terminal error must still be retained. Tags, releases, issues, workflow runs, Actions caches, artifacts, and unrelated local worktrees are outside the deletion boundary.
