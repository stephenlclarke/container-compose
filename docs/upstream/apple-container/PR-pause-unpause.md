...
This change adds pause/unpause lifecycle controls to the local `container` integration branch.
...
Following JLogan's 2026-06-23 guidance in [apple/container#1769](https://github.com/apple/container/pull/1769#issuecomment-4780439328), pause/unpause remains an Apple-native resource-management primitive. Compose service selection and Docker Compose command semantics stay in `container-compose`.

The change builds on the local `stephenlclarke/containerization` fork branch, which adds the missing public `LinuxContainer.pause()` and `LinuxContainer.resume()` bridge over the already-present VM pause/resume hooks.
...
- Updated the SwiftPM pin to `stephenlclarke/containerization@integration/blkio-runtime` revision `e172174`, which contains the lower-level runtime bridge.
...
This branch intentionally depends on the local `stephenlclarke/containerization` fork while the lower-level pause/resume `LinuxContainer` bridge is not yet available from upstream `apple/containerization`.
