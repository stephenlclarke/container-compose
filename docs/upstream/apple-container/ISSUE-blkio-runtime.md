...
The current fork implementation builds on those PRs rather than replacing them. The useful Apple-facing shape is the lower-runtime `Containerization.LinuxBlockIO` model plus an `apple/container` typed runtime-data bridge. [apple/container#1595](https://github.com/apple/container/pull/1595) also proposes a repeatable key-value CLI option:
