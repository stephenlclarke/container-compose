...
file parsing. `ComposeRuntimeSPI` provides a useful injection boundary, but
`ComposeCore` still imports and publicly exposes Apple package types. At the
audit baseline, the three support forks contained 414 non-merge commits beyond
the reviewed Apple upstream heads. Current divergence is generated by
`make upstream-divergence-report`. The retained delta gives the project
valuable capabilities, but creates a large review, release, and convergence
burden.
