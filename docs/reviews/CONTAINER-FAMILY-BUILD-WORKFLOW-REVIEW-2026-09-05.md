...
The current native recoverable graph now treats residue and repository hygiene as build-system contracts. A locked preflight removes only allowlisted direct children of the marker-owned external transient root, and an always-run postflight repeats the operation after success, failure, cancellation, or a handled signal. Empty live scratch roots are recreated before the next command, while exact pins, content-addressed artifacts, timing logs, and cleanup receipts remain internal and retained. Human and JSON receipts make every removal auditable. The scheduled GitHub hygiene workflow separately removes only unchanged, unprotected heads proved to belong to pull requests merged at least seven days earlier; it revalidates the SHA, open-pull-request state, and merge proof immediately before deletion, restores an exact head with create-only semantics if pull-request activity begins during deletion, and reports every retained branch.
