...
The same build lock encloses residue reconciliation. Unlocked preflight establishes only the exact ownership markers and retained lock/evidence directories; it never creates a cleanup target. After the lock is held, the controller removes only allowlisted direct children of the exact marker-owned transient root and recreates empty `scratch` and `process-tmp` directories. The native build then runs in its bounded supervised process session. After that session has drained, an always-run outer exit trap performs postflight cleanup under a separate 120-second deadline, so expiration of the build deadline cannot kill its own cleanup. The trap covers success, failure, cancellation, and handled signals. Cleanup never touches the internal retained store, source checkouts, an unmarked root, or a path reached through a symbolic link. Human-readable and JSON receipts for successful and partially failed phases are retained below `$(STACK_RETAINED_ROOT)/hygiene`; failure evidence records completed mutations and the terminal error whenever the destination remains writable. `make stack-transient-clean-plan` reports the same allowlist without changing it; `make stack-transient-clean` performs the locked cleanup explicitly.
