...
CodeQL is a release-only required authority. The package workflow runs it only
after distribution configuration has passed fail-fast checks and before native
release builds begin. Pull requests, ordinary `main` checkpoints, schedules,
documentation, and benchmark reports do not dispatch it.
...
The current repository routes selected release work and the nested-virtualization Current demo to its self-hosted MBP labels. Independent DocC sites use hosted macOS fan-out before hosted Pages assembly. Release-only CodeQL uses a hosted Linux prerequisite job, while the supported local CodeQL target remains available for reproducible release preparation and evidence.
...
- Keep the GitHub CodeQL authority on its isolated hosted Linux runner. Run it only from the package release graph, after Homebrew preflight and before native package builds. The standalone workflow is manual published-release recovery only.
...
4. <a id="workflow-enabler-04"></a>`WORKFLOW-ENABLER-04` — Local wrapper targets for SwiftLint, SwiftFormat, Address Sanitizer, and Thread Sanitizer so the Quality workflow can be reproduced without a hosted runner. The repository now runs its `actionlint` adapter through `make check` and installs the pinned `v1.7.12` tool in CI; the remaining wrappers are still outstanding.
