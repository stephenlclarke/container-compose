...
| Item | Value |
| --- | --- |
| Status | Active overall. Stable 0.15.1 retains the core logging architecture, durable native histories, maintained remote/provider implementations, exact-process attachment, public REST lifecycle, and the focused Docker Compose and Docker CLI certificates introduced through the 0.14.2 design checkpoint. The remaining external-client/provider/failure/migration/security matrix, focused GELF retry and Unix-socket validation paths, cold-runtime/resource collectors, and comparable release-artifact performance evidence remain open. |
...
| Stable release | [`container-compose` 0.15.1](https://github.com/stephenlclarke/container-compose/releases/tag/0.15.1); the 0.14.2 graph below is retained as the design checkpoint at which this work was reviewed |
| Public-build Container revision | `a252482bbacbd15742845764893585131e2c4825` |
| Compose checkpoints | The 0.14.2 source is the retained reviewed design checkpoint; earlier signed checkpoints below preserve the implementation and oracle history that led to it |
...
| Matched Containerization checkpoints | 0.14.2 candidate revision `b404e03bb914904107a6a9305ba1f0e44c79a59c`; earlier validation checkpoints remain recorded below |
| Shared Engine API | The candidate Container graph resolves `container-engine-api` at `386a40c726ecd25d67a3e5933582aebbfbe4fa2f`; earlier signed handoff heads below retain the provider-control, public-lifecycle, bounded-history, framing, trust, and exact-process validation history |
...
The retained 0.14.2 design checkpoint required
...
package; the 0.14.2 Stable Release Gate remains pending. The remaining blockers
