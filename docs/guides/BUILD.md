...
Every full stack build performs locked transient cleanup immediately before native work and through an always-run postflight trap. Unlocked preflight establishes only the ownership markers and retained lock/evidence directories; the cleanup controller creates the live `scratch` and `process-tmp` directories only after the build lock is held, so a second invocation cannot recreate them during another build's cleanup. Allowlisted dangling links are discovered without following them, unlinked as entries, and replaced by empty live directories where required. The build itself runs inside `STACK_BUILD_TIMEOUT_SECONDS` with `STACK_BUILD_TERMINATION_GRACE_SECONDS`; after that supervised session drains, postflight runs outside it with the independent `STACK_CLEANUP_TIMEOUT_SECONDS` bound. A timed-out build therefore cannot terminate its own cleanup. This covers successful, failed, cancelled, and handled-signal exits without weakening recovery: exact pins, promoted artifacts, timing logs, and cleanup receipts remain under the internal retained root, while `attempts`, `builds`, `downloads`, `scratch`, `process-tmp`, and legacy `tmp` entries are the only removable external names. Cleanup refuses an unmarked, indirect, or concurrently active root. Successful and failed phases both write Markdown/JSON receipts under `$(STACK_RETAINED_ROOT)/hygiene`; a partial failure records every completed removal, every successfully recreated directory, and the terminal error before returning failure whenever the receipt destination remains writable.

Remote branch cleanup is separate from build storage. `.github/workflows/repository-hygiene.yml` runs weekly or on demand, resolves the repository API's current default branch before checkout, then gives the controller the event-supplied branch only as a stale-state expectation. This lets a renamed default branch check out successfully and produce the promised failure report before any mutation. It deletes only unchanged, unprotected branch heads with an exact pull request merged into the authoritative default branch at least seven days earlier. It rechecks the SHA, open pull requests, the complete branch pull-request snapshot, merge proof, and current repository default immediately before an atomic Git `--force-with-lease` deletion bound to that SHA; a default-branch rename or concurrent push preserves the branch, while a concurrent deletion is recorded as already reconciled. A successful deletion is provisionally journaled before fallible reconciliation. The controller then queries pull-request activity again and restores the exact deleted head with a create-only GitHub reference mutation if a new or previously closed pull request became active during the deletion window. An unavailable post-delete query is treated as ambiguous and triggers the same create-only restoration; a concurrently recreated or changed branch is preserved rather than overwritten. Default, protected, `upstream/`, `release/`, legacy `release-`, `archive/`, active, changed, and ambiguous branches are retained. The workflow never deletes tags, releases, issues, Actions evidence, caches, or workflow runs. Its always-run artifact contains atomic Markdown/JSON evidence for successful or partially failed runs, including every completed decision and the terminal error.
...
- Every push to `main`, including documentation and workflow-control changes, runs full runtime validation and is eligible to publish one immutable, content-addressed `current-<full-sha>` GitHub prerelease named **Current build** only while its pinned Container, Containerization, and builder-shim revisions still match their sibling repositories' `main` branches. Publication requires exact-revision CI and SonarQube authority, rebuilds the package for that repository revision, and atomically updates the opt-in `container-current` / `container-compose-current` Homebrew pair. The workflow rejects a revision already superseded when its pre-publication freshness check runs and verifies the exact published closure again after publication. If Compose `main` or a sibling head advances during the final tap-and-release window, that closing check fails visibly and the later exact-main run replaces the temporarily installable candidate with the newest coherent stack.
...
**Scheduled Stable Release** runs every Monday at 09:17 UTC. When the Current build has soaked for seven days and `main` contains source newer than the latest semantic tag, it derives the next version from first-parent Conventional Commit history. It ends successfully without allocating the release runner when there is no release-producing commit or the soak is incomplete, so an unready week is not a failed release. Manual dispatch is the general unattended controller: select milestone, maintenance, or security intent; choose `auto`, `--+`, `-+-`, `+--`, or an exact `MAJOR.MINOR.PATCH`; supply the required maintenance/security reason or exceptional milestone soak-override reason. The same exact helper then resumes or completes the release without a terminal prompt. After it dispatches or reuses the candidate-bound Stable Release Gate, it records the exact run ID and exits the self-hosted bootstrap job. A hosted continuation waits for that run, dispatches and waits for the exact package and documentation runs, and verifies the immutable release and tap. The sole release runner is therefore free to execute each downstream gate instead of being occupied by its own waiter.

The scheduled stable-release workflow and the Current package workflow run only from `main` on the dedicated `container-compose-release` Apple-silicon self-hosted runner. It creates clean, disposable stack checkouts, reconstructs the read-only Apple remotes and Stephen-owned push remotes, and invokes the exact checked-in helper in its internal handoff mode. Normal operator invocations of that helper remain synchronous. This preserves the required local runtime and Docker Compose parity gate, signed semantic tag, source-promotion pull request, stable gate, immutable package assets, and paired Homebrew update without making the runner wait for itself. The separate Current Demo workflow consumes the already-published exact-SHA signed packages on the same hardware-virtualization-capable runner, uses a stable internal-volume runtime path, and applies a process-group deadline. Its exact-commit visual output is retained as a bounded workflow artifact and remains deliberately outside the package, attestation, release, and Homebrew critical path. It never modifies an immutable release. GitHub-hosted macOS workers cannot provide the nested virtualization needed to record Container guest startup, so they must never publish that recording.
...
The installer verifies hardware virtualization, the Git author identity and SSH tag- and commit-signing configuration, that the signing key can operate without an interactive passphrase, the local `gh` account, the release toolchain, and the names of every required unattended repository secret/variable before it registers a repository-only runner and starts its standard `launchd` service. Rerun the same command during normal release maintenance: it queries the latest GitHub Actions macOS ARM64 runner asset, verifies its published SHA-256 digest before stopping anything, and, when the configured runner is stale, replaces only the runner program files, confirms the installed version, and restarts the existing registration and `launchd` service.

Release jobs authenticate GitHub with the purpose-specific `CONTAINER_FAMILY_RELEASE_TOKEN` Actions secret rather than reading the login Keychain. Before any long validation, the controller imports `DEVELOPER_ID_APPLICATION_P12_BASE64` with `DEVELOPER_ID_APPLICATION_P12_PASSWORD` into a run-owned temporary keychain, requires it to match `DEVELOPER_ID_APPLICATION_SIGNING_IDENTITY`, checks certificate validity, and completes a timestamped hardened-runtime signing probe with closed standard input and a finite deadline. The identity fingerprint is then passed to the unchanged release helper. An `always()` cleanup restores the exact prior keychain search list and deletes the temporary keychain. The helper publishes its restoration record only after a successful capture; if restoration or deletion fails, it retains that record and fails so cleanup can be retried without clearing the user's search list. A missing secret, wrong identity, locked credential, signing failure, or timestamp-service failure therefore stops during preflight instead of displaying or waiting behind a login-Keychain approval dialog.
...
A Current package accepts only an exact-main successful CI run with a passed SonarQube scan and retained per-metric history, whether that CI was triggered by a push or by an explicit full-validation dispatch. It waits within the configured quality-evidence window for the exact analysis and all required metrics to finish indexing. Documentation-only pull requests may skip the runtime lanes before merge, but protected `main` still establishes full runtime and SonarQube authority and publishes a content-addressed Current candidate for that exact repository revision.
