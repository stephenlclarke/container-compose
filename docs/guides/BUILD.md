...
Every full stack build performs locked transient cleanup immediately before native work and through an always-run postflight trap. The build itself runs inside `STACK_BUILD_TIMEOUT_SECONDS` with `STACK_BUILD_TERMINATION_GRACE_SECONDS`; after that supervised session drains, postflight runs outside it with the independent `STACK_CLEANUP_TIMEOUT_SECONDS` bound. A timed-out build therefore cannot terminate its own cleanup. This covers successful, failed, cancelled, and handled-signal exits without weakening recovery: exact pins, promoted artifacts, timing logs, and cleanup receipts remain under the internal retained root, while `attempts`, `builds`, `downloads`, `scratch`, `process-tmp`, and legacy `tmp` entries are the only removable external names. Cleanup refuses an unmarked, indirect, or concurrently active root. Its Markdown and JSON receipts are retained under `$(STACK_RETAINED_ROOT)/hygiene`.

Remote branch cleanup is separate from build storage. `.github/workflows/repository-hygiene.yml` runs weekly or on demand and deletes only unchanged, unprotected branch heads with an exact pull request merged into the default branch at least seven days earlier. It rechecks the SHA, open pull requests, and merge proof immediately before an atomic Git `--force-with-lease` deletion bound to that SHA; a concurrent push loses the lease and preserves the branch, while a concurrent deletion is recorded as already reconciled. It then queries pull-request activity again and restores the exact deleted head with a create-only GitHub reference mutation if a pull request opened during the deletion window; a concurrently recreated or changed branch is preserved rather than overwritten. Default, protected, `upstream/`, `release/`, legacy `release-`, `archive/`, active, changed, and ambiguous branches are retained. The workflow never deletes tags, releases, issues, Actions evidence, caches, or workflow runs, and it retains a Markdown/JSON decision report for every run.
