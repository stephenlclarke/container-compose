...
Every full stack build performs locked transient cleanup immediately before native work and through an always-run postflight trap. Unlocked preflight establishes only the ownership markers and retained lock/evidence directories; the cleanup controller creates the live `scratch` and `process-tmp` directories only after the build lock is held, so a second invocation cannot recreate them during another build's cleanup. The build itself runs inside `STACK_BUILD_TIMEOUT_SECONDS` with `STACK_BUILD_TERMINATION_GRACE_SECONDS`; after that supervised session drains, postflight runs outside it with the independent `STACK_CLEANUP_TIMEOUT_SECONDS` bound. A timed-out build therefore cannot terminate its own cleanup. This covers successful, failed, cancelled, and handled-signal exits without weakening recovery: exact pins, promoted artifacts, timing logs, and cleanup receipts remain under the internal retained root, while `attempts`, `builds`, `downloads`, `scratch`, `process-tmp`, and legacy `tmp` entries are the only removable external names. Cleanup refuses an unmarked, indirect, or concurrently active root. Its Markdown and JSON receipts are retained under `$(STACK_RETAINED_ROOT)/hygiene`.
