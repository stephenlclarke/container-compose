...
This guide describes the [`0.14.0` stable lane](https://github.com/stephenlclarke/container-compose/releases/tag/0.14.0), published on 28 August 2026, and the rolling `current` lane. A later immutable semantic release supersedes 0.14.0 automatically in the stable formulae; `container compose version` and `container system version` report the exact installed pair.
