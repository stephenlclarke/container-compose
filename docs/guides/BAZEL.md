...
`test-unit` selects the explicit `unit_stock` or `unit_enhanced` suite, matching the package's provider-conditional test modules. Both include all six Go test targets, focused private-storage regressions and runtime-neutral Core tests. Stock now runs 284 existing Core cases without importing either concrete provider; enhanced retains all 1,246 Core cases and provider fixtures. The explicit 14-file provider-coupled inventory is identical in Bazel and Package.swift, so new neutral suites run in both profiles by default. The remaining 962 enhanced Core cases still need separation from concrete provider helpers before stock can claim equivalent Core coverage. The opt-in live `ComposeRuntimeTests` suite is not silently counted as a unit pass; its runtime-lease adapter remains unfinished.
