...
| Verified Syslog UDP public-socket certificate | Signed local Container `5ebce8a2aa90bf96775285e1a79ac48b375f69c5` fixes native macOS routing of `host.docker.internal` for the fork-only Syslog provider while retaining TCP/TLS identity. The focused TCP/UDP transport regression and Docker CLI 29.7.1 / Engine 29.2.1 reference plus two exact candidates pass cache-disabled `rfc5424micro` delivery, ordering, facility/severity, tag, inspect, unreadable history, native authority, and cleanup. Candidate 3.08×/2.31× Docker timings satisfy the 10× functional guard only; [LOGGING-SYSLOG-UDP-REST-01](parity/handoffs/LOGGING-SYSLOG-UDP-REST-01.md) retains the exact fingerprint and remaining scope. |
