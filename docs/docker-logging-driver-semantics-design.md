...
| Compose policy | [`ComposeOrchestratorRuntimeSupport.swift`](../Sources/ComposeCore/ComposeOrchestratorRuntimeSupport.swift) now carries the lossless request into config, hashing, and create planning while retaining the v1 runtime allowlist at the execution boundary. Runtime preflight preserves optional and unknown installed capability identifiers separately from the mandatory manifest and injects the immutable selection into execution options. | Arbitrary requests remain deliberately unavailable until the lower runtime advertises and implements the logging capability. |
| Compose runtime SPI | [`ComposeRuntimeCreateModels.swift`](../Sources/ComposeRuntimeSPI/ComposeRuntimeCreateModels.swift) carries the exact optional driver and complete option map; [`ComposeRuntimeLogging.swift`](../Sources/ComposeRuntimeSPI/ComposeRuntimeLogging.swift) adds driver-neutral binary records, historical-read requests, and a distinct live-attach contract; [`ComposeRuntimeCapabilities.swift`](../Sources/ComposeRuntimeSPI/ComposeRuntimeCapabilities.swift) owns the exact optional logging-v2 identifier. | Capability discovery is ready, but the new record and attach contracts are not yet backed by the Container authority and the identifier is not yet advertised. |
...
- Work package 1 has deterministic Engine 29.2.1 and Compose 5.3.1 fixtures for every locally stable core case, including pressure, built-in rotation/compression, foreground fallback, complete runtime fingerprints, and monotonic case timings. Provider wire payloads, blocking slow-sink behavior, sustained dual-cache rotation, plugins, and cloud providers remain open as explicitly tracked oracle families.
- Work package 2 preserves logging through compose-go normalisation, Swift decoding, offline config, stable hashing, create planning, and a guarded v1 down-conversion. Structured `logging` is authoritative over retained legacy fields. Runtime preflight now discovers the optional logging-v2 identifier without adding it to the mandatory capability set, retains unknown identifiers for forward compatibility, and passes the selection into runtime execution options.
