...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; local Compose typed execution, the complete production journald path, enhanced Engine logs/inspect/info/hijack/WebSocket/resize routes, the local public gateway lifecycle, isolated Docker-plugin hosting/direct reads, durable provider-generation cutover, and both sides of the immutable devcontainer logging transfer are implemented and focused-tested; whole-stack gateway/root cutover rehearsal, dependency publication, third-party certification, and paired release evidence remain |
...
| Shared Engine API | Released `container-engine-api` 0.3.5 at signed commit `78cb4cb5781d6dbe9f0d34a1b925ee8dcaacdc98`; active signed local handoff head `2ed00204c28589be6480ab06635933b8a992f67b` adds provider control, source export, signed manifest assembly, portable logging payloads, sealed object publication, and crash-replayable contribution state |
| Logging handoff checkpoints | Signed local Container heads `170b8769e3105169b4a01d99ac3bc93cd860d6fc` and `8005bf8fb8036da0b15d63763d5f51dff12cd5eb` implement and verify destination staging/re-resolution, protected receipt, abort compensation, signed promotion, Complete-only activation, immutable history publication, cursor adoption, and the provider-neutral devcontainer payload through that destination flow; signed local `devcontainer` head `385cf69878a87e67589eb36fff2bc3c9dd66695f` adds quiesced Apple Container history export, durable provider identity/key enrollment, object transport, source contribution replay, and authenticated private-session wiring |
| Last implementation review | 4 August 2026 |
...
- Work package 10 is in progress. Signed local Engine API head `2ed00204c28589be6480ab06635933b8a992f67b` supplies the authenticated provider-control protocol, destination-key proof, crash-replayable source contributions, signed manifest assembly, content-addressed sealed objects, and the bounded provider-neutral Docker `json-file` history package. Signed local devcontainer head `385cf69878a87e67589eb36fff2bc3c9dd66695f` exports stopped, quiesced Apple Container stdout/stderr history including rotations and wires durable provider key enrollment plus the source responder into both private and embedded provider sessions. Signed local Container head `170b8769e3105169b4a01d99ac3bc93cd860d6fc` already provides the production destination controller: collision-free staging and local re-resolution, protected-option resealing and exact receipt, signed-commit promotion, immutable local/provider-history adoption, new cursor epoch, signed-Complete activation, and abort compensation/replay. Signed head `8005bf8fb8036da0b15d63763d5f51dff12cd5eb` now drives the provider-neutral devcontainer package through stage, exact-replay promotion and activation; the staging suite passes 8/8, and the signed responder suite passes 5/5 in consecutive parallel runs after canonicalising possession-proof fixture digests. Existing focused Engine API payload/manifest, production payload-decoder, devcontainer record-projection, and private provider-session tests also pass on the MBP. Remaining work is one actual gateway-driven multi-provider/root-CAS rehearsal that rematerialises the destination container, proves public history reads plus a new writer across the imported epoch, and exercises crash/abort recovery end to end; histories beyond the current 64 MiB source bound; synchronized dependency publication; and the complete security/performance matrix.
