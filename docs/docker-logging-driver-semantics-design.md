...
| Shared Engine API | `container-engine-api` implements Docker-shaped `/info`, inspect `LogConfig`, logs, attach/hijack, version routing, framing, query parsing, and error envelopes against an abstract authoritative backend. | The package is not yet integrated with Container's authority/gateway, socket lifecycle, events, or the devcontainer shared API boundary. |
...
- Work package 8 has the logging-specific Docker REST/streaming routes in `container-engine-api`; integration with Container's authority, shared gateway/socket, lifecycle events, and external-client suites remains unimplemented.
