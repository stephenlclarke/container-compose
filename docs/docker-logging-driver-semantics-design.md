...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; the journald codec/provider contract, generation-fenced shared-sandbox transport, and bounded reconnect-safe client wire are implemented, but the production Linux service is not; plugin hosting, Engine integration, handoff, and release evidence remain |
...
| Local Container development head | `bc3f3c1c1f9b29a966d2673436092461154efd31` (`887848ed719a05836d2f846b69a22749e61f2f62` journald provider; `20071d97d10b386c2a24c84c51bca0e37c0280aa` shared-service transport; `a42ecf2fe1ffa582e34cfa74f6cf1ddba8505368` journald client wire) |
...
| Remote providers | Syslog, Fluentd, GELF, Splunk HEC, AWS Logs, and Google Cloud Logs have option contracts, authority activation, recovery, and focused semantic tests. AWS uses an isolated official SDK adapter. Google Cloud uses the official Go client inside the signed semantic helper, with Application Default Credentials and Moby-compatible project, instance metadata, payload, asynchronous delivery, overflow, flush, and close behavior. Journald now has its Moby-compatible option/field codec, direct-reader and generation-fenced provider lifecycle, conditional catalog registration, narrow signed Linux-service boundary, an exact-generation shared-sandbox XPC/vsock connection, and a versioned one-MiB framed client protocol with exact response-loss replay and ordered reader events. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | Journald remains correctly unadvertised because its production signed Linux workload, wire server/replay cache, system-journal persistence/query adapter, production reader routing, supervision, and recovery are absent. The isolated production Docker-plugin service plane is also absent. |
