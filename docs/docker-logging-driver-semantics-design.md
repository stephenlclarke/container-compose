...
| Item | Value |
| --- | --- |
| Status | Active overall; the Docker Compose 5.3.1 static `none` history/foreground reliability contract plus the Docker CLI `local` public-socket lifecycle, compressed rotation, deferred invalid-start inspect contract, and narrow GELF UDP public-socket lifecycle are Verified, while synchronized dependency publication, the remaining external-client/provider/failure/migration/security matrix, cold-runtime/resource performance collectors, release-artefact results, and release evidence remain Queued |
