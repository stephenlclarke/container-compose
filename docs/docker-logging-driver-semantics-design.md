...
| Matched Container revision | `2a79b4553a342e33411666a88ad20ccd2ce46551` |
...
| --- | --- | --- |
| Compose normalisation | [`Tools/compose-normalizer/main.go`](../Tools/compose-normalizer/main.go) emits a typed optional driver and complete compose-go-normalised string option map, preserving an explicit empty driver separately from an omitted logging object. | Normalisation is implemented; the compose-go 2.13.0/2.14.0 skew remains a whole-project differential input. |
| Compose policy | [`ComposeOrchestratorRuntimeSupport.swift`](../Sources/ComposeCore/ComposeOrchestratorRuntimeSupport.swift) carries the lossless request into config, hashing, and create planning. Runtime preflight negotiates `io.github.stephenlclarke.container.logging-drivers.v1` and preserves unknown installed identifiers for forward compatibility. | No remaining request-loss or capability-advertisement gap is known on the matched stack. |
| Compose runtime SPI | [`ComposeRuntimeCreateModels.swift`](../Sources/ComposeRuntimeSPI/ComposeRuntimeCreateModels.swift) carries the exact optional driver and complete option map. [`ComposeRuntimeLogging.swift`](../Sources/ComposeRuntimeSPI/ComposeRuntimeLogging.swift) separates driver-neutral historical reads from live output attachment. | Production adapters implement both contracts; final remote/plugin and Engine-client integration evidence remains. |
| Production create | [`ContainerLaunchAdapter.swift`](../Sources/ComposeContainerRuntime/ContainerLaunchAdapter.swift) launches through the typed `ContainerCreate`/`ContainerRun` API and suppresses embedded CLI progress and identifier output. | Direct Container CLI output remains unchanged. Exact typed production coverage is implemented; final provider-wide live coverage remains. |
...
| Foreground attachment | [`ContainerAttachAdapter.swift`](../Sources/ComposeContainerRuntime/ContainerAttachAdapter.swift) attaches to the exact process handle before start where required. Foreground `up`, non-interactive `run`, `pre_start`, dependency restart, and `attach --no-stdin` no longer depend on persisted history. | Provider-wide, restart, scaled, signal, and TTY paired Docker oracles remain release evidence. |
