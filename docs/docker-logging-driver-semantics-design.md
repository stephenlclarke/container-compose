...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; the journald codec/provider contract and generation-fenced shared-sandbox transport are implemented, but the production Linux service is not; plugin hosting, Engine integration, handoff, and release evidence remain |
...
| Local Container development head | `b0aa9b76964f1752e025d6e988f8af1faf68c351` (`887848ed719a05836d2f846b69a22749e61f2f62` journald provider; `20071d97d10b386c2a24c84c51bca0e37c0280aa` shared-service transport) |
...
| Remote providers | Syslog, Fluentd, GELF, Splunk HEC, AWS Logs, and Google Cloud Logs have option contracts, authority activation, recovery, and focused semantic tests. AWS uses an isolated official SDK adapter. Google Cloud uses the official Go client inside the signed semantic helper, with Application Default Credentials and Moby-compatible project, instance metadata, payload, asynchronous delivery, overflow, flush, and close behavior. Journald now has its Moby-compatible option/field codec, direct-reader and generation-fenced provider lifecycle, conditional catalog registration, narrow signed Linux-service boundary, and an exact-generation shared-sandbox XPC/vsock connection. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | Journald remains correctly unadvertised because its production signed Linux workload, journald-specific bounded wire, system-journal persistence/query adapter, reader routing, supervision, and recovery are absent. The isolated production Docker-plugin service plane is also absent. |
...
- Work package 7 has a Docker logging-plugin codec/adapter and provider lifecycle primitives. Signed Container commit `887848ed719a05836d2f846b69a22749e61f2f62` adds the journald option/field codec, Docker tag and RE2 delegation, native-reader contract, idempotent writer/reader sessions, generation/effect fencing, conditional catalog registration, and exact service boundary. Signed commit `20071d97d10b386c2a24c84c51bca0e37c0280aa` adds the reusable XPC/vsock file-descriptor route, double-fenced by the durable API authority and the helper's exact live sandbox generation and unable to overlap shutdown. The driver is intentionally not advertised without a concrete service. The production signed Linux journald workload, journald-specific bounded wire protocol, system-journal persistence/query adapter, reader routing, supervision/recovery, real-systemd conformance, and the isolated production plugin service plane remain unimplemented.
