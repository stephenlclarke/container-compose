...
| Item | Value |
| --- | --- |
| Status | Active overall; the Docker Compose 5.3.1 static `none` history/foreground reliability contract plus the Docker CLI `local` public-socket lifecycle, compressed rotation, deferred invalid-start inspect contract, and narrow GELF UDP/TCP public-socket lifecycles are Verified, while synchronized dependency publication, the remaining external-client/provider/failure/migration/security matrix, cold-runtime/resource performance collectors, release-artefact results, and release evidence remain Queued |
...
The versioned Engine and Compose oracle is maintained by [`capture-docker-logging-driver-oracle.py`](../Tools/parity/capture-docker-logging-driver-oracle.py) and documented in [Docker logging-driver oracle](parity/docker-logging-driver-oracle.md). Its deterministic Engine 29.2.1 and Compose 5.3.1 fixture freezes default/explicit identity, validation phases and residue, the `none` option quirk, inspect paths, native and cached reads, disabled-cache failure, framing, raw `json-file` records, restart retention, sustained built-in rotation/compression, non-blocking pressure/drop invariants, and foreground output independent of historical readability. VM-local receivers freeze Syslog, Fluentd, direct-Engine GELF UDP-default-gzip/TCP-NUL payload contracts, direct-Engine GELF address/compression/reconnect create-validation, and selected tag/environment/label metadata behavior. Wire, configuration, and metadata component tests now pass against the exact local graph via an identity-preserving editable SwiftPM overlay. Exact candidate Docker CLI/public-socket lanes now pass for `local` and the narrow cache-disabled GELF UDP/TCP lifecycles; they retain strict cross-stream order, metadata/tag, inspect, unreadable-reader, native-authority, peer-close (TCP), and cleanup proof in [LOGGING-GELF-REST-01](parity/slice-ledger.md#logging-gelf-rest-01) and [LOGGING-GELF-TCP-REST-01](parity/slice-ledger.md#logging-gelf-tcp-rest-01). The remaining provider/external-client matrix is still unproven, and [Container issue 74](https://github.com/stephenlclarke/container/issues/74) tracks the separately observed create-response name-as-ID gap. Blocking slow-sink backpressure, sustained dual-cache rotation, plugins, cloud providers, and the remaining candidate/external-client matrix remain separate required oracle additions.
