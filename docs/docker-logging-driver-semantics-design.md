...
| Item | Value |
| --- | --- |
| Status | Active overall; the Docker Compose 5.3.1 static `none` history/foreground reliability contract plus the Docker CLI `local` public-socket lifecycle, compressed rotation, deferred invalid-start inspect contract, narrow GELF UDP/TCP public-socket lifecycles, cache-disabled Syslog UDP and stopped-container Syslog TCP `rfc5424micro` host-alias delivery, one bounded GELF TCP peer-loss/reconnect path, and an initially unavailable/zero-budget TCP failure lifecycle contract are Verified, while synchronized dependency publication, the remaining external-client/provider/failure/migration/security matrix, cold-runtime/resource performance collectors, release-artefact results, and release evidence remain Queued |
...
| Compose checkpoints | Published reproducible `origin/main`: `1f0f944b3d918a39ad97d1f12bb7b7c5ef6146a0`; active signed local implementation through `c7a50e28438ca0c5bd5a668d3b8e87db25c4a176` adds matched local graph selection, Docker-compatible unreadable static history, complete scaled identifiers, and transactional focused validation; the local manifest still names unpublished Container checkpoint `2a79b4553a342e33411666a88ad20ccd2ce46551`, so synchronized publication remains deferred |
...
| Verified Syslog UDP/TCP public-socket certificates | Signed local Container `5ebce8a2aa90bf96775285e1a79ac48b375f69c5` fixes native macOS routing of `host.docker.internal` for the fork-only Syslog provider while retaining TCP/TLS identity. Its signed successor `b82e34874b944d2b9ecc65d4068aee5d7b46905e` reconstructs a stopped cache-disabled native Syslog logger before returning the Docker unreadable-history error. The focused provider suite passes 8/8 with 10/10 executable new-method lines covered. Docker CLI 29.7.1 / Engine 29.2.1 and two exact candidates prove UDP delivery and the TCP second empty connection after `docker logs`, including ordering, facility/severity, tag, inspect, unreadable history, native authority, and cleanup. UDP candidates are 3.08×/2.31× Docker; TCP candidates are 2.07×/1.72×. All satisfy the 10× functional guard only. [LOGGING-SYSLOG-UDP-REST-01](parity/handoffs/LOGGING-SYSLOG-UDP-REST-01.md) and [LOGGING-SYSLOG-TCP-REST-01](parity/handoffs/LOGGING-SYSLOG-TCP-REST-01.md) retain the exact scope and fingerprints. |
| Last implementation review | 6 August 2026 |
...
| Production create | Local `main` launches through typed `ContainerCreate`/`ContainerRun`, suppresses embedded CLI output, and carries the exact logging request. The exact matched candidate passes the pinned TTY oracle and the Compose 5.3.1 non-TTY `none`/readable, restart, three-replica, tail, signal, and disconnect fixture. | Complete the remaining external-client/provider/failure/migration/security/performance evidence, then publish the remotely consumable matched stack. |
...
- Work package 1 has deterministic Engine 29.2.1 and Compose 5.3.1 fixtures for the locally stable core cases plus Syslog, Fluentd, and direct-Engine GELF UDP-default-gzip/TCP-NUL wire, address/compression/reconnect validation, and selected tag/environment/label metadata semantics. Wire/configuration/metadata provider-component regressions pass on the exact local graph through an identity-preserving editable SwiftPM overlay. Fresh candidate public-socket Docker CLI certificates additionally verify the `local` built-in lifecycle, narrow cache-disabled GELF UDP/TCP delivery, cache-disabled Syslog UDP delivery, stopped-container Syslog TCP logger reconstruction, and a bounded forced GELF TCP peer-loss/reconnect path with strict output order, metadata/tag, inspect, unsupported history, native authority, and cleanup. A retained Docker Fluentd TCP/ACK reference now proves selected `host.docker.internal` metadata, EventTime records, and chunk acknowledgements; signed Container `d67b614ebd7e0c1fade908c4a5ab6e48b751e393` maps that VM-only alias only at the native macOS TCP dial boundary and its 12-test suite reports 92.28% focused line coverage. The Fluentd public candidate and positive-budget GELF delayed-retry relay are `Queued`, not `Verified`: [RUNTIME-ISOLATED-PUBLIC-SOCKET-01](parity/handoffs/RUNTIME-ISOLATED-PUBLIC-SOCKET-01.md) removes the shared-service safety blocker only for fresh namespace-aware candidate roots, and neither logging contract has run through that path. These proofs do not certify the complete TCP retry/failing-sink matrix or the remaining provider runtime/external-client matrix. The representative paired performance harness now covers attached/detached startup to first log, fixed stdout/stderr/mixed writers, exact 16 KiB/1 MiB records, blocking slow-sink backpressure, three non-blocking queue sizes with drop assertions, compressed `json-file`/`local` rotation, tail/since/until/follow reads, dual-cache delivery/read, and 1/10/50-service aggregation with counterbalanced schedule evidence, semantic digests, median/P95 ratios, and separate comparable-or-better and regression results. Cold-runtime CPU/RSS/I/O collectors, production plugins, cloud-provider reference oracles, and a release-artefact performance run remain open.
