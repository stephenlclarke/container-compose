...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; local Compose typed execution, the complete production journald path, enhanced Engine logs/inspect/info/hijack/WebSocket/resize routes, the local public gateway lifecycle, installed Docker-plugin hosting/direct reads/restart/recreated-ID certification, durable provider-generation cutover, both sides of the immutable devcontainer logging transfer, chunked histories above 64 MiB, and the coordinator-driven signed root, public-history/new-writer, and staged-abort/restart transactions are implemented and focused-tested; public Docker create/control, provider-root trust namespacing, the defensive 4096-store/32 GiB ceiling decision, dependency publication, and paired release evidence remain |
...
| Public-build Container revision | `6c3f7d3701cf9400855849fa0e29dd75d7b9c45d` |
| Compose checkpoints | Published reproducible `origin/main`: `1f0f944b3d918a39ad97d1f12bb7b7c5ef6146a0`; active signed local implementation `784aacaa26f2ad698460044a573adae30665628a`; evidence reconciliation `3cd0da4a72735f5d8e4823a1b631ddeb557b55ff`; public-gateway closure record `9479e0aaba2bca3402d30fb7b77e21478e0455bd`; provider-generation record `b6bcc3d7905e8e229cfbad45d2fc5dea0e6c589c`; the local manifest still names unreachable Container checkpoint `2a79b4553a342e33411666a88ad20ccd2ce46551` and therefore must not be pushed yet |
| Local Container checkpoints | Signed enhanced authority `6a668b2b5d42246efcad3316374f6d0e0d2eaf14`; signed public gateway lifecycle and discovery fix `ac1803ec555960ce49fcec1d6a5b718d781629e0`; production journald supervision/reclamation `84d160671f3ba6c265a02b49b2ff4309f6584d30`; isolated Docker-plugin lifecycle implementation `08677dc8b5a677533de80cf634fee1d14f4da069`; complete provider-generation cutover implementation `6e462443dd744bda0b605bf26e093833d7818e77`; coordinator-driven logging cutover evidence `0808fa92fcfa3e2a2b3a4c5b05e0057e83edf6ba`; public-history and abort-recovery evidence `4e426116968e8382317e9cb3ac17821c9e6733af`; chunked-history staging and one-file publication `08e7324b1dc89d0d1acd91680562d9223d807d56`; installed certification and recreated-ID identity fix `36ef9c8fbed136641550eed695039440e578de70` |
| Matched Containerization checkpoints | Published provenance `77f06d4c44341e04241941072fb69e2b85a6f5c1`; signed local shared-sandbox head `864455bf1a104f0215b7c912a45800b0a0538973` |
| Shared Engine API | Released `container-engine-api` 0.3.5 at signed commit `78cb4cb5781d6dbe9f0d34a1b925ee8dcaacdc98`; active signed local handoff head `12cafed46d2af379b456e5cbd70723edd97836f3` adds provider control, source export, signed manifest assembly, canonical ordered 8 MiB portable logging stores, sealed object publication, and crash-replayable contribution state |
| Logging handoff checkpoints | Signed local Container heads `170b8769e3105169b4a01d99ac3bc93cd860d6fc`, `8005bf8fb8036da0b15d63763d5f51dff12cd5eb`, `0808fa92fcfa3e2a2b3a4c5b05e0057e83edf6ba`, `4e426116968e8382317e9cb3ac17821c9e6733af`, and `08e7324b1dc89d0d1acd91680562d9223d807d56` implement and verify destination staging/re-resolution, protected receipt, abort compensation, signed promotion, Complete-only activation, immutable history publication, cursor adoption, the provider-neutral devcontainer payload, the real coordinator's possession/export/manifest/object-transfer/root-commit/promotion/activation transaction, public imported-history read-back plus a greater-sequence writer, staged-abort compensation replay across coordinator reconstruction, strict complete chunk-set validation, and ordered one-active-file publication; signed local `devcontainer` head `385cf69878a87e67589eb36fff2bc3c9dd66695f` adds quiesced Apple Container history export, durable provider identity/key enrollment, object transport, source contribution replay, and authenticated private-session wiring; focused Engine API/devcontainer/Container evidence passes 4/4, 2/2, 6/6, 5/5, and 7/7 on the MBP |
| Last implementation review | 5 August 2026 |

## Goal

Close the Docker logging-driver row in [STATUS.md](../STATUS.md) without equating schema acceptance with runtime support, collapsing distinct drivers onto one file writer, or making foreground output depend on historical log readability. Completion means that the Container family:
...
| --- | --- | --- |
| Compose normalisation | Signed local `main` at `784aacaa26f2ad698460044a573adae30665628a` preserves the lossless optional driver and complete compose-go-normalised option map. Published `origin/main` remains the remotely reproducible baseline at `1f0f944b3d918a39ad97d1f12bb7b7c5ef6146a0`. | Publish the local implementation only with a remotely fetchable matched Container dependency. |
| Compose policy | Local `main` carries every requested driver and option through config, hashing, create planning, and optional `io.github.stephenlclarke.container.logging-drivers.v1` negotiation. The required release manifest deliberately does not require that optional capability, so older/public runtimes retain the supported local subset. | Certify the immutable matched runtime and then publish the same source without weakening capability negotiation. |
| Compose runtime SPI | Local `main` carries exact requested driver/options plus separate driver-neutral historical-read and live-output attachment contracts. The published baseline lacks that advanced path only because its matching dependency is not remotely available. | Publish the matched SPI/authority pair together and retain compatibility tests for the public baseline. |
| Production create | Local `main` launches through typed `ContainerCreate`/`ContainerRun`, suppresses embedded CLI output, and carries the exact logging request. A strict warnings-as-errors build and all 1,360 Swift tests pass against the matched local stack. | Complete live provider/restart/TTY/scaled oracles, then publish the remotely consumable matched stack. |
| Container authority/model | The matched Container authority persists lossless requested and resolved v2 state, system defaults, protected options, provider identity/generation, read policy, and delivery mode. Direct typed and CLI requests share the same resolver. The enhanced private provider projects complete `SystemInfo` and `ContainerInspect` documents from that same authority and applies logging fields through a fail-closed shared-response contract. Its advertised catalogue is side-effect free; create/start still requires concrete provider readiness. | Routine native inspection remains redacted. The common public gateway now selects this authority locally; the remaining external-client route matrix must prove that no second authority appears. |
...
| Foreground attachment | Local Compose `784aacaa26f2ad698460044a573adae30665628a` attaches to the exact process handle before start where required, so foreground `up`, non-interactive `run`, `pre_start`, dependency restart, and `attach --no-stdin` do not depend on persisted history. Published `origin/main` retains the reproducible baseline until the matching Container dependency can be fetched by hosted CI. | Provider-wide, restart, scaled, signal, TTY, WebSocket, resize, and detach/re-attach paired Docker oracles remain release evidence. |
| Remote providers | Syslog, Fluentd, GELF, Splunk HEC, AWS Logs, and Google Cloud Logs have option contracts, authority activation, recovery, and focused semantic tests. AWS uses an isolated official SDK adapter. Google Cloud uses the official Go client inside the signed semantic helper, with Application Default Credentials and Moby-compatible project, instance metadata, payload, asynchronous delivery, overflow, flush, and close behavior. Journald now has its Moby-compatible option/field codec, direct-reader lifecycle, versioned bounded Swift/Go protocol, restart-safe backend, concrete systemd adapter, verified pinned OCI workload, protected persistent mounts, shared-sandbox supervision, exact sandbox/workload-generation routing, readiness-gated catalog publication, safe generation rollover, and authority-ordered terminal reclamation. Signed Container `08677dc8b5a677533de80cf634fee1d14f4da069` adds an isolated Docker-plugin service with a closed digest-pinned installation contract, HMAC-authenticated pooled Swift/Go transport, durable replay-safe writer/reader claims, real Linux FIFOs, readable and write-only Unix-plugin conformance, direct `ReadLogs`, and readiness-gated dynamic discovery. Signed Container `6e462443dd744bda0b605bf26e093833d7818e77` completes immutable multi-generation staging, quiescence, durable configuration/history migration, terminal proof, atomic cutover, exact workload stop with lost-response recovery, and final reclamation. | The local journald and plugin runtime paths are implemented. Remaining gaps are release-signature trust, synchronized dependency publication, whole-stack distributable-plugin certification, paired Docker certification, and final programme-level lifecycle evidence. |
| Shared Engine API | Released `container-engine-api` 0.3.5 at `78cb4cb5781d6dbe9f0d34a1b925ee8dcaacdc98` implements the hardened listener, generated API 1.44 through 1.53 route ledger, shared executable/gateway, gateway-owned `GET`/`HEAD /_ping`, exclusive provider identity, deterministic private-session shutdown, starvation-free framed I/O, WebSocket framing, resize dispatch, and fail-closed complete-response composition. Signed Container `ac1803ec555960ce49fcec1d6a5b718d781629e0` packages and signs that executable; `system start/status/stop` installs, probes, reports, and deregisters it around the same enhanced authority. Canonical stopped replay, active follow, exact-process fallback, bounded hijack and WebSocket attach, detach keys/events, terminal resize, complete `SystemInfo`, and complete `ContainerInspect` all use that authority. Signed Container `08677dc8b5a677533de80cf634fee1d14f4da069` routes direct isolated-provider readers through the same logging authority rather than through live attachment. | The enhanced provider advertises `ContainerLogs`, hijack and WebSocket `ContainerAttach`, `ContainerResize`, `ContainerInspect`, and `SystemInfo`. The API-socket grant, exact provenance for several broader lifecycle fields, and the complete external-client route/provider matrix remain unimplemented or uncertified. |
...
third-party readable plugin, two native lifecycle cycles, exact FIFO history,
independent Docker `info`, `inspect`, and `logs`, stopped-state projection as
`exited`, history after API-service/shared-sandbox restart, and same-ID
delete/recreate after another daemon restart. Signed Container `36ef9c8f`
binds writer and reader identity to the persisted container incarnation so a
new container cannot collide with an old protected-effect tombstone.

This does not certify public Docker lifecycle control. The selected provider
still reports public `ContainerCreate`/`ContainerStart` as unimplemented
([container#61](https://github.com/stephenlclarke/container/issues/61)). A
separate rebuilt-root gate found that production handoff trust uses one global
Keychain namespace; provider-root isolation is tracked in
[container-engine-api#14](https://github.com/stephenlclarke/container-engine-api/issues/14).
The older restart/stale-state inspect discrepancy remains tracked in
[container#62](https://github.com/stephenlclarke/container/issues/62), although
the clean installed certification projects `exited`.
...
The versioned Engine and Compose oracle is maintained by [`capture-docker-logging-driver-oracle.py`](../Tools/parity/capture-docker-logging-driver-oracle.py) and documented in [Docker logging-driver oracle](parity/docker-logging-driver-oracle.md). Its deterministic Engine 29.2.1 and Compose 5.3.1 fixture freezes default/explicit identity, validation phases and residue, the `none` option quirk, inspect paths, native and cached reads, disabled-cache failure, framing, raw `json-file` records, restart retention, sustained built-in rotation/compression, non-blocking pressure/drop invariants, and foreground output independent of historical readability. Wire-level remote receiver payloads, blocking slow-sink backpressure, sustained dual-cache rotation, plugins, and cloud providers remain separate required oracle additions.

## Implementation Progress

As of 5 August 2026:

- Work package 1 has deterministic Engine 29.2.1 and Compose 5.3.1 fixtures for the locally stable core cases plus Syslog, Fluentd, and GELF wire contracts. Blocking slow-sink behaviour, sustained dual-cache rotation, production plugins, cloud providers, and the final performance matrix remain open.
...
Docker Compose catches an unsupported logs reader during foreground `up` and falls back to live attach. Thus `driver: none` can display output in `docker compose up` even though `docker compose logs` cannot retrieve it later.
...
- `compose logs` remains a driver/cache historical operation;
...
| GELF | `gelf-address`, `gelf-compression-type`, `gelf-compression-level`, `gelf-tcp-max-reconnect`, `gelf-tcp-reconnect-delay` |
...
- `compose logs` exact unsupported-reader result and later historical availability.
...
| Read/attach | Historical read is driver/cache based; every effectful live/stopped reader has one generation-fenced, retry-safe session and terminal close; live attach is independent; `none` foreground output works without becoming historically readable. |
