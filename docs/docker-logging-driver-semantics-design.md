...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; the journald contract, generation-fenced transport, bounded client/server wire, durable backend, Linux/arm64 workload, concrete systemd adapter, pinned OCI build, and real-systemd cross-language lane are implemented locally; production supervision/routing/signing, plugin hosting, Engine integration, handoff, and paired release evidence remain |
...
| Preserved logging implementation checkpoints | Compose `d6361aab6ef636246709f452b5483b54493a9764`; Container `2a79b4553a342e33411666a88ad20ccd2ce46551` (signed ancestors, not the active public source/dependency pair) |
| Local Container development head | `5b688fce035d3c78c00afc044dc1b785b6a09e36` (`887848ed719a05836d2f846b69a22749e61f2f62` journald provider; `20071d97d10b386c2a24c84c51bca0e37c0280aa` shared-service transport; `a42ecf2fe1ffa582e34cfa74f6cf1ddba8505368` journald client wire; `bed5de1686bc005ad77ab63025a5582f37601738` journald wire server; `79c89babc7399c0cc1d4f800bd5ec092cc6c153d` durable backend; `dcefedba2b3b5806953c32e35ca2edaea24658a0` reader checkpoint recovery; `e8cc75f001d24896144a5e44e33d1f7a5d1e5729` Linux workload/systemd adapter) |
| Matched Containerization revision | `77f06d4c44341e04241941072fb69e2b85a6f5c1` |
| Last implementation review | 3 August 2026 |
...
| --- | --- | --- |
| Compose normalisation | The public build retains the published baseline representation in [`Tools/compose-normalizer/main.go`](../Tools/compose-normalizer/main.go): structured and legacy logging fields reach Compose, but arbitrary driver requests are not preserved as the terminal runtime contract. Signed checkpoint `d6361aab6ef636246709f452b5483b54493a9764` contains the lossless optional driver and complete compose-go-normalised option map. | Restore the preserved implementation only when its Container dependency is published or otherwise remotely reproducible. |
| Compose policy | The public build supports the published local logging subset (`json-file`, `local`, `none`, `max-size`, and `max-file`) and deliberately does not require `io.github.stephenlclarke.container.logging-drivers.v1`. The signed checkpoint carries the lossless request through config, hashing, create planning, and optional capability negotiation. | Public `main` remains buildable but does not claim the advanced logging contract. |
| Compose runtime SPI | The active public SPI carries the published `ComposeLogConfiguration` storage policy. The signed checkpoint adds exact requested driver/options plus separate driver-neutral historical-read and live-output attachment contracts. | The advanced SPI is preserved in history, not active in the public source tree. |
| Production create | The public build uses the published Container launch contract and supported local logging projection. The signed checkpoint launches through typed `ContainerCreate`/`ContainerRun`, suppresses embedded CLI output, and carries the exact logging request. | Reactivate the typed path after the matched Container implementation becomes remotely consumable. |
| Container authority/model | The matched Container authority persists lossless requested and resolved v2 state, system defaults, protected options, provider identity/generation, read policy, and delivery mode. Direct typed and CLI requests share the same resolver. The enhanced private provider now projects complete `SystemInfo` and `ContainerInspect` documents from that same authority and applies logging fields through a fail-closed shared-response contract. | Routine native inspection remains redacted. The common public gateway and external-client certification still need to expose the selected enhanced provider without adding another authority. |
...
| Foreground attachment | The signed Compose checkpoint attaches to the exact process handle before start where required, so foreground `up`, non-interactive `run`, `pre_start`, dependency restart, and `attach --no-stdin` do not depend on persisted history. The active public source retains the published baseline until the matching Container dependency can be fetched by hosted CI. | Provider-wide, restart, scaled, signal, and TTY paired Docker oracles plus safe reactivation of the preserved adapter remain release evidence. |
| Remote providers | Syslog, Fluentd, GELF, Splunk HEC, AWS Logs, and Google Cloud Logs have option contracts, authority activation, recovery, and focused semantic tests. AWS uses an isolated official SDK adapter. Google Cloud uses the official Go client inside the signed semantic helper, with Application Default Credentials and Moby-compatible project, instance metadata, payload, asynchronous delivery, overflow, flush, and close behavior. Journald now has its Moby-compatible option/field codec, direct-reader and generation-fenced provider lifecycle, conditional catalog registration, exact-generation shared-sandbox XPC/vsock connection, versioned one-MiB framed client/server protocol, restart-safe bounded writer/reader state, Linux/arm64 AF_VSOCK workload, and concrete go-systemd append/query adapter. The pinned OCI build records source/test/archive/platform digests and provenance; portable race tests, real-systemd tests, deterministic manifest checks, and the packaged Swift cross-language round trip pass locally. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | Journald remains correctly unadvertised because production authority supervision/routing, readiness withdrawal, release signing/installation, terminal-state reclamation, recovery, and paired Docker certification are absent. The isolated production Docker-plugin service plane is also absent. |
| Shared Engine API | `container-engine-api` 0.2.3 implements the hardened listener, generated API 1.44 through 1.53 route ledger, shared executable/gateway, exclusive provider identity, deterministic private-session shutdown, starvation-free framed I/O, and fail-closed complete-response composition. Devcontainer `77f3cb5dc7cce8d3f88de960b7d93d676a768137` supplies the stock adapter. Signed Container head `ad4f229c99db02be5f40a58b31b12fdf84e5a924` supplies the enhanced authority adapter and restart-stable private provider session. `ContainerLogs` uses the canonical reader for stopped history and a lossless record stream for active follow. `ContainerAttach` combines that replay/follow source with exact-process stdin/stdout/stderr fallback, bounded no-drop hijack transport, Docker detach keys, and canonical attach/detach events. Complete `SystemInfo` and `ContainerInspect` bases come from the same authority; the controller verifies Moby's non-optional top-level contract before overlaying only logging-owned fields. | `ContainerLogs`, `ContainerAttach`, `ContainerInspect`, and `SystemInfo` are advertised by the enhanced provider. WebSocket attach, resize, the common public-gateway route owner, exact provenance for several broader lifecycle fields, and the complete external-client route/provider matrix remain unimplemented or uncertified. |
...
- Work package 7 has a Docker logging-plugin codec/adapter and provider lifecycle primitives. Signed Container commit `887848ed719a05836d2f846b69a22749e61f2f62` adds the journald option/field codec, Docker tag and RE2 delegation, native-reader contract, idempotent writer/reader sessions, generation/effect fencing, conditional catalog registration, and exact service boundary. Signed commit `20071d97d10b386c2a24c84c51bca0e37c0280aa` adds the reusable XPC/vsock file-descriptor route, double-fenced by the durable API authority and the helper's exact live sandbox generation and unable to overlap shutdown. Signed commit `a42ecf2fe1ffa582e34cfa74f6cf1ddba8505368` adds the journald-specific one-MiB framed client protocol with explicit versioned projections, binary-safe validation, exact operation-ID replay, ordered reader-next calls, `SIGPIPE` suppression, and cancellation-safe shutdown. Signed commit `bed5de1686bc005ad77ab63025a5582f37601738` adds the matching server engine: concurrent identical requests join one effect, completed outcomes replay under count and complete retained-byte bounds, conflicts fail before effects, and persistent framed connections interoperate with the client. Signed commits `79c89babc7399c0cc1d4f800bd5ec092cc6c153d` and `dcefedba2b3b5806953c32e35ca2edaea24658a0` add the bounded restart-safe backend and exact opaque reader-checkpoint recovery; the combined Swift wire/server/backend gate passes 20 tests. Signed commit `e8cc75f001d24896144a5e44e33d1f7a5d1e5729` adds the Linux/arm64 AF_VSOCK workload, dedicated systemd-journald process, concrete digest-reconciling/query-visible go-systemd adapter, Docker-compatible static/follow query projection, fixed resource bounds, pinned OCI/provenance build, 17-test portable race lane, 20-test real-systemd lane, deterministic platform-manifest check, and packaged Swift cross-language round trip. The driver remains intentionally unadvertised until the authority supervises, verifies, routes, and readiness-gates the workload. Release signing, terminal-state reclamation, paired Docker behavior/performance evidence, and the isolated production plugin service plane remain unimplemented.
- Work package 8 has the logging-specific Docker REST/streaming routes plus the released shared wire/listener/provider-identity foundation in `container-engine-api` 0.2.3; devcontainer consumes that foundation without a duplicate listener. Signed Container implementation `9e23d41fc18dde5ae926e0cbdd1f35d8c86fc512`, with handoff head `ad4f229c99db02be5f40a58b31b12fdf84e5a924`, integrates canonical stopped replay and lossless active follow with the enhanced authority, maps exact-process stdin/stdout/stderr fallback into bounded no-drop Docker hijack sessions, consumes detach keys, publishes attach/detach events through the canonical broadcaster, and advertises `ContainerLogs` plus `ContainerAttach`. The same provider now composes and advertises complete `SystemInfo` and `ContainerInspect` responses through a fail-closed shared contract. WebSocket attach, resize, the common public route owner, exact broader lifecycle-field provenance, and the external-client suites remain unimplemented or uncertified.
- Work package 9 now uses typed Container launches and exact-process live output attachment. An isolated `b5981bd611d9bdbdbec88bad036cdd49bd4060a3` runtime smoke proved three foreground records for both `none` and `json-file`, zero persisted `none` bytes, three readable `json-file` records, no blank lines, and no embedded identifier leak. Provider-wide, restart, scaled, signal, and TTY oracles remain.
...
- Work package 11 is in progress: Container's supported `make test` and `make check` gates pass, all 1,359 Compose tests pass, and isolated exact-head smoke evidence is green. Security review, migration rehearsal, complete same-host performance evidence, publication, and rollback evidence remain.
