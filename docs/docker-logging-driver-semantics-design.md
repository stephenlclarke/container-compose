...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; the journald codec/provider contract, generation-fenced shared-sandbox transport, bounded reconnect-safe client/server wire, and restart-safe writer/reader state backend are implemented, but the production Linux workload and concrete systemd adapter are not; plugin hosting, Engine integration, handoff, and release evidence remain |
...
| Local Container development head | `eeeeff83851bcb9fec0a5e438f830174f5794c89` (`887848ed719a05836d2f846b69a22749e61f2f62` journald provider; `20071d97d10b386c2a24c84c51bca0e37c0280aa` shared-service transport; `a42ecf2fe1ffa582e34cfa74f6cf1ddba8505368` journald client wire; `bed5de1686bc005ad77ab63025a5582f37601738` journald wire server; `79c89babc7399c0cc1d4f800bd5ec092cc6c153d` durable journald backend) |
...
| Remote providers | Syslog, Fluentd, GELF, Splunk HEC, AWS Logs, and Google Cloud Logs have option contracts, authority activation, recovery, and focused semantic tests. AWS uses an isolated official SDK adapter. Google Cloud uses the official Go client inside the signed semantic helper, with Application Default Credentials and Moby-compatible project, instance metadata, payload, asynchronous delivery, overflow, flush, and close behavior. Journald now has its Moby-compatible option/field codec, direct-reader and generation-fenced provider lifecycle, conditional catalog registration, narrow signed Linux-service boundary, exact-generation shared-sandbox XPC/vsock connection, versioned one-MiB framed client protocol, matching server engine with in-flight joining/bounded replay/conflict rejection, and a restart-safe bounded writer/reader state backend with append reconciliation, close fencing, private atomic persistence, and durable reader resume. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | Journald remains correctly unadvertised because its production signed Linux workload/listener, concrete systemd append/query adapter, production reader routing, supervision, and recovery are absent. The isolated production Docker-plugin service plane is also absent. |
