...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; local Compose typed execution, the complete production journald path, enhanced Engine logs/inspect/info/hijack/WebSocket/resize plus public create/start/stop/delete routes, installed Docker-plugin hosting/direct reads/restart/recreated-ID certification, durable lifecycle and provider-generation recovery, protected-effect reconciliation, provider-root trust namespacing, both sides of the immutable devcontainer logging transfer, fixed-ceiling-free record/file-backed history transfer, the coordinator-driven signed root/history transactions, the paired Docker 29.2.1/candidate live TTY resize and detach/re-attach oracle, and the 29-fixture counterbalanced logging performance harness are implemented and focused-tested; synchronized dependency publication, the complete remaining external-client/provider/failure/migration/security matrix, cold-runtime/resource performance collectors, release-artefact results, and release evidence remain |
...
| Production create | Local `main` launches through typed `ContainerCreate`/`ContainerRun`, suppresses embedded CLI output, and carries the exact logging request. A strict warnings-as-errors build and all 1,360 Swift tests pass against the matched local stack. The signed candidate now matches Docker's attach-before-start lifecycle in the paired live TTY oracle. | Complete the remaining provider/restart/scaled/non-TTY oracles, then publish the remotely consumable matched stack. |
...
| Foreground attachment | Local Compose `784aacaa26f2ad698460044a573adae30665628a` attaches to the exact process handle before start where required, so foreground `up`, non-interactive `run`, `pre_start`, dependency restart, and `attach --no-stdin` do not depend on persisted history. Signed Container `a10ad44d8675b27a665d72fbe054f12f403f8412` and Engine API `c7973ac641fb6f6e07df1358114f36222bd9ca59` make the identical Docker 29.2.1/candidate live case pass pre-start TTY attach, resize, detach, re-attach, exit, EOF, and cleanup. Candidate timing is 1.238173 seconds cold and 0.920554 seconds warm against the 0.184992-second Docker fixture: inside the <10× guard but not yet comparable. Published `origin/main` retains the reproducible baseline until the matching Container dependency can be fetched by hosted CI. | Provider-wide, restart, scaled, signal, non-TTY, comparable-performance, and release certification remain. |
...
- Work package 11 is in progress: Container's packaging gates, supported `make check`, focused Engine integration, isolated signed public-gateway lifecycle, the 63-test cutover/runtime gate, the scoped 1,868 Swift Testing plus 94 XCTest package suite, macOS Go race/vet at 70.7% coverage, and pinned Linux/arm64 Go race at 74.3% pass; Compose's strict build, all 1,360 tests, Go normalizer suite, and policy gates pass. The paired performance harness now retains 29 counterbalanced lifecycle/logging fixtures, fixed-work direction, raw schedule position, semantic assertion digests, remote-sink summaries, median/P95 ratios, a configurable noise band, and separate comparable-or-better and 10× guard results. Its focused inventory/finalizer/sink tests, shell lint, generated Docker/container-compose model checks, and one-repetition Docker behaviour smokes pass on the MBP; the expensive release-artefact run remains batched for the coherent checkpoint. The canonical public Containerization pin `77f06d4c44341e04241941072fb69e2b85a6f5c1` cannot compile the existing branch because it lacks the sandbox/workload/network APIs used by the matched source; signed implementation checkpoint `864455bf1a104f0215b7c912a45800b0a0538973` compiles and passes, while local `upstream/engine-linux-sandbox` remains intentionally unpublished at signed handoff head `38d9c695e7a6915e5ce45d12c893dc323a661af7`. In the current devcontainer runtime-parity run, the Container Compose lane passes E03 in 2.521 seconds, fails C04 functionally, meets the Docker timing target only on C03, and completes the other fixtures at 1.126–7.609× Docker. The separate Apple-stock lane owns the 302-second E03 timeout and 12× E04 result. The latest isolated MBP certificate passes the installed plugin through public create/start/logs/stop/delete plus native cycles, and its focused service suite passes 10/10. Security review, migration rehearsal, cold/resource collection, the complete same-host release run, immutable publication, and rollback evidence remain.
