...
This closes aggregate ciphertext buffering, but not the entire large-history
memory contract. The Apple Container history client still returns aggregate
records, the compatibility package decoder returns complete entry values, and
the logging destination stages aggregate history values. Closure therefore
requires record-file acquisition and a callback/file-backed canonical decoder
that hands one verified history segment at a time to destination staging.
