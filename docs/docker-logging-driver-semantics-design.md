...
| Item | Value |
| --- | --- |
| Status | Active overall; the Docker Compose 5.3.1 static `none` history/foreground reliability contract plus the Docker CLI `local` public-socket lifecycle, compressed rotation, deferred invalid-start inspect contract, narrow GELF UDP/TCP public-socket lifecycles, cache-disabled Syslog UDP and stopped-container Syslog TCP `rfc5424micro` host-alias delivery, one bounded GELF TCP peer-loss/reconnect path, an initially unavailable/zero-budget TCP failure lifecycle contract, and the narrow Splunk required-option start diagnostic are Verified. The Splunk correction passed its frozen focused graph and fresh public-socket candidate; its 90% coverage measurement remains a non-functional evidence gap after a disk-protecting aborted instrumentation run. Synchronized dependency publication, the remaining external-client/provider/failure/migration/security matrix, cold-runtime/resource performance collectors, release-artefact results, and release evidence remain Queued. |
