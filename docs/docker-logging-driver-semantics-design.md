...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; request/default resolution, native drivers, delivery/read control, three remote drivers, typed Compose launch, and exact live foreground attachment are implemented; cloud/service providers, Engine integration, handoff, and release evidence remain |
...
| Matched Container revision | `c3ea6f99a837a390afb5466ac13df6d160eac77c` |
...
| Remote providers | Syslog, Fluentd, GELF, and Splunk HEC have generated option contracts, encoders/transports, authority activation, recovery, and loopback/oracle tests. The Splunk provider includes protected-token handling, Moby-compatible inline/json/raw encoding, batching/retry/drop behavior, gzip, verification, index acknowledgement, proxy, and custom-CA TLS. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | AWS Logs and Google Cloud implementations are absent. Journald and the isolated production Docker-plugin service plane are absent. |
| Shared Engine API | `container-engine-api` 0.2.2 implements the hardened listener, generated API 1.44 through 1.53 route ledger, shared executable/gateway, exclusive provider identity, deterministic private-session shutdown, and starvation-free framed I/O. Devcontainer `77f3cb5dc7cce8d3f88de960b7d93d676a768137` consumes that exact release and supplies the stock adapter. The logging package already implements Docker-shaped `/info`, inspect `LogConfig`, logs, attach/hijack, framing, query parsing, and error envelopes against an abstract authoritative backend. | The logging controller and routes are not yet registered behind the enhanced Container authority or its canonical lifecycle events, and the complete external-client route/provider matrix is not certified. |
...
- Work package 6 has production Syslog, Fluentd, GELF, and Splunk HEC providers. AWS Logs and Google Cloud remain unimplemented. The Splunk slice is pinned at signed Container commit `c3ea6f99a837a390afb5466ac13df6d160eac77c`; 17 focused provider tests, one authority secret-persistence test, real HTTP/custom-CA TLS loopback coverage, `make check`, and the 1,751-test full Container gate pass on the programme MacBook Pro. The matched Compose pin builds and its 78 launch/orchestrator tests pass. Issue `#187` records the stale Compose capability declaration found by stack validation; the slice adds `io.github.stephenlclarke.container.logging-drivers.v1` to both typed and generated manifests.
