...
| Matched Container revision | `b5981bd611d9bdbdbec88bad036cdd49bd4060a3` |
| Matched Containerization revision | `77f06d4c44341e04241941072fb69e2b85a6f5c1` |
| Last implementation review | 2 August 2026 |
...
| Remote providers | Syslog, Fluentd, and GELF have generated option contracts, encoders/transports, authority activation, recovery, and loopback/oracle tests. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | AWS Logs, Splunk, and Google Cloud implementations are absent. Journald and the isolated production Docker-plugin service plane are absent. |
| Shared Engine API | `container-engine-api` 0.2.1 implements the hardened listener, generated API 1.44 through 1.53 route ledger, shared executable/gateway, exclusive provider identity, and private out-of-process provider session. Devcontainer `3a62fb981bd3` consumes those products and supplies the stock adapter. The logging package already implements Docker-shaped `/info`, inspect `LogConfig`, logs, attach/hijack, framing, query parsing, and error envelopes against an abstract authoritative backend. | The logging controller and routes are not yet registered behind the enhanced Container authority or its canonical lifecycle events, and the complete external-client route/provider matrix is not certified. |
...
- Work package 6 has production Syslog, Fluentd, and GELF providers. AWS Logs, Splunk, and Google Cloud remain unimplemented.
