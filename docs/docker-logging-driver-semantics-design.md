...
| Shared Engine API | `container-engine-api` 0.2.2 implements the hardened listener, generated API 1.44 through 1.53 route ledger, shared executable/gateway, exclusive provider identity, deterministic private-session shutdown, and starvation-free framed I/O. Devcontainer `77f3cb5dc7cce8d3f88de960b7d93d676a768137` consumes that exact release and supplies the stock adapter. Signed Container head `db9cbfbdc9059454cb0f60f7fcbdf1714ebd6e03` adds the enhanced authority adapter and restart-stable private provider session: `ContainerLogs` uses the canonical reader for stopped history and a lossless record stream for active follow; `/info` and inspect project authority-owned logging state. | Only `ContainerLogs` is advertised by the enhanced provider today. Whole-route composition must safely expose `/info` and inspect alongside their non-logging fields; attach/hijack and attach/detach events remain unimplemented against the enhanced authority; the complete external-client route/provider matrix is not certified. |
...
- Work package 8 has the logging-specific Docker REST/streaming routes plus the released shared wire/listener/provider-identity foundation in `container-engine-api`; devcontainer consumes that foundation without a duplicate listener. Signed Container head `db9cbfbdc9059454cb0f60f7fcbdf1714ebd6e03` integrates the canonical stopped reader and lossless active-follow stream with the enhanced authority and starts its restart-stable private provider session. Safe whole-route composition for `/info` and inspect, attach/hijack, canonical attach/detach events, and the external-client suites remain unimplemented.
