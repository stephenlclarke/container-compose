...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; local Compose typed execution, the complete production journald path, enhanced Engine logs/inspect/info/hijack/WebSocket/resize routes, the local public gateway lifecycle, isolated Docker-plugin hosting/direct reads, durable provider-generation cutover, and the immutable devcontainer source-export path are implemented and focused-tested; destination handoff import/activation, dependency publication, whole-stack third-party certification, and paired release evidence remain |
...
| Logging handoff checkpoints | Signed local `container-logging` head `dff9d1b2f37dc43e701283cd92f670e953304310` verifies that the provider-neutral payload decodes through the production logging controller; signed local `devcontainer` head `385cf69878a87e67589eb36fff2bc3c9dd66695f` adds quiesced Apple Container history export, durable provider identity/key enrollment, object transport, source contribution replay, and authenticated private-session wiring |
...
- Work package 10 is in progress. Signed local Engine API head `2ed00204c28589be6480ab06635933b8a992f67b` supplies the authenticated provider-control protocol, destination-key proof, crash-replayable source contributions, signed manifest assembly, content-addressed sealed objects, and the bounded provider-neutral Docker `json-file` history package. Signed local `container-logging` head `dff9d1b2f37dc43e701283cd92f670e953304310` proves that package decodes through the production logging controller. Signed local devcontainer head `385cf69878a87e67589eb36fff2bc3c9dd66695f` exports stopped, quiesced Apple Container stdout/stderr history including rotations and wires durable provider key enrollment plus the source responder into both private and embedded provider sessions. Focused MBP evidence passes 3 portable-payload tests, 2 signed-manifest/source-replay tests, 5 production payload-decoder tests, 2 devcontainer record-projection tests, and the exact private provider-session integration test. Remaining work is destination stage/import receipt and collision validation, destination re-resolution and protected-option resealing, commit-authorised promotion, Complete-only history adoption/merged reads, abort compensation and crash recovery, histories beyond the current 64 MiB source bound, synchronized dependency publication, and the complete multi-provider rehearsal/security/performance matrix.
