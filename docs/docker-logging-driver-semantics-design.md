...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; local Compose typed execution, the complete production journald path, enhanced Engine logs/inspect/info/hijack/WebSocket/resize plus public create/start/stop/delete routes, installed Docker-plugin hosting/direct reads/restart/recreated-ID certification, durable lifecycle and provider-generation recovery, protected-effect reconciliation, provider-root trust namespacing, both sides of the immutable devcontainer logging transfer, fixed-ceiling-free record/file-backed history transfer, the coordinator-driven signed root/history transactions, and the counterbalanced representative logging performance harness are implemented and focused-tested; synchronized dependency publication, the complete external-client/provider/failure/migration/security matrix, remaining slow-sink/queue/resource/dual-cache performance lanes, release-artefact results, and release evidence remain |
...
issues are commented and closed. These commits remain unpublished until the
synchronized programme publication checkpoint.
...
- Work package 1 has deterministic Engine 29.2.1 and Compose 5.3.1 fixtures for the locally stable core cases plus Syslog, Fluentd, and GELF wire contracts. The representative paired performance harness now covers startup to first log, fixed stdout/stderr/mixed writers, exact 16 KiB/1 MiB records, tail/follow/rotation, and 1/10/50-service aggregation with counterbalanced schedule evidence, median/P95 ratios, and separate comparable-or-better and regression results. Blocking slow-sink behaviour, non-blocking queue loads, resource counters, sustained dual-cache/local-driver rotation, production plugins, cloud-provider reference oracles, and a release-artefact performance run remain open.
...
- Work package 11 is in progress: Container's packaging gates, supported `make check`, focused Engine integration, isolated signed public-gateway lifecycle, the 63-test cutover/runtime gate, the scoped 1,868 Swift Testing plus 94 XCTest package suite, macOS Go race/vet at 70.7% coverage, and pinned Linux/arm64 Go race at 74.3% pass; Compose's strict build, all 1,360 tests, Go normalizer suite, and policy gates pass. The paired performance harness now retains 19 counterbalanced lifecycle/logging fixtures, fixed-work direction, raw schedule position, median/P95 ratios, a configurable noise band, and separate comparable-or-better and 10× guard results. Its focused inventory, synthetic evidence, shell lint, and generated Compose-model checks pass on the MBP; the expensive release-artefact run remains batched for the coherent checkpoint. The canonical public Containerization pin `77f06d4c44341e04241941072fb69e2b85a6f5c1` cannot compile the existing branch because it lacks the sandbox/workload/network APIs used by the matched source; signed implementation checkpoint `864455bf1a104f0215b7c912a45800b0a0538973` compiles and passes, while local `upstream/engine-linux-sandbox` remains intentionally unpublished at signed handoff head `38d9c695e7a6915e5ce45d12c893dc323a661af7`. In the current devcontainer runtime-parity run, the Container Compose lane passes E03 in 2.521 seconds, fails C04 functionally, meets the Docker timing target only on C03, and completes the other fixtures at 1.126–7.609× Docker. The separate Apple-stock lane owns the 302-second E03 timeout and 12× E04 result. The latest isolated MBP certificate passes the installed plugin through public create/start/logs/stop/delete plus native cycles, and its focused service suite passes 10/10. Security review, migration rehearsal, the complete same-host performance run, immutable publication, and rollback evidence remain.
...
The current `docker-compose-performance-matrix` implements the warm fixed-work subset for detached startup to first readable log, stdout/stderr/mixed small records, exact 16 KiB and 1 MiB records, tail 10/1,000/all, follow across forced `json-file` rotation, and 1/10/50-service foreground aggregation. It alternates Docker-first and candidate-first repetitions, records schedule position and direction in the raw TSV, requires every declared fixture/repetition, and reports median/P95 ratios with a configurable noise band separately from the 10× regression guard. Attached cold-start, slow-sink, non-blocking queue, `local`/compression resource, since/until, dual-cache/remote, idle resource, and release-artefact runs remain required before the performance contract is complete.
