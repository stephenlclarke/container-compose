...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; the journald codec/provider contract is implemented but its production Linux service is not; plugin hosting, Engine integration, handoff, and release evidence remain |
...
| Local Container development head | `99e0fa1df8eb27c603ded95a971cfd47a3d5ea3c` (`887848ed719a05836d2f846b69a22749e61f2f62` journald implementation) |
...
| Remote providers | Syslog, Fluentd, GELF, Splunk HEC, AWS Logs, and Google Cloud Logs have option contracts, authority activation, recovery, and focused semantic tests. AWS uses an isolated official SDK adapter. Google Cloud uses the official Go client inside the signed semantic helper, with Application Default Credentials and Moby-compatible project, instance metadata, payload, asynchronous delivery, overflow, flush, and close behavior. Journald now has its Moby-compatible option/field codec, direct-reader and generation-fenced provider lifecycle, conditional catalog registration, and a narrow signed Linux-service boundary. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | Journald remains correctly unadvertised because its production signed Linux workload, system-journal persistence/query adapter, reader routing, supervision, and recovery are absent. The isolated production Docker-plugin service plane is also absent. |
...
- Work package 7 has a Docker logging-plugin codec/adapter and provider lifecycle primitives. Signed Container commit `887848ed719a05836d2f846b69a22749e61f2f62` adds the journald option/field codec, Docker tag and RE2 delegation, native-reader contract, idempotent writer/reader sessions, generation/effect fencing, conditional catalog registration, and exact service boundary. The driver is intentionally not advertised without a concrete service. The production signed Linux journald workload, system-journal persistence/query adapter, reader routing, supervision/recovery, real-systemd conformance, and the isolated production plugin service plane remain unimplemented.
