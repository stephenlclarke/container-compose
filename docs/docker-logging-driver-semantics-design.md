...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; AWS Logs and the prior native/remote paths are implemented; Google Cloud, journald, production plugin hosting, Engine integration, handoff, and release evidence remain |
...
| Matched Container revision | `39a36e287b3462227f83a33ec0bf7a9299051f1e` |
...
| Remote providers | Syslog, Fluentd, GELF, Splunk HEC, and AWS Logs have generated option contracts, authority activation, recovery, and focused semantic tests. AWS Logs uses an isolated official SDK adapter for default credentials, SigV4, endpoints, retry, and IMDS, with Moby-compatible group/stream creation, bounded startup queue, multiline, batching, UTF-8 sizing, and sequence-token recovery. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | Google Cloud Logging is absent. Journald and the isolated production Docker-plugin service plane are absent. |
...
- Work package 6 has production Syslog, Fluentd, GELF, Splunk HEC, and AWS Logs providers. Google Cloud remains unimplemented. The AWS slice is pinned at signed Container commit `39a36e287b3462227f83a33ec0bf7a9299051f1e`; 10 focused provider tests, one authority-plane publication test, `make check`, and the 1,762-test full Container gate pass on the programme MacBook Pro. The official AWS Swift SDK is isolated behind a dependency-neutral client boundary and supplies default credentials, SigV4, endpoint resolution, retry/throttling, and IMDS. The matched Compose check and 922 launch/orchestrator tests pass against that exact pin.
