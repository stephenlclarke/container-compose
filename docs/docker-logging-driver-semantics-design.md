...
| Container authority/model | The matched Container authority persists lossless requested and resolved v2 state, system defaults, protected options, provider identity/generation, read policy, and delivery mode. Direct typed and CLI requests share the same resolver. | Routine inspection is redacted. Engine-facing public inspection still needs to be integrated with the shared Engine server. |
...
| Shared Engine API | `container-engine-api` 0.2.2 implements the hardened listener, generated API 1.44 through 1.53 route ledger, shared executable/gateway, exclusive provider identity, deterministic private-session shutdown, and starvation-free framed I/O. Devcontainer `77f3cb5dc7cce8d3f88de960b7d93d676a768137` consumes that exact release and supplies the stock adapter. Signed Container head `4ad84ff95cb68741132c455e1b2f537ac904414e` adds the enhanced authority adapter and restart-stable private provider session. `ContainerLogs` uses the canonical reader for stopped history and a lossless record stream for active follow. `ContainerAttach` composes that replay/follow source with exact-process stdin/stdout/stderr fallback, bounded no-drop hijack transport, Docker detach keys, and canonical attach/detach events. `/info` and inspect project authority-owned logging state without advertising incomplete whole responses. | `ContainerLogs` and `ContainerAttach` are advertised by the enhanced provider. Whole-route composition must safely expose `/info` and inspect alongside their non-logging fields. WebSocket attach, resize, the common public-gateway route owner, and the complete external-client route/provider matrix remain unimplemented or uncertified. |
...
- Work package 8 has the logging-specific Docker REST/streaming routes plus the released shared wire/listener/provider-identity foundation in `container-engine-api`; devcontainer consumes that foundation without a duplicate listener. Signed Container implementation `72a76ab596e95fa775593bc3bcbef67135c384e4`, with handoff head `4ad84ff95cb68741132c455e1b2f537ac904414e`, integrates canonical stopped replay and lossless active follow with the enhanced authority, maps exact-process stdin/stdout/stderr fallback into bounded no-drop Docker hijack sessions, consumes detach keys, publishes attach/detach events through the canonical broadcaster, and advertises `ContainerLogs` plus `ContainerAttach`. Safe whole-route composition for `/info` and inspect, WebSocket attach, resize, the common public route owner, and the external-client suites remain unimplemented or uncertified.
