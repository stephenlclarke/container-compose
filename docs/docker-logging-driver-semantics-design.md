...
| Item | Value |
| --- | --- |
| Status | Implementation in progress; local Compose typed execution, the complete production journald path, and the enhanced Engine logs/inspect/info/hijack/WebSocket/resize routes are implemented and validated; public gateway installation, dependency publication, plugin hosting, devcontainer handoff, and paired release evidence remain |
...
| Compose checkpoints | Published reproducible `origin/main`: `1f0f944b3d918a39ad97d1f12bb7b7c5ef6146a0`; active signed local `main`: `784aacaa26f2ad698460044a573adae30665628a`; the local manifest still names unreachable Container checkpoint `2a79b4553a342e33411666a88ad20ccd2ce46551` and therefore must not be pushed yet |
| Local Container checkpoints | Signed implementation `6a668b2b5d42246efcad3316374f6d0e0d2eaf14`; signed handoff head `f5182dd9771153d231025fb449fe98fac5f645da`; production journald supervision/reclamation remains at `84d160671f3ba6c265a02b49b2ff4309f6584d30` in that ancestry |
...
| Shared Engine API | Released `container-engine-api` 0.3.4 at signed commit `73cef37b3693e3fc1acd650782ee7b449ab65b92` |
...
| Remote providers | Syslog, Fluentd, GELF, Splunk HEC, AWS Logs, and Google Cloud Logs have option contracts, authority activation, recovery, and focused semantic tests. AWS uses an isolated official SDK adapter. Google Cloud uses the official Go client inside the signed semantic helper, with Application Default Credentials and Moby-compatible project, instance metadata, payload, asynchronous delivery, overflow, flush, and close behavior. Journald now has its Moby-compatible option/field codec, direct-reader lifecycle, versioned bounded Swift/Go protocol, restart-safe backend, concrete systemd adapter, verified pinned OCI workload, protected persistent mounts, shared-sandbox supervision, exact sandbox/workload-generation routing, readiness-gated catalog publication, safe generation rollover, and authority-ordered terminal reclamation. Current warnings-as-errors Swift, Thread Sanitizer, Go race, deterministic multi-build, staging, integrity, and packaged real-systemd integration gates pass locally. A generation-aware provider registry and Docker logging-plugin protocol adapter exist. | Journald's local production runtime path is complete, but release-signature trust, synchronized dependency publication, paired Docker certification, and final programme-level install/upgrade/rollback/shutdown evidence remain. The isolated production Docker-plugin service plane is also absent. |
| Shared Engine API | Released `container-engine-api` 0.3.4 at `73cef37b3693e3fc1acd650782ee7b449ab65b92` implements the hardened listener, generated API 1.44 through 1.53 route ledger, shared executable/gateway, gateway-owned `GET`/`HEAD /_ping`, exclusive provider identity, deterministic private-session shutdown, starvation-free framed I/O, WebSocket framing, resize dispatch, and fail-closed complete-response composition. Devcontainer `4f71de4cdf4d8f59d27f5fa7972f4f2f4d2da7af` supplies the stock adapter while still pinned to Engine API 0.3.3. Signed Container implementation `6a668b2b5d42246efcad3316374f6d0e0d2eaf14`, with handoff head `f5182dd9771153d231025fb449fe98fac5f645da`, supplies the 0.3.4 enhanced authority adapter and restart-stable private provider session. Canonical stopped replay, active follow, exact-process fallback, bounded hijack and WebSocket attach, detach keys/events, terminal resize, complete `SystemInfo`, and complete `ContainerInspect` all use that authority. | The enhanced provider advertises `ContainerLogs`, hijack and WebSocket `ContainerAttach`, `ContainerResize`, `ContainerInspect`, and `SystemInfo`. Public service installation/supervision, the API-socket grant, direct isolated-provider reads, exact provenance for several broader lifecycle fields, and the complete external-client route/provider matrix remain unimplemented or uncertified. |
...
- Work package 7 has a Docker logging-plugin codec/adapter and a complete local production journald path. Signed Container commits `887848ed719a05836d2f846b69a22749e61f2f62`, `20071d97d10b386c2a24c84c51bca0e37c0280aa`, `a42ecf2fe1ffa582e34cfa74f6cf1ddba8505368`, `bed5de1686bc005ad77ab63025a5582f37601738`, `79c89babc7399c0cc1d4f800bd5ec092cc6c153d`, `dcefedba2b3b5806953c32e35ca2edaea24658a0`, and `e8cc75f001d24896144a5e44e33d1f7a5d1e5729` establish the Moby codec, direct reader, generation-fenced lifecycle, bounded replay-safe Swift/Go protocol, restart-safe backend/checkpoints, and pinned Linux/arm64 systemd workload. Signed commit `84d160671f3ba6c265a02b49b2ff4309f6584d30` adds installed asset and exact digest verification, separate protected protocol/journal mounts, shared-sandbox supervision, exact sandbox/workload-generation service routing, terminal readiness withdrawal/rematerialization, catalog probing, protocol-v2 terminal writer/reader reclamation, and safe sandbox-generation rollover. The current gate passes 31 warnings-as-errors Swift tests, 14 Engine routing/recovery tests with a clean Thread Sanitizer run, Go race tests, deterministic multi-build verification at workload digest `sha256:9e3ab273aa26ba6bb62d6905b4034451b19d42964592e673197242bd05638841`, OCI archive integrity at `e391eb6f62570f2ea41b04185c3f01ed6a736f2376b833b6854952314e18ad0d`, debug staging, and a packaged Swift-to-real-systemd round trip. Release-signature trust, synchronized dependency publication, paired Docker behavior/performance evidence, final programme-level lifecycle evidence, and the isolated production plugin service plane remain.
- Work package 8 has the logging-specific Docker REST/streaming routes plus released `container-engine-api` 0.3.4 at `73cef37b3693e3fc1acd650782ee7b449ab65b92`; devcontainer consumes Engine API 0.3.3 of that foundation without a duplicate listener. Signed Container implementation `6a668b2b5d42246efcad3316374f6d0e0d2eaf14`, with handoff head `f5182dd9771153d231025fb449fe98fac5f645da`, integrates the 0.3.4 canonical stopped replay and lossless active follow, exact-process fallback, bounded no-drop hijack and WebSocket sessions, detach keys/events, terminal resize, complete `SystemInfo`, and complete `ContainerInspect`. The enhanced fingerprint advertises `ContainerLogs`, `ContainerAttach`, `ContainerAttachWebsocket`, `ContainerResize`, `ContainerInspect`, and `SystemInfo`; the gateway independently owns `GET`/`HEAD /_ping`. Thirty-eight focused Engine logs/attach/WebSocket/resize tests and the complete 1,832-test Container suite pass. Real-runtime TTY resize and detach/re-attach, public service installation/supervision, direct isolated-provider reads, exact broader lifecycle-field provenance, devcontainer 0.3.4 adoption, and external-client suites remain.
