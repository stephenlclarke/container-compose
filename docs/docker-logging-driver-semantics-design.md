...
| Shared Engine API | `container-engine-api` implements Docker-shaped `/info`, inspect `LogConfig`, logs, attach/hijack, version routing, framing, query parsing, and error envelopes against an abstract authoritative backend. Version 0.1.0 also owns the hardened Unix listener and exclusive provider identity; devcontainer `4d98d240fab8` consumes those shared products. | Container's authority, the common `container-engine` executable/provider session, logging lifecycle events, and the complete external-client route surface are not yet integrated. |
