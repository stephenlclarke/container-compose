...
The original devcontainer evidence is at revision `b31e80b2b9c09ecc73bb3badf9cd5cf16550a538`:
...
The sibling checkout now contains substantial later Engine/router/runtime work and is dirty. The revision above is therefore historical design evidence only. Implementation MUST first select a clean, reviewed, immutable accepted devcontainer head and record the extraction diff; it MUST NOT extract from, rewrite, or treat the dirty checkout as validated source.
