...
| Item | Value |
| --- | --- |
| Status | Design complete; implementation in progress—the neutral Engine API 0.3.5, enhanced Container logs/inspect/info/hijack/WebSocket/resize adapter, and locally live public gateway lifecycle exist; full route closure, socket grants, volume providers, advanced mounts, devcontainer adoption, and `use_api_socket` remain |
...
| Matched Container checkpoints | Enhanced authority `6a668b2b5d42246efcad3316374f6d0e0d2eaf14`; public gateway lifecycle and discovery fix `ac1803ec555960ce49fcec1d6a5b718d781629e0` |
...
Those components were inputs to the extraction, not proof of closure. The accepted implementation now lives in the clean `stephenlclarke/container-engine-api` repository: signed release 0.3.5 is revision `78cb4cb5781d6dbe9f0d34a1b925ee8dcaacdc98`. It adds the reusable gateway lifecycle used by performance and adopter tests to the shared executable/gateway, checksum-pinned generated route ledger, private framed provider session with pull streams and bidirectional hijack, provider-owned state-root identity migration, bounded large-body streaming, gateway ping ownership, WebSocket attach framing, resize routing, and fail-closed whole-response composition. Devcontainer signed revision `4f71de4cdf4d8f59d27f5fa7972f4f2f4d2da7af` still consumes Engine API 0.3.3 and supplies the stock adapter; signed local Container `ac1803ec555960ce49fcec1d6a5b718d781629e0` consumes 0.3.5, supplies the enhanced authority, and owns the local public service lifecycle. Engine hosted CI, 66 same-host gateway performance samples, the matched 1,835 Swift Testing plus 94 XCTest Container suite, and isolated Docker CLI info lifecycle are green; devcontainer 0.3.5 adoption and whole-stack runtime/performance closure are not.

`STORAGE-WP-02` remains **in progress**, not complete. The neutral listener/wire protocol, generated API 1.44 through 1.53 ledger, stock/enhanced capability profiles, immutable state-root identity, canonical SHA-256 provider fingerprint, shared `container-engine` executable, fail-closed gateway, private out-of-process provider session, devcontainer stock adapter, enhanced Container logging/inspection adapter, ping, WebSocket attach, resize, and local public service lifecycle now exist. Remaining exit evidence is complete route profiles and external-client conformance, immutable authority handoff, typed guest grant, and whole-stack comparable-performance proof. Devcontainer's capability set still truthfully omits image push/auth/build sessions, and the managed-volume store still supports only the local directory case.
