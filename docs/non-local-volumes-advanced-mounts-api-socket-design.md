...
| API socket | `container-engine-api` 0.1.0 now supplies neutral Docker wire types, version routing, the hardened Unix listener, and exclusive provider identity, but there is no shared `container-engine` executable or Container-authority adapter. | A raw socket bind still has no complete authoritative Docker service behind it and cannot satisfy `use_api_socket`. |
| Devcontainer | At `4d98d240fab8`, devcontainer pins `container-engine-api` 0.1.0 and consumes its wire/router/server/runtime-identity products, removing its duplicate listener implementation. Its router still intentionally implements a bounded Dev Containers subset and its provider owns separate state. | The shared transport/identity foundation is real, but mounting the devcontainer socket would still omit image push/auth and create split-brain state. |
...
Those components were inputs to the extraction, not proof of closure. The accepted implementation now lives in the clean `stephenlclarke/container-engine-api` repository: version 0.1.0 is code revision `5f8e01beeef9823d65d0d029817e6cb3e5c59efb`, while main revision `60b09cbcb249829918e91cc536af730dff9ef181` adds its green macOS CI workflow. Devcontainer revision `4d98d240fab885bb0da817af4aeb41704f081f73` consumes that exact release and removes 1,517 lines of duplicate transport/runtime-support code. The package's 48 tests and devcontainer's 206-test, 95.52%-covered closure gate are green.

`STORAGE-WP-02` therefore remains **in progress**, not complete. The neutral listener, wire protocol, version routing, stock/enhanced capability profiles, immutable state-root identity, canonical SHA-256 provider fingerprint, and fail-closed private provider selection exist. The shared `container-engine` executable, out-of-process stock/enhanced provider-session protocol, complete generated API 1.53 route ledger, and explicit provider-handoff operation remain to be implemented. Devcontainer's endpoint policy still lacks image push/auth and is not the enhanced authority. The managed-volume store still supports only the local directory case.
