...
| API socket | `container-engine-api` 0.2.1 supplies neutral Docker wire types, the generated 107-operation API 1.44 through 1.53 ledger, hardened Unix listener, shared `container-engine` executable, exclusive provider identity, fail-closed gateway, and private out-of-process provider session. There is still no enhanced Container-authority adapter or complete handler set. | A raw socket bind still lacks the complete enhanced authority, credentials, grants, auth/push/session handlers, and handoff required by `use_api_socket`. |
| Devcontainer | At `3a62fb981bd3`, devcontainer pins `container-engine-api` 0.2.1 and exposes its stock adapter through a private fingerprint-bound provider session while retaining a separately selected legacy standalone mode for migration. Its router still intentionally implements a bounded Dev Containers subset and its provider owns separate state. | The stock provider boundary is real and no longer needs to own the public gateway selection, but mounting it would still omit image push/auth and would not become the enhanced Container authority. |
...
Those components were inputs to the extraction, not proof of closure. The accepted implementation now lives in the clean `stephenlclarke/container-engine-api` repository: signed version 0.2.1 is revision `5ae9a05bf01dfb7740bc4eb993adeddfe9fbac79`. It adds the shared executable/gateway, checksum-pinned generated route ledger, private framed provider session with pull streams and bidirectional hijack, and provider-owned state-root identity migration. Devcontainer signed revision `3a62fb981bd3bde8a98d4ec320229809aa78c922` consumes that exact release and supplies the stock adapter. The package's 54 tests and devcontainer's 207-test, 95.53%-covered local closure gates are green.

`STORAGE-WP-02` remains **in progress**, not complete. The neutral listener/wire protocol, generated API 1.44 through 1.53 ledger, stock/enhanced capability profiles, immutable state-root identity, canonical SHA-256 provider fingerprint, shared `container-engine` executable, fail-closed gateway, private out-of-process provider session, and devcontainer stock adapter now exist. Remaining exit evidence is the accepted hosted build and comparable-performance proof; the explicit provider handoff and enhanced Container adapter land in the later authority/cutover work. Devcontainer's capability set still truthfully omits image push/auth/session, and the managed-volume store still supports only the local directory case.
