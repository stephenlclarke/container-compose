...
1. A trusted general container-family self-hosted MBP runner lane, separate from release/signing labels.
2. Local and MBP-runner equivalents for each required GitHub workflow step, with documented hosted-only exceptions.
3. A reproducible local CodeQL target for the supported Go surface and SARIF retention/upload path.
4. Local wrapper targets for SwiftLint, SwiftFormat, `actionlint`, Address Sanitizer, and Thread Sanitizer so the Quality workflow can be reproduced without a hosted runner; `make check` alone does not currently run those checks.
5. A release-build performance harness with paired sampling, comparable-or-better median/P95 gates, and startup, teardown, logs, watch-sync, and build-context lanes; retain the current 10x/debug matrix as diagnostic only until then.
6. A shared slice ledger and exact-head evidence layout covering every repository in the matched stack, including a read-only issue/pull-request/discussion watch record.
7. A test-impact map from contract/controller/file areas to focused, component, slice, stack, and release targets.
8. Immutable verification worktrees and artefact naming keyed by commit and dependency fingerprint.
...
The current release helper opens and immediately merges its Compose promotion
pull request and still exposes the legacy direct-main mode. Those are recorded
implementation gaps, not exceptions to this process. Enablers 13 and 14 are
required before the helper may perform the next programme source promotion.
