...
progress. CodeQL is release-only: the package workflow runs the exact Go
normalizer analysis after the fail-fast Homebrew preflight and before any
native release package build. Pull requests, ordinary `main` pushes,
documentation changes, schedules, and published benchmark reports do not run
CodeQL. The standalone workflow is restricted to recovery of an already
published `current` or semantic release. A missing or unsuccessful release
analysis is not a passing result.
...
The weekly `Repository Hygiene` workflow applies the same fail-closed principle to GitHub branches. It removes only an unchanged, unprotected branch that is the exact recorded head of a pull request merged into `main` at least seven days earlier, after rechecking that no pull request has opened and the SHA and merge proof are unchanged. The final deletion uses Git's atomic `--force-with-lease` comparison against that SHA, so a concurrent push preserves the branch. A post-delete pull-request query restores the exact deleted head with create-only semantics if a pull request opened during the deletion window; it never replaces newer work. It reports and preserves all other branches. Local worktrees remain outside that workflow and must still be closed out with `make worktree-audit` and the explicit commands above.
