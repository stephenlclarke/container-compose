...
CLI coverage also exposed a product defect: guest `run`/`exec` help and option names could be consumed by Compose's help dispatcher or argument parser. The candidate now protects the guest argument boundary, tests through the actual root parser, and exercises twelve independently timed native CLI/parser cases. The downloaded Docker Compose 5.3.1 reference provides the no-runtime missing-file/error-phase comparison. This does not establish full runtime parity or close the remaining coverage gate.
