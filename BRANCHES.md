...
`container` pins the builder shim through `BUILDER_SHIM_REPOSITORY` and `BUILDER_SHIM_VERSION`. The current Stephen fork default is `ghcr.io/stephenlclarke/container-builder-shim/builder:0.13.3`. Update the shim source on `main`, publish an immutable image tag, verify the GHCR manifest is available, then update `container` to that tag; do not point release packages at untagged, unpublished, or debug builder images.
