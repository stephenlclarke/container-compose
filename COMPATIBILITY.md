...
| Status | Commands |
| --- | --- |
| Supported | `config`, `convert`, `create`, `up`, `down`, `build`, `pull`, `pull --include-deps`, `pull --ignore-buildable`, `pull --ignore-pull-failures`, `pull --policy always/missing`, `pull --quiet/-q`, `push`, `push --include-deps`, `push --ignore-push-failures`, `push --quiet/-q`, `ls`, `ps`, `logs`, output-only `attach --no-stdin --sig-proxy=false`, `exec`, `exec --index`, `run`, `start`, `stop`, `restart`, `rm`, `images`, `volumes`, `stats`, `stats --all`, `cp`, `cp --index`, `export`, `export --index`, explicit published-port `port`, `port --index`, `kill`, `version` |
...
Expected result: `container compose config`, `container compose convert`, `build`, `pull --include-deps --policy missing --quiet`, `push --include-deps --quiet`, `create`, `up`, `ps`, `logs`, `exec`, `stats`, `cp`, `volumes`, `rm --force --volumes` for anonymous volumes, and `down --volumes` run through [`apple/container`][apple-container].
