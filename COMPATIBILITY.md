...
| [C4: Plugin Gap, Metadata, Storage, And API Socket](#c4-plugin-gap-metadata-storage-and-api-socket) | `container-compose` gap | Annotations, logging options, inherited mounts, advanced service volume options, API socket, and block I/O |
...
Expected result: `container compose up` rejects this because annotations, logging/storage options, inherited mounts, advanced service volume options, API socket exposure, and block I/O controls need plugin implementation and security review.
...
- `container-compose`: maps service `pull_policy: daily`, `weekly`, and `every_<duration>` through direct image pulls and local pull timestamp metadata, maps service `pull_policy: build` through the existing build path, and maps long-form tmpfs `size`/`mode` through Apple `container --mount type=tmpfs`. It still needs runtime mapping, inherited mount behavior, advanced volume option policy beyond tmpfs size/mode, logging/storage policy, API socket security review, and block I/O handling.
