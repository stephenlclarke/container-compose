...
4. <img alt="IMPLEMENT" src="https://img.shields.io/badge/IMPLEMENT-7C3AED?style=flat-square"> Slice new small apple/container PRs for the remaining unimplemented issue groups: events, attach, process signal proxying, multi-network aliases, fixed IPs, BuildKit option passthrough, build `HEALTHCHECK`, remaining blkio/device controls, and volume seed/copy behavior.
...
- **Missing apple/container primitive:** A safe Docker-compatible or apple/container-compatible API socket boundary with credential handoff and least-privilege controls, plus accepted block I/O resource controls for blkio weight and read/write throttling. apple/container can mount Unix sockets and report block I/O stats, and [apple/container#1595](https://github.com/apple/container/pull/1595) proposes a blkio cgroup tuning API, but released upstream apple/container does not yet expose the Docker Compose API-socket or accepted blkio resource-control behavior that these fields require.
- **container-compose status:** Rejected before resources are created.
