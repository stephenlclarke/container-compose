...
- **Status:** <img alt="PARTIAL" src="https://img.shields.io/badge/PARTIAL-B26A00?style=flat-square">
- **Compose surface:** Explicit service `healthcheck`, Dockerfile-inherited image `HEALTHCHECK`, `depends_on.condition: service_healthy`, `deploy.mode: replicated-job`, `deploy.mode: global-job`, external/content/environment-backed service `configs` and `secrets`, service `restart`, and `deploy.restart_policy`.
- **Missing apple/container primitive:** Released upstream `apple/container` still needs accepted health status/configuration/observer/create surfaces before explicit health support can leave the fork. Released upstream also needs accepted image config parsing for Dockerfile-inherited `HEALTHCHECK` metadata; the local fork exposes this through `ImageResource.Variant.healthCheck` in the image healthcheck metadata slice. Compose-compatible job lifecycle and scheduler semantics, first-class config/secret store or materialization primitives, ownership/mode controls for non-file-backed grants, and accepted restart policy create/runtime/timing support remain runtime gaps for released upstream use. Exit-code metadata for `service_completed_successfully` is implemented only on the local fork until [apple/container#1562](https://github.com/apple/container/pull/1562) or an equivalent upstream API lands.
- **container-compose status:** Explicit service healthchecks, Dockerfile-inherited image healthchecks, timing-only Compose healthcheck overrides over image defaults, `depends_on.condition: service_healthy`, `depends_on.condition: service_completed_successfully`, service-level `restart`, and the fork-backed subset of `deploy.restart_policy` are supported on the integration branch. Supported deploy restart values are `condition: none`, `condition: any` or an empty policy, `condition: on-failure` with optional `max_attempts`, plus `delay` and `window` when the fork's restart-policy timing slice is present. Timing-only healthcheck overrides reject before resources are created when the image does not expose Dockerfile `HEALTHCHECK` metadata. Job modes and non-file-backed config/secrets are still rejected before resources are created.
...
- <img alt="PARTIAL" src="https://img.shields.io/badge/PARTIAL-B26A00?style=flat-square"> [A4: Partial, Health, Config And Secret Stores, And Restart](#a4-partial-health-config-and-secret-stores-and-restart): Demonstrates explicit healthchecks, healthy dependency gates, deploy job modes, external/content/environment-backed service secrets/configs, and restart policies. The explicit `healthcheck`, `service_healthy`, and `service_completed_successfully` gates in that example are supported on the fork-backed branch, but the file still rejects because of the remaining apple/container gaps.
...
Expected result: on the fork-backed integration branch, explicit service `healthcheck` definitions, Dockerfile-inherited image healthchecks, timing-only healthcheck overrides over image defaults, `depends_on.condition: service_healthy`, service-level `restart`, and the `deploy.restart_policy` condition/max-attempt/timing subset are accepted when the runtime health, image metadata, and restart slices are present. `container compose up` still rejects this complete file because [`apple/container`][apple-container] needs job lifecycle/scheduler semantics and first-class config/secret stores or materialization. File-backed service configs/secrets are supported in [S1](#s1-supported-local-web-stack), `service_completed_successfully` is supported on the fork-backed integration branch through [apple/container#1562](https://github.com/apple/container/pull/1562), explicit health support depends on the fork's health status/configuration/observer/CLI handoff slices, Dockerfile-inherited healthchecks depend on the fork's image healthcheck metadata slice, and service restart depends on the fork's restart-policy create/runtime/timing slices.
