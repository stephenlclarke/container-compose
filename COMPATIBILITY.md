...
2. <img alt="FORK MERGE" src="https://img.shields.io/badge/FORK%20MERGE-2563EB?style=flat-square"> Keep the #1562 exit-metadata cherry-pick, healthcheck handoff slices, and restart-policy create/runtime slices in the fork integration branch, then continue testing stopped-container `wait`, `service_completed_successfully`, explicit service `healthcheck`, `service_healthy`, service `restart`, fork-backed `deploy.restart_policy` condition/max-attempt mapping, and failed one-shot jobs there. Next, decide whether the next restart slice is upstream deploy timing (`delay` / `window`) or a plugin-side health/image capability.
...
- **Status:** <img alt="PARTIAL" src="https://img.shields.io/badge/PARTIAL-B26A00?style=flat-square">
- **Compose surface:** Explicit service `healthcheck`, `depends_on.condition: service_healthy`, `deploy.mode: replicated-job`, `deploy.mode: global-job`, external/content/environment-backed service `configs` and `secrets`, service `restart`, and `deploy.restart_policy`.
- **Missing apple/container primitive:** Released upstream `apple/container` still needs accepted health status/configuration/observer/create surfaces before explicit health support can leave the fork. Dockerfile-inherited `HEALTHCHECK` metadata is still missing from image config parsing. Compose-compatible job lifecycle and scheduler semantics, first-class config/secret store or materialization primitives, ownership/mode controls for non-file-backed grants, and accepted restart policy create/runtime support remain runtime gaps for released upstream use. Exit-code metadata for `service_completed_successfully` is implemented only on the local fork until [apple/container#1562](https://github.com/apple/container/pull/1562) or an equivalent upstream API lands. `deploy.restart_policy.delay` and `deploy.restart_policy.window` still need upstream restart timing primitives because the current fork-backed restart runtime exposes policy mode and retry count only.
- **container-compose status:** Explicit service healthchecks, `depends_on.condition: service_healthy`, `depends_on.condition: service_completed_successfully`, service-level `restart`, and the fork-backed subset of `deploy.restart_policy` are supported on the integration branch. Supported deploy restart values are `condition: none`, `condition: any` or an empty policy, and `condition: on-failure` with optional `max_attempts`. Dockerfile-inherited healthchecks without an explicit Compose `healthcheck.test`, job modes, non-file-backed config/secrets, and deploy restart `delay` / `window` are rejected before resources are created.
...
Expected result: on the fork-backed integration branch, explicit service `healthcheck` definitions, `depends_on.condition: service_healthy`, service-level `restart`, and the `deploy.restart_policy` condition/max-attempt subset are accepted when the runtime health and restart slices are present. `container compose up` still rejects this complete file because [`apple/container`][apple-container] needs job lifecycle/scheduler semantics and first-class config/secret stores or materialization. If a Compose file uses `deploy.restart_policy.delay` or `deploy.restart_policy.window`, `container-compose` rejects those fields before resources are created until [`apple/container`][apple-container] exposes restart timing primitives. File-backed service configs/secrets are supported in [S1](#s1-supported-local-web-stack), `service_completed_successfully` is supported on the fork-backed integration branch through [apple/container#1562](https://github.com/apple/container/pull/1562), explicit health support depends on the fork's health status/configuration/observer/CLI handoff slices, and service restart depends on the fork's restart-policy create/runtime slices. Dockerfile-inherited healthchecks remain blocked until [`apple/container`][apple-container] exposes image `HEALTHCHECK` metadata.
...
- [`apple/container`][apple-container]: the local fork can run configured healthchecks, publish `ContainerSnapshot.health`, parse service restart create options, and restart containers from local policy; released upstream does not yet expose that full surface. Dockerfile-inherited `HEALTHCHECK` metadata, job lifecycle/scheduler semantics, first-class config/secret store or materialization primitives, and deploy restart timing semantics remain gaps; exit metadata is available only on the local fork until #1562 or an equivalent API lands.
- `container-compose`: maps explicit service healthchecks, `service_healthy`, `service_completed_successfully`, service-level `restart`, and `deploy.restart_policy` condition/max-attempt values on the fork-backed branch, then detects the remaining fields before reporting the relevant apple/container runtime gap.
