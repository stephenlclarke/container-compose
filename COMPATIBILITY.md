...
- **apple/container path:** Direct `ContainerClient` list/get/logs/logRecords/logRecordFile/copy/export/stats APIs, `ProcessIO`, `ContainerClient.createProcess`, and `ClientProcess.start`. Static `logs --since`, `logs --until`, and `logs --timestamps` use the local apple/container log-options and structured log-record API work; the local apple/container CLI accepts RFC 3339 and Unix timestamp filters, renders static `container logs --timestamps`, renders followed `container logs --follow --timestamps/--since/--until` from structured records, flushes final unterminated followed structured records when direct container status shows the target is no longer live, and exposes static plus followed rotated raw and structured replay through `ContainerLogOptions`.
