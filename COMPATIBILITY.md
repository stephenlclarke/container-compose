...
| Status | Commands |
| --- | --- |
| Supported | `config`, `convert`, `create`, `up`, `down`, `build`, `pull`, `push`, `ls`, `ps`, `logs`, output-only `attach --no-stdin --sig-proxy=false`, `exec`, `run`, `start`, `stop`, `restart`, `rm`, `images`, `volumes`, `stats`, `stats --all`, `cp`, `export`, explicit published-port `port`, `kill`, `version` |
| Present but blocked by [`apple/container`][apple-container] runtime gaps | dynamic host-port allocation, `top`, `events`, `port --index` values other than `1`, `pause`, `unpause`, `wait`, `stats --no-trunc`, `cp --archive`, `cp --follow-link` |
