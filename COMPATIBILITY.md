...
#### Health, config/secret stores, service restart, and job modes
...
- **container-compose status:** Explicit service healthchecks, `depends_on.condition: service_healthy`, `depends_on.condition: service_completed_successfully`, and service-level `restart` are supported on the fork-backed integration branch. Dockerfile-inherited healthchecks without an explicit Compose `healthcheck.test`, job modes, non-file-backed config/secrets, and `deploy.restart_policy` are rejected before resources are created.
...
Expected result: on the fork-backed integration branch, explicit service `healthcheck` definitions, `depends_on.condition: service_healthy`, and service-level `restart` are accepted when the runtime health and restart slices are present. `container compose up` still rejects this complete file because [`apple/container`][apple-container] needs job lifecycle/scheduler semantics, first-class config/secret stores or materialization, and additional deploy restart timing semantics for `deploy.restart_policy`. File-backed service configs/secrets are supported in [S1](#s1-supported-local-web-stack), `service_completed_successfully` is supported on the fork-backed integration branch through [apple/container#1562](https://github.com/apple/container/pull/1562), explicit health support depends on the fork's health status/configuration/observer/CLI handoff slices, and service restart depends on the fork's restart-policy create/runtime slices. Dockerfile-inherited healthchecks remain blocked until [`apple/container`][apple-container] exposes image `HEALTHCHECK` metadata.
...
- `container-compose`: maps explicit service healthchecks, `service_healthy`, `service_completed_successfully`, and service-level `restart` on the fork-backed branch, then detects the remaining fields before reporting either the apple/container runtime gap or the deploy restart model gap.
