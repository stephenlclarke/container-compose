...
| Status | Commands |
| --- | --- |
| Supported | `config`, `convert`, `create`, `up`, `down`, `build`, `pull`, `push`, `ls`, `ps`, `logs`, output-only `attach --no-stdin --sig-proxy=false`, `exec`, `run`, `start`, `stop`, `restart`, `rm`, `images`, `volumes`, `stats`, `cp`, `export`, explicit published-port `port`, `kill`, `version` |
| Present but blocked by [`apple/container`][apple-container] runtime gaps | dynamic host-port allocation, `top`, `events`, `port --index` values other than `1`, `pause`, `unpause`, `wait`, `stats --all`, `stats --no-trunc`, `cp --archive`, `cp --follow-link` |
...
| [A5: Apple Gap, Runtime Data Commands](#a5-apple-gap-runtime-data-commands) | [`apple/container`][apple-container] gap | Process listing, event streams, dynamic host-port allocation, pause/unpause, wait metadata, and stats output controls |
...
- [`apple/container`][apple-container]: missing dynamic host-port allocation, process listing, event streaming, pause/unpause, wait/exit metadata, stats all-container/truncation primitives, and copy archive/follow-link controls.
- `container-compose`: exposes the command names, resolves explicit published-port lookups from runtime snapshots, and reports the Apple runtime gap for requests that need unavailable runtime state.
