...
- **apple/container path:** Direct `ContainerClient` list/get/logs/logRecords/logRecordFile/copy/export/stats APIs, `ProcessIO`, `ContainerClient.createProcess`, and `ClientProcess.start`. Static `logs --since`, `logs --until`, and `logs --timestamps` use the local apple/container log-options and structured log-record API work; the local apple/container CLI accepts RFC 3339 and Unix timestamp filters, renders static `container logs --timestamps`, renders followed `container logs --follow --timestamps/--since/--until` from structured records, and exposes static rotated raw and structured replay through `ContainerLogOptions`.
- **container-compose status:** Supported for the listed direct API paths on the local integration stack. Static raw and timestamped log replay requests include rotated local log files where apple/container exposes them, while followed streams stay on active log handles. Time filters accept RFC 3339 timestamps, Unix timestamps in seconds with optional fractional seconds, and relative durations. Line-boundary tests cover empty logs, blank records, final newlines, CRLF/CR separators, prefixed blank records, and unterminated final records. Released timestamped follow, filtered log follow, rotated replay, and byte-preserving structured log support wait for upstream apple/container acceptance of the structured record, record-file, and rotated replay API shape. Released structured stdout/stderr log identity semantics and runtime process/event controls remain apple/container gaps or compatibility decisions.
- **Example:** [S1](#s1-supported-local-web-stack).
...
- **Missing apple/container primitive:** Compose-compatible remote service logging drivers, rotated follow semantics, and log metadata controls. Current released apple/container log APIs expose runtime log streams but not per-service logging driver/option configuration. The local integration stack treats `--log-driver json-file` and `--log-driver local` as local capture aliases, adds disabled persisted capture through `--log-driver none`, parses local `--log-opt max-size=<size>` and `--log-opt max-file=<count>`, has writer-level local rotation when a runtime logging policy supplies max size and file count, and exposes static rotated raw and structured replay.
- **container-compose status:** Accepts file-backed `json-file` and `local` logging without options as no-op mappings to apple/container's local stdio log capture. On the local integration stack, `logging.driver: none` and legacy `log_driver: none` map to disabled persisted capture. Static `logs` and `logs --timestamps` requests include rotated files when apple/container supports them. Remote drivers, Compose logging options such as `max-size` and `max-file`, and rotated follow behavior remain rejected or unimplemented until container-compose maps the local options and apple/container exposes the required followed rotated read semantics.
...
Expected result: `container compose up api worker quiet` accepts the file-backed `json-file` and `local` logging drivers without options because apple/container already captures local stdio logs, and maps `none` to disabled persisted capture on the local integration stack. Static log replay includes rotated local raw and structured log files on the local integration stack. `container compose up rotated shipper` rejects before creating resources because container-compose has not yet mapped Compose `max-size`/`max-file` options and apple/container does not yet expose rotated follow semantics or remote logging driver primitives.

Status path:

- Docker Compose v2: accepts and normalizes service logging configuration.
- [`apple/container`][apple-container]: exposes local runtime log streams. The local integration stack accepts `--log-driver json-file` and `--log-driver local` as local capture aliases, adds a disabled persisted-capture policy through `--log-driver none`, parses `--log-opt max-size=<size>` and `--log-opt max-file=<count>`, has writer-level local rotation when a runtime policy supplies max size and file count, and exposes static rotated raw and structured replay. It still lacks rotated follow semantics, remote drivers, and driver-specific metadata controls.
- `container-compose`: accepts `json-file` and `local` without options as local log capture behavior, maps `none` to disabled persisted capture on the local integration stack, requests static rotated replay for raw and timestamped logs, then reports the remaining gap for Compose logging option mapping, rotated follow semantics, and remote drivers.
