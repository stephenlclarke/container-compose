...
| --- | --- | --- | --- |
| Replica scaling edge cases and local deploy handling | Standalone `scale`, scaled services that publish host ports, set fixed MAC addresses, use `container_name`, or use anonymous volumes, and `deploy` fields beyond local replica count and CPU/memory limits | Safe port-range allocation across replicas, per-replica MAC policy, per-replica anonymous volume naming, standalone scale command behavior, and a local interpretation of deploy mode/placement/update/rollback/endpoint/labels/restart/resources beyond local replica count and CPU/memory limits | [C1](#c1-plugin-gap-replica-scaling-edge-cases-and-deploy) |
...
| Present but blocked by `container-compose` design gaps | default stdin/signal-proxy `attach`, `watch`, standalone `scale`, `commit`, `publish` |
...
| [C1: Plugin Gap, Replica Scaling Edge Cases And Deploy](#c1-plugin-gap-replica-scaling-edge-cases-and-deploy) | `container-compose` gap | Standalone scale command, scaled published ports, per-replica anonymous volumes, DNS, and deploy semantics |
...
Expected result: `container compose up` accepts simple local replica counts for services that can be safely duplicated, but rejects this example because `update_config` still needs Compose deploy orchestration semantics beyond local replica count and CPU/memory limits. Scaled services are currently limited to Compose-managed names and services without published host ports, fixed MAC addresses, or anonymous volumes.
...
- `container-compose`: maps simple `scale`, `up --scale`, `create --scale`, and local `deploy.replicas` to indexed containers; maps `deploy.resources.limits.cpus` and `deploy.resources.limits.memory` to local runtime limits; and can target indexed service containers for `logs`, `attach`, `exec`, `cp`, `export`, and `port`. It still needs standalone `scale`, safe published-port allocation, per-replica anonymous volume handling, DNS semantics, and broader deploy semantics.

The equivalent supported CLI scaling form is:
