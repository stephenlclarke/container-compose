...
The log rows distinguish the local integration stack from the direction intended for [`apple/container`][apple-container]. The local [`logs-integration-chris` branch](https://github.com/stephenlclarke/container/tree/logs-integration-chris) is a proving branch built from Chris George's log retrieval direction, not a single upstream PR target. The intended runtime path is a sequence of small apple/container changes: keep retrieval filters separate from replay policy and presentation flags, add line-correct `tail`/`since`/`until` filtering after logical log-line reconstruction, add Docker-compatible timestamp parsing, add structured log records, add static rotated replay with bounded tail scans, add a bounded rotation-aware follow cursor or stream, then add local logging policy support for `json-file`, `local`, `none`, `max-size`, and `max-file`. The first structured-storage slice is the `logs-structured-record-storage` branch in the container fork; it adds the runtime `stdio.jsonl` JSON Lines sidecar format beside the existing raw `stdio.log` output. The next extracted slice is the `logs-structured-record-api` branch; it exposes `ContainerClient.logRecords(id:options:)`, `ContainerClient.logRecordFile(id:)`, `ContainerLogRecord`, XPC routes `containerLogRecords` and `containerLogRecordFile`, active raw bytes in `stdio.log`, active structured JSON Lines records in `stdio.jsonl`, and `ContainerLogOptions.tail/since/until` retrieval filters. The static rotated-tail slice is tracked by `ISSUE-logs-static-rotated-tail.md` and `PR-logs-static-rotated-tail.md` in the container fork. The local logging policy model slice is tracked by `ISSUE-logs-local-policy-model.md` and `PR-logs-local-policy-model.md`; the disabled local capture slice is tracked by `ISSUE-logs-disabled-local-capture.md` and `PR-logs-disabled-local-capture.md`; the local driver/options parser slice is tracked by `ISSUE-logs-local-driver-options.md` and `PR-logs-local-driver-options.md` in the container fork. These slices keep Compose-specific prefixing, service fan-out, and command formatting in this plugin.
