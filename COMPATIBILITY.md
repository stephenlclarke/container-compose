...
| Process options | Maps service command, entrypoint, working directory, user, TTY, stdin, read-only root filesystem, init, DNS, DNS search, Linux capabilities, memory limit, CPU limit, shared-memory size, and ulimits. | `container run` flags |
