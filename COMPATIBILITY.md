...
- **Missing apple/container primitive:** Released upstream `apple/container` still needs accepted health status/configuration/observer/create surfaces before explicit health support can leave the fork. Released upstream also needs accepted image config parsing for Dockerfile-inherited `HEALTHCHECK` metadata; the local fork exposes this through `ImageResource.Variant.healthCheck` in the image healthcheck metadata slice. Compose-compatible job lifecycle and scheduler semantics, first-class external config/secret lookup, ownership remapping for generated grants, and accepted restart policy create/runtime/timing support remain runtime gaps for released upstream use. Exit-code metadata for `service_completed_successfully` is implemented only on the local fork until [apple/container#1562](https://github.com/apple/container/pull/1562) or an equivalent upstream API lands.
- **container-compose status:** Explicit service healthchecks, Dockerfile-inherited image healthchecks, timing-only Compose healthcheck overrides over image defaults, `depends_on.condition: service_healthy`, `depends_on.condition: service_completed_successfully`, service-level `restart`, the fork-backed subset of `deploy.restart_policy`, and Docker Compose local runtime config/secret file sources are supported on the integration branch. Supported deploy restart values are `condition: none`, `condition: any` or an empty policy, `condition: on-failure` with optional `max_attempts`, plus `delay` and `window` when the fork's restart-policy timing slice is present. Timing-only healthcheck overrides reject before resources are created when the image does not expose Dockerfile `HEALTHCHECK` metadata. Generated config/secret grant `mode` values are honored with writable bits ignored; file-backed grant metadata follows Docker Compose bind-mount behavior. Job modes, external configs/secrets, and strict config/secret `uid`/`gid` ownership requests are still rejected before resources are created.
...
Expected result: on the fork-backed integration branch, explicit service `healthcheck` definitions, Dockerfile-inherited image healthchecks, timing-only healthcheck overrides over image defaults, `depends_on.condition: service_healthy`, service-level `restart`, the `deploy.restart_policy` condition/max-attempt/timing subset, file-backed configs/secrets, inline config content, environment-backed configs, environment-backed secrets, and generated config/secret grant modes are accepted when the runtime health, image metadata, and restart slices are present. `container compose up` still rejects this complete file because [`apple/container`][apple-container] needs job lifecycle/scheduler semantics and first-class external config/secret lookup. File-backed service configs/secrets are supported in [S1](#s1-supported-local-web-stack), inline and environment-backed runtime grants are supported by plugin-side local materialization, `service_completed_successfully` is supported on the fork-backed integration branch through [apple/container#1562](https://github.com/apple/container/pull/1562), explicit health support depends on the fork's health status/configuration/observer/CLI handoff slices, Dockerfile-inherited healthchecks depend on the fork's image healthcheck metadata slice, and service restart depends on the fork's restart-policy create/runtime/timing slices.
...
- [`apple/container`][apple-container]: the local fork can run configured healthchecks, publish `ContainerSnapshot.health`, expose image `HEALTHCHECK` metadata through `ImageResource.Variant.healthCheck`, parse service restart create options, restart containers from local policy, and apply restart delay/window timing; released upstream does not yet expose that full surface. Job lifecycle/scheduler semantics and first-class external config/secret lookup remain gaps; exit metadata is available only on the local fork until #1562 or an equivalent API lands.
- `container-compose`: maps explicit service healthchecks, Dockerfile-inherited image healthchecks, timing-only healthcheck overrides, `service_healthy`, `service_completed_successfully`, service-level `restart`, and `deploy.restart_policy` condition/max-attempt/timing values on the fork-backed branch. It materializes Docker Compose local runtime config/secret sources that can be expressed as files, applies generated grant modes, then detects the remaining fields before reporting the relevant apple/container runtime gap.
