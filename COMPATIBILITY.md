...
- **container-compose status:** Supported for the listed direct API paths on the local integration stack. Static raw/timestamped log replay requests can include rotated local log files where apple/container exposes them. Static timestamped and time-window rendering delegates `tail`, `since`, and `until` to the direct apple/container structured record API, then renders the returned line-correct records locally. Followed structured streams use the active `ContainerClient.logRecordFile(id:)` JSONL file with a bounded cursor and use direct runtime status to flush an unterminated final line once the target container is no longer live. Raw rotated follow still uses plugin-side merged replay polling in this plugin, but the fork now has the runtime `ContainerClient.followLogs(id:options:)` stream needed to replace that workaround. Time filters accept RFC 3339 timestamps, RFC 3339 nano timestamps, Unix timestamps in seconds with optional fractional seconds, and relative durations. Line-boundary tests cover empty logs, blank records, final newlines, CRLF/CR separators, prefixed blank records, split structured records, and unterminated final records. `make docker-log-fixtures` captures optional Docker Compose parity evidence for rotated `json-file` and `local` `logs --tail` behavior without adding Docker to the required CI path. Released timestamped follow, filtered log follow, static rotated replay, raw rotation-aware follow, and byte-preserving structured log support wait for upstream apple/container acceptance of the #1592/#1764/#1765 retrieval/parser stack and subsequent structured record/logging policy APIs; structured/timestamped rotated follow still needs an upstream cursor or stream design. Released structured stdout/stderr log identity semantics and runtime process/event controls remain apple/container gaps or compatibility decisions.
- **Example:** [S1](#s1-supported-local-web-stack).
...
- `container-compose`: accepts `json-file` and `local` as local log capture behavior, maps `none` to disabled persisted capture on the local integration stack, maps local `max-size`/`max-file` options to runtime `--log-opt` flags, requests static rotated replay for raw and timestamped logs, follows active structured record files, keeps plugin-side raw rotation-aware follow as an experimental validation path until it is retargeted to `ContainerClient.followLogs(id:options:)`, and carries an optional Docker Compose fixture for rotated `logs --tail` parity, then reports the remaining gap for remote drivers and runtime cursor/filter acceptance.
