...
- **Status:** <img alt="PARTIAL" src="https://img.shields.io/badge/PARTIAL-B26A00?style=flat-square">
- **Compose surface:**
  - Discovery and output: `ps`, filtered `ps`, prefixed `logs` across selected service replicas, colored log prefixes when ANSI policy allows them, concurrent `logs --follow`, indexed `logs`, fixture-covered blank-line/trailing-newline handling, non-UTF-8-preserving log replay/follow, static and followed `logs --timestamps`, `logs --no-color`, `logs --no-log-prefix`, static and followed `logs --since` and `logs --until` with RFC 3339, Unix timestamp, and relative-duration filters, `events --json [SERVICE...]`, `events --json --since/--until [SERVICE...]`, output-only `attach --no-stdin --sig-proxy=false`, and indexed attach.
...
- **Missing apple/container primitive:** Released upstream apple/container still needs accepted process listing, event streaming, pause/unpause lifecycle controls, stored process exit codes after container stop, and copy follow-link/archive controls.
- **container-compose status:** On the local integration stack, `pause` and `unpause` use direct `ContainerClient.pause(id:)` and `ContainerClient.unpause(id:)` calls from [`stephenlclarke/container` `logs-integration-chris`](https://github.com/stephenlclarke/container/tree/logs-integration-chris), backed by [`stephenlclarke/containerization` `integration/blkio-runtime`](https://github.com/stephenlclarke/containerization/tree/integration/blkio-runtime). `container compose cp --follow-link` and `container compose cp --archive` now map to fork-backed direct copy APIs. `top` now maps to the fork-backed PID-only `ContainerClient.processes(id:)` surface from container commit `14a3067`. The first Apple-shaped event primitive now exists in the same container fork as code commits `b71e4bb323e3` and `0da7890b2632`, adding `ContainerEvent`, `ContainerClient.events()`, lifecycle event emission, non-blocking event subscribers, and `container events` JSON Lines output without Compose filtering. `container compose events --json [SERVICE...]` now maps that stream in this repository as commit `113be38063ea`: it filters to Compose project/service labels, skips one-off containers, applies selected-service arguments, strips Compose-private attributes, and renders Docker Compose compatible JSON Lines. The optional local-only `make docker-compose-events-parity` check passed on 2026-06-22 against Docker Compose 5.1.4, validating the Docker behavior this mapping mirrors without adding Docker to CI. Non-JSON output plus `--since` and `--until` remain future plugin/runtime slices. Branches pinned to released upstream apple/container must continue treating event support as runtime-gated until an equivalent event-stream API is accepted.
