...
- [`apple/container`][apple-container]: exposes local runtime log streams. The local integration stack accepts `--log-driver json-file` and `--log-driver local` as local capture aliases, adds a disabled persisted-capture policy through `--log-driver none`, parses `--log-opt max-size=<size>` and `--log-opt max-file=<count>`, has writer-level local rotation when a runtime policy supplies max size and file count, and exposes static rotated raw and structured replay. It still lacks remote drivers, driver-specific metadata controls, and an accepted rotation-aware follow cursor or stream.
