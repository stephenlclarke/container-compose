...
- **Optional Docker parity:** `make docker-compose-restart-policy-parity` validates Docker Compose V2 `HostConfig.RestartPolicy` behavior for service `restart`, deploy-over-service precedence, deploy `condition: any`, deploy `condition: none`, and `on-failure:0`. This target is local-only and intentionally outside CI.
