...
Expected result: `container compose config` preserves this metadata. Runtime commands do not publish `expose`, do not act on `x-*`, and reject service-level model bindings or config/secret grants only when they need unsupported runtime behavior such as a model runner, external config/secret lookup, or strict ownership/mode materialization.
