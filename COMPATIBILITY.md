...
- **apple/container path:** Direct `ContainerClient` list/get/logs/logRecords/logRecordFile/copy/export/stats APIs, `ProcessIO`, `ContainerClient.createProcess`, and `ClientProcess.start`. Static `logs --since`, `logs --until`, and `logs --timestamps` use the local [`logs-integration-chris`](https://github.com/stephenlclarke/container/tree/logs-integration-chris) log-options and structured log-record API work, which starts from Chris George's `ContainerLogOptions(since:timestamps:)` shape; the local apple/container CLI accepts RFC 3339 and Unix timestamp filters, renders static `container logs --timestamps`, renders followed `container logs --follow --timestamps/--since/--until` from structured records, exits raw `container logs --follow` when direct container status shows the target is no longer live, flushes final unterminated followed structured records at the same stop boundary, and exposes static plus followed rotated raw and structured replay through `ContainerLogOptions`.
- **container-compose status:** Supported for the listed direct API paths on the local integration stack. Static and followed raw/timestamped log replay requests include rotated local log files where apple/container exposes them; followed streams poll merged replay snapshots and emit only newly appended raw bytes or structured records. Raw and structured followed streams use direct runtime status to flush an unterminated final line once the target container is no longer live. Time filters accept RFC 3339 timestamps, Unix timestamps in seconds with optional fractional seconds, and relative durations. Line-boundary tests cover empty logs, blank records, final newlines, CRLF/CR separators, prefixed blank records, and unterminated final records. Released timestamped follow, filtered log follow, rotated replay, and byte-preserving structured log support wait for upstream apple/container acceptance of the structured record and rotated merged-replay API shape. Released structured stdout/stderr log identity semantics and runtime process/event controls remain apple/container gaps or compatibility decisions.
- **Example:** [S1](#s1-supported-local-web-stack).
...
- **Missing apple/container primitive:** Compose-compatible remote service logging drivers and log metadata controls. Current released apple/container log APIs expose runtime log streams but not per-service logging driver/option configuration. The local integration stack treats `--log-driver json-file` and `--log-driver local` as local capture aliases, adds disabled persisted capture through `--log-driver none`, parses local `--log-opt max-size=<size>` and `--log-opt max-file=<count>`, has writer-level local rotation when a runtime logging policy supplies max size and file count, and exposes static plus followed rotated raw and structured replay.
- **container-compose status:** Accepts file-backed `json-file` and `local` logging as mappings to apple/container's local stdio log capture. On the local integration stack, `logging.driver: none` and legacy `log_driver: none` map to disabled persisted capture. Local `logging.options` and legacy `log_opt` values for `max-size` and `max-file` map to apple/container `--log-opt` flags for the default, `json-file`, and `local` drivers. Static and followed `logs` / `logs --timestamps` requests include rotated files when apple/container supports them. Remote drivers, non-local logging options, and options attached to `none` remain rejected until apple/container exposes the required runtime primitives.
...
Expected result: `container compose up api worker quiet rotated` accepts the file-backed `json-file` and `local` logging drivers, maps `none` to disabled persisted capture on the local integration stack, and maps `max-size`/`max-file` to apple/container local `--log-opt` flags. Static and followed log replay include rotated local raw and structured log files on the local integration stack. `container compose up shipper` rejects before creating resources because apple/container does not yet expose remote logging driver primitives.

Status path:

- Docker Compose v2: accepts and normalizes service logging configuration.
- [`apple/container`][apple-container]: exposes local runtime log streams. The local integration stack accepts `--log-driver json-file` and `--log-driver local` as local capture aliases, adds a disabled persisted-capture policy through `--log-driver none`, parses `--log-opt max-size=<size>` and `--log-opt max-file=<count>`, has writer-level local rotation when a runtime policy supplies max size and file count, and exposes static plus followed rotated raw and structured replay. It still lacks remote drivers and driver-specific metadata controls.
- `container-compose`: accepts `json-file` and `local` as local log capture behavior, maps `none` to disabled persisted capture on the local integration stack, maps local `max-size`/`max-file` options to runtime `--log-opt` flags, requests static and followed rotated replay for raw and timestamped logs, then reports the remaining gap for remote drivers.
