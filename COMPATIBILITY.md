...
2. <img alt="FORK MERGE" src="https://img.shields.io/badge/FORK%20MERGE-2563EB?style=flat-square"> Keep the #1562 exit-metadata cherry-pick and healthcheck handoff slices in the fork integration branch, then continue testing stopped-container `wait`, `service_completed_successfully`, explicit service `healthcheck`, `service_healthy`, and failed one-shot jobs there. Next, merge or reimplement the restart PR set for `restart` and `deploy.restart_policy`.
...
- **Missing apple/container primitive:** Released upstream `apple/container` still needs accepted health status/configuration/observer/create surfaces before explicit health support can leave the fork. Dockerfile-inherited `HEALTHCHECK` metadata is still missing from image config parsing. Compose-compatible job lifecycle and scheduler semantics, first-class config/secret store or materialization primitives, ownership/mode controls for non-file-backed grants, and restart policy support remain runtime gaps. Exit-code metadata for `service_completed_successfully` is implemented only on the local fork until [apple/container#1562](https://github.com/apple/container/pull/1562) or an equivalent upstream API lands.
- **container-compose status:** Explicit service healthchecks and `depends_on.condition: service_healthy` are supported on the fork-backed integration branch. Dockerfile-inherited healthchecks without an explicit Compose `healthcheck.test`, job modes, non-file-backed config/secrets, and restart policies are rejected before resources are created. `depends_on.condition: service_completed_successfully` is also supported on the fork-backed integration branch.
...
Expected result: on the fork-backed integration branch, explicit service `healthcheck` definitions and `depends_on.condition: service_healthy` are accepted when the runtime health slices are present. `container compose up` still rejects this complete file because [`apple/container`][apple-container] needs job lifecycle/scheduler semantics, first-class config/secret stores or materialization, and restart policies for both service `restart` and `deploy.restart_policy`. File-backed service configs/secrets are supported in [S1](#s1-supported-local-web-stack), `service_completed_successfully` is supported on the fork-backed integration branch through [apple/container#1562](https://github.com/apple/container/pull/1562), and explicit health support depends on the fork's health status/configuration/observer/CLI handoff slices. Dockerfile-inherited healthchecks remain blocked until [`apple/container`][apple-container] exposes image `HEALTHCHECK` metadata.
...
- [`apple/container`][apple-container]: the local fork can run configured healthchecks and publish `ContainerSnapshot.health`; released upstream does not yet expose that full surface. Dockerfile-inherited `HEALTHCHECK` metadata, job lifecycle/scheduler semantics, first-class config/secret store or materialization primitives, and restart policy support remain gaps; exit metadata is available only on the local fork until #1562 or an equivalent API lands.
- `container-compose`: maps explicit service healthchecks and `service_healthy` on the fork-backed branch, maps `service_completed_successfully` on the fork-backed branch, and detects the remaining fields before reporting the apple/container runtime gap.
