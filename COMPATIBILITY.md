...
Expected result: on the fork-backed integration branch, explicit service `healthcheck` definitions, `depends_on.condition: service_healthy`, service-level `restart`, and the `deploy.restart_policy` condition/max-attempt/timing subset are accepted when the runtime health and restart slices are present. `container compose up` still rejects this complete file because [`apple/container`][apple-container] needs job lifecycle/scheduler semantics and first-class config/secret stores or materialization. File-backed service configs/secrets are supported in [S1](#s1-supported-local-web-stack), `service_completed_successfully` is supported on the fork-backed integration branch through [apple/container#1562](https://github.com/apple/container/pull/1562), explicit health support depends on the fork's health status/configuration/observer/CLI handoff slices, and service restart depends on the fork's restart-policy create/runtime/timing slices. Dockerfile-inherited healthchecks remain blocked until [`apple/container`][apple-container] exposes image `HEALTHCHECK` metadata.
...
- [`apple/container`][apple-container]: the local fork can run configured healthchecks, publish `ContainerSnapshot.health`, parse service restart create options, restart containers from local policy, and apply restart delay/window timing; released upstream does not yet expose that full surface. Dockerfile-inherited `HEALTHCHECK` metadata, job lifecycle/scheduler semantics, and first-class config/secret store or materialization primitives remain gaps; exit metadata is available only on the local fork until #1562 or an equivalent API lands.
- `container-compose`: maps explicit service healthchecks, `service_healthy`, `service_completed_successfully`, service-level `restart`, and `deploy.restart_policy` condition/max-attempt/timing values on the fork-backed branch, then detects the remaining fields before reporting the relevant apple/container runtime gap.
