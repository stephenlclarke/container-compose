...
- **Host entries, DNS, and network identity:** <img alt="FORK MERGE" src="https://img.shields.io/badge/FORK%20MERGE-2563EB?style=flat-square"> [apple/container#1563](https://github.com/apple/container/pull/1563) and [apple/container#1340](https://github.com/apple/container/pull/1340) directly map to Compose `extra_hosts`; the local fork now combines those directions through `ISSUE-host-entries.md` / `PR-host-entries.md`, and `container-compose` maps static IP-literal `extra_hosts` through `ISSUE-extra-hosts.md` / `PR-extra-hosts.md`. The fork also adds explicit `--hostname` support through `ISSUE-hostname.md` / `PR-hostname.md`, with Compose mapping tracked in `ISSUE-service-hostname.md` / `PR-service-hostname.md`. [apple/container#1740](https://github.com/apple/container/pull/1740) helps deterministic address behavior across stop/start. [apple/container#1614](https://github.com/apple/container/pull/1614) helps service DNS configuration. [apple/container#1463](https://github.com/apple/container/pull/1463), [apple/container#1622](https://github.com/apple/container/pull/1622), and [apple/container#1661](https://github.com/apple/container/pull/1661) are competing or complementary bridge-networking directions; test only one coherent bridge direction in the fork after comparing API shape.
...
3. <img alt="FORK MERGE" src="https://img.shields.io/badge/FORK%20MERGE-2563EB?style=flat-square"> Continue testing static `extra_hosts`, explicit host entries, explicit hostnames, sticky IPs, DNS defaults, `host-gateway`, and one chosen bridge-networking direction together before exposing more networking fields in `container-compose`.
...
- **Missing apple/container primitive:** Released upstream still needs accepted explicit host entries, explicit hostname controls, domain-name controls, Docker `host-gateway` gateway resolution, and legacy link/alias semantics. The local fork now exposes static host-entry injection through `ContainerConfiguration.HostEntry`, `container run/create --add-host`, and explicit hostnames through `container run/create --hostname`.
- **container-compose status:** Static IP-literal `extra_hosts` entries are supported on the fork-backed integration branch and map to `--add-host`. Compose service `hostname` is supported on the fork-backed integration branch and maps to `--hostname`. `host-gateway`, `domainname`, `links`, and `external_links` still reject before resources are created.
...
- <img alt="PARTIAL" src="https://img.shields.io/badge/PARTIAL-B26A00?style=flat-square"> [A2: Partial, Host Identity And Links](#a2-apple-gap-host-identity-and-links): Demonstrates fork-backed static host entries and service hostname mapping plus remaining domain name, `host-gateway`, and legacy link gaps.
...
Expected result: on the fork-backed integration branch, static IP-literal `extra_hosts` entries are accepted and mapped to `container run/create --add-host`, and service `hostname` is accepted and mapped to `container run/create --hostname`. This complete example still rejects because [`apple/container`][apple-container] needs domain-name controls, Docker `host-gateway` gateway resolution, and legacy link semantics.

Status path:

- Docker Compose v2: accepts and normalizes host identity, static host entries, `host-gateway`, and legacy link fields.
- [`apple/container`][apple-container]: the local fork supports static host-entry injection through `ContainerConfiguration.HostEntry` and `--add-host`, plus explicit container hostnames through `--hostname`; released upstream still needs accepted host-entry and hostname support, domain-name controls, gateway resolution for `host-gateway`, and legacy link semantics.
- `container-compose`: maps static IP-literal `extra_hosts` to `--add-host` and service `hostname` to `--hostname` on the fork-backed integration branch, then reports the remaining apple/container runtime gaps before resources are created.
