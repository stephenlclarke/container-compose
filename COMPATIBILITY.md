...
The log rows distinguish the local integration stack from the direction intended for [`apple/container`][apple-container]. The local [`logs-integration-chris` branch](https://github.com/stephenlclarke/container/tree/logs-integration-chris) is a proving branch built on Chris George's `ContainerLogOptions(since:timestamps:)` direction, not a single upstream PR target. The intended runtime path is a sequence of small apple/container changes: keep the Chris-compatible options shape, add line-correct `tail`/`since`/`until` filtering after logical log-line reconstruction, add Docker-compatible timestamp parsing, add structured log records, add static rotated replay, add a bounded rotation-aware follow cursor or stream, then add local logging policy support for `json-file`, `local`, `none`, `max-size`, and `max-file`. This repository should update the compatibility rows as each runtime primitive becomes accepted upstream and should keep Compose-specific prefixing, service fan-out, and command formatting in the plugin.
...
- **apple/container path:** Direct `ContainerClient` list/get/logs/logRecords/logRecordFile/copy/export/stats APIs, `ProcessIO`, `ContainerClient.createProcess`, and `ClientProcess.start`. Static `logs --since`, `logs --until`, and `logs --timestamps` use the local [`logs-integration-chris`](https://github.com/stephenlclarke/container/tree/logs-integration-chris) log-options and structured log-record API work, which starts from Chris George's `ContainerLogOptions(since:timestamps:)` shape; the local apple/container CLI accepts RFC 3339 and Unix timestamp filters, renders static `container logs --timestamps`, renders followed `container logs --follow --timestamps/--since/--until` from active structured record files, renders raw follow from active raw log handles, and exposes static rotated raw and structured replay through `ContainerLogOptions`. Upstream adoption should split that branch into small PRs and make `tail`, `since`, and `until` line-correct after logical log-line reconstruction.
- **container-compose status:** Supported for the listed direct API paths on the local integration stack. Static raw/timestamped log replay requests can include rotated local log files where apple/container exposes them. Static timestamped and time-window rendering delegates `tail`, `since`, and `until` to the direct apple/container structured record API, then renders the returned records locally. That is local scaffolding for the intended runtime contract; strict Docker parity depends on apple/container filtering reconstructed logical lines, not storage fragments. Followed streams currently use plugin-side merged replay polling to validate rotation-aware behavior and use direct runtime status to flush an unterminated final line once the target container is no longer live. Time filters accept RFC 3339 timestamps, Unix timestamps in seconds with optional fractional seconds, and relative durations. Line-boundary tests cover empty logs, blank records, final newlines, CRLF/CR separators, prefixed blank records, split structured records, and unterminated final records. Released timestamped follow, filtered log follow, static rotated replay, and byte-preserving structured log support wait for upstream apple/container acceptance of the structured record APIs; rotation-aware follow still needs an upstream cursor or stream design. Released structured stdout/stderr log identity semantics and runtime process/event controls remain apple/container gaps or compatibility decisions.
- **Example:** [S1](#s1-supported-local-web-stack).
...
- [`apple/container`][apple-container]: exposes local runtime log streams. The local integration stack accepts `--log-driver json-file` and `--log-driver local` as local capture aliases, adds a disabled persisted-capture policy through `--log-driver none`, parses `--log-opt max-size=<size>` and `--log-opt max-file=<count>`, has writer-level local rotation when a runtime policy supplies max size and file count, stores structured records at Docker-like line boundaries, and exposes static rotated raw and structured replay. It still lacks remote drivers, driver-specific metadata controls, accepted line-correct replay filtering across every log source, and an accepted rotation-aware follow cursor or stream.
- `container-compose`: accepts `json-file` and `local` as local log capture behavior, maps `none` to disabled persisted capture on the local integration stack, maps local `max-size`/`max-file` options to runtime `--log-opt` flags, requests static rotated replay for raw and timestamped logs, keeps plugin-side rotation-aware follow as an experimental validation path, then reports the remaining gap for remote drivers and runtime cursor/filter acceptance.
