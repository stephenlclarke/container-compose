...
The log rows distinguish the local integration stack from the direction intended for [`apple/container`][apple-container]. The local [`logs-integration-chris` branch](https://github.com/stephenlclarke/container/tree/logs-integration-chris) is a proving branch built from Chris George's log retrieval direction, not a single upstream PR target. The intended runtime path is a sequence of small apple/container changes: keep retrieval filters separate from replay policy and presentation flags, add line-correct `tail`/`since`/`until` filtering after logical log-line reconstruction, add Docker-compatible timestamp parsing, add structured log records, add static rotated replay, add a bounded rotation-aware follow cursor or stream, then add local logging policy support for `json-file`, `local`, `none`, `max-size`, and `max-file`. The first structured-storage slice is the `logs-structured-record-storage` branch in the container fork; it adds the runtime `stdio.jsonl` JSON Lines sidecar format beside the existing raw `stdio.log` output. This repository should update the compatibility rows as each runtime primitive becomes accepted upstream and should keep Compose-specific prefixing, service fan-out, and command formatting in the plugin.
...
- **Logs:** <img alt="FORK MERGE" src="https://img.shields.io/badge/FORK%20MERGE-2563EB?style=flat-square"> [apple/container#1592](https://github.com/apple/container/pull/1592), [apple/container#1764](https://github.com/apple/container/pull/1764), and [apple/container#1765](https://github.com/apple/container/pull/1765) are the first log-runtime stack for Compose `logs --tail`, `logs --since`, `logs --until`, timestamp parsing, and a direct client-side filtering contract. The [`logs-structured-record-storage`](https://github.com/stephenlclarke/container/tree/logs-structured-record-storage) fork branch is the next small upstream slice for the `stdio.jsonl` record format needed by timestamped structured replay. [apple/container#1758](https://github.com/apple/container/pull/1758) is low-risk log handler hygiene that should stay with the log stack but is not itself a Compose feature.
