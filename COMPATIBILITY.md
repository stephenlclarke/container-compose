...
| --- | --- | --- | --- |
| Replica scaling edge cases and local deploy handling | Scaled services that publish host ports, set fixed MAC addresses, use `container_name`, or use anonymous volumes, and `deploy` fields beyond local replica count and CPU/memory limits | Safe port-range allocation across replicas, per-replica MAC policy, per-replica anonymous volume naming, and a local interpretation of deploy mode/placement/update/rollback/endpoint/labels/restart/resources beyond local replica count and CPU/memory limits | [C1](#c1-plugin-gap-replica-scaling-edge-cases-and-deploy) |
...
| [C1: Plugin Gap, Replica Scaling Edge Cases And Deploy](#c1-plugin-gap-replica-scaling-edge-cases-and-deploy) | `container-compose` gap | Scaled published ports, per-replica anonymous volumes, DNS, and deploy semantics |
...
- `container-compose`: maps standalone `scale`, `up --scale`, `create --scale`, service `scale`, and local `deploy.replicas` to indexed containers; maps `deploy.resources.limits.cpus` and `deploy.resources.limits.memory` to local runtime limits; and can target indexed service containers for `logs`, `attach`, `exec`, `cp`, `export`, and `port`. It still needs safe published-port allocation, per-replica anonymous volume handling, DNS semantics, and broader deploy semantics.
