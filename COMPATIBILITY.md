...
| Metadata, logging, storage shortcuts | `annotations`, `logging`, `log_driver`, `log_opt`, `storage_opt`, `volumes_from`, service-level `volume_driver`, advanced service volume options such as bind propagation/SELinux/recursive controls, volume labels/nocopy/subpath, image mounts, and mount consistency | Runtime mapping, inherited mount behavior, logging behavior, storage option and advanced mount policy | [C4](#c4-plugin-gap-metadata-storage-api-socket-and-pull-policy) |
| API socket, block I/O, build-trigger pull policy | `use_api_socket`, `blkio_config`, service `pull_policy: build` | Security review, resource-control mapping, and build-trigger pull semantics | [C4](#c4-plugin-gap-metadata-storage-api-socket-and-pull-policy) |
...
| [C4: Plugin Gap, Metadata, Storage, API Socket, And Pull Policy](#c4-plugin-gap-metadata-storage-api-socket-and-pull-policy) | `container-compose` gap | Annotations, logging options, inherited mounts, advanced service volume options, API socket, block I/O, and build-trigger pull policy |
...
### C4: Plugin Gap, Metadata, Storage, API Socket, And Pull Policy

Expected result: `container compose up` rejects this because annotations, logging/storage options, inherited mounts, advanced service volume options, API socket exposure, block I/O controls, and build-trigger pull policy need plugin implementation and security review.
...
- `container-compose`: maps service `pull_policy: daily`, `weekly`, and `every_<duration>` through direct image pulls and local pull timestamp metadata, and maps long-form tmpfs `size`/`mode` through Apple `container --mount type=tmpfs`. It still needs runtime mapping, inherited mount behavior, advanced volume option policy beyond tmpfs size/mode, logging/storage policy, API socket security review, block I/O handling, and build-trigger pull semantics.
