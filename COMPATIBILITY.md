...
- **apple/container path:** Direct `ContainerClient` list/get/logs/logRecords/logRecordFile/followLogs/copy/export/stats APIs, `ProcessIO`, `ContainerClient.createProcess`, and `ClientProcess.start`. Static `logs --since`, `logs --until`, and `logs --timestamps` use the local [`logs-integration-chris`](https://github.com/stephenlclarke/container/tree/logs-integration-chris) log-options and structured log-record API work, which keeps `ContainerLogOptions` focused on retrieval filters and `ContainerLogReplayOptions` focused on replay policy; the local apple/container CLI accepts RFC 3339, RFC 3339 nano, Unix timestamp, and relative-duration filters, renders static `container logs --timestamps`, renders followed `container logs --follow --timestamps/--since/--until` from active structured record files, exposes static rotated raw and structured replay through replay options, and exposes raw rotation-aware follow through `ContainerClient.followLogs(id:options:)`. The first extracted structured-storage slice is [`logs-structured-record-storage`](https://github.com/stephenlclarke/container/tree/logs-structured-record-storage), which stores active runtime records in `stdio.jsonl` as ISO 8601 timestamp, stdout/stderr stream, and base64 payload fields beside the existing raw `stdio.log`. The extracted active-retrieval slice is [`logs-structured-record-api`](https://github.com/stephenlclarke/container/tree/logs-structured-record-api), which exposes `ContainerClient.logRecords(id:options:)`, `ContainerClient.logRecordFile(id:)`, `ContainerLogRecord`, XPC routes `containerLogRecords` and `containerLogRecordFile`, and `ContainerLogOptions.tail/since/until` retrieval filters. The raw follow handoff is `ISSUE-logs-rotation-aware-follow.md` / `PR-logs-rotation-aware-follow.md` in the container fork. The open upstream path starts with Chris George's [apple/container#1592](https://github.com/apple/container/pull/1592), then the ready-for-review [apple/container#1764](https://github.com/apple/container/pull/1764) and [apple/container#1765](https://github.com/apple/container/pull/1765) follow-ups, then structured-storage, retrieval, rotated replay, logging-policy, writer-rotation, and raw-follow PRs. This repository must wait for accepted upstream APIs before treating those local integration capabilities as released apple/container support.
- **container-compose status:** Supported for the listed direct API paths on the local integration stack. Static raw/timestamped log replay requests can include rotated local log files where apple/container exposes them. Static timestamped and time-window rendering delegates `tail`, `since`, and `until` to the direct apple/container structured record API, then renders the returned line-correct records locally. Followed structured streams use the active `ContainerClient.logRecordFile(id:)` JSONL file with a bounded cursor and use direct runtime status to flush an unterminated final line once the target container is no longer live. Raw rotated follow calls the fork's runtime `ContainerClient.followLogs(id:options:)` stream, then keeps only Compose line reconstruction, container-stop flushing, prefixes, colors, and service fan-out in this plugin. Time filters accept RFC 3339 timestamps, RFC 3339 nano timestamps, Unix timestamps in seconds with optional fractional seconds, and relative durations. Line-boundary tests cover empty logs, blank records, final newlines, CRLF/CR separators, prefixed blank records, split structured records, followed raw stream chunks, and unterminated final records. `make docker-log-fixtures` captures optional Docker Compose parity evidence for rotated `json-file` and `local` `logs --tail` behavior without adding Docker to the required CI path. Released timestamped follow, filtered log follow, static rotated replay, raw rotation-aware follow, and byte-preserving structured log support wait for upstream apple/container acceptance of the #1592/#1764/#1765 retrieval/parser stack and subsequent structured record/logging policy APIs; structured/timestamped rotated follow still needs an upstream cursor or stream design. Released structured stdout/stderr log identity semantics and runtime process/event controls remain apple/container gaps or compatibility decisions.
- **Example:** [S1](#s1-supported-local-web-stack).
...
- [`apple/container`][apple-container]: exposes local runtime log streams. The local integration stack accepts `--log-driver json-file` and `--log-driver local` as local capture aliases, adds a disabled persisted-capture policy through `--log-driver none`, parses `--log-opt max-size=<size>` and `--log-opt max-file=<count>`, has writer-level local rotation when a runtime policy supplies max size and file count, stores structured records at Docker-like line boundaries, exposes line-correct static rotated raw and structured replay, and exposes raw rotation-aware follow through `ContainerClient.followLogs(id:options:)`. It still lacks remote drivers, driver-specific metadata controls, upstream-accepted replay/filter contracts, and structured/timestamped rotation-aware follow over retained `stdio.jsonl`.
- `container-compose`: accepts `json-file` and `local` as local log capture behavior, maps `none` to disabled persisted capture on the local integration stack, maps local `max-size`/`max-file` options to runtime `--log-opt` flags, requests static rotated replay for raw and timestamped logs, follows active structured record files, uses `ContainerClient.followLogs(id:options:)` for raw rotation-aware follow on the local integration stack, and carries an optional Docker Compose fixture for rotated `logs --tail` parity, then reports the remaining gap for remote drivers and upstream runtime cursor/filter acceptance.
