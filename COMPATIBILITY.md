...
- **Health, completion, and restart:** <img alt="FORK MERGE" src="https://img.shields.io/badge/FORK%20MERGE-2563EB?style=flat-square"> [apple/container#1562](https://github.com/apple/container/pull/1562) is now cherry-picked into the local `logs-integration-chris` fork branch as `feat(api): expose container exit metadata`, and `container-compose` uses that fork-only `ContainerSnapshot.exitCode`/`exitedDate` surface for stopped-container `wait` replay and `depends_on.condition: service_completed_successfully`. [apple/container#1503](https://github.com/apple/container/pull/1503), [apple/container#1504](https://github.com/apple/container/pull/1504), and [apple/container#1258](https://github.com/apple/container/pull/1258) remain tracked for health status and restart-policy work. Upstream release branches must still treat this as pending until #1562 or an equivalent exit-metadata API is accepted.
...
2. <img alt="FORK MERGE" src="https://img.shields.io/badge/FORK%20MERGE-2563EB?style=flat-square"> Keep the #1562 exit-metadata cherry-pick in the fork integration branch and continue testing stopped-container `wait`, `service_completed_successfully`, and failed one-shot jobs there. Next, merge or reimplement the health and restart PR set for `service_healthy`, `restart`, and `deploy.restart_policy`.
...
- **Status:** <img alt="APPLE GAP" src="https://img.shields.io/badge/APPLE%20GAP-C62828?style=flat-square">
- **Compose surface:** `healthcheck`, `depends_on.condition: service_healthy`, `deploy.mode: replicated-job`, `deploy.mode: global-job`, external/content/environment-backed service `configs` and `secrets`, service `restart`, and `deploy.restart_policy`.
- **Missing apple/container primitive:** Health status, Compose-compatible job lifecycle and scheduler semantics, first-class config/secret store or materialization primitives, ownership/mode controls for non-file-backed grants, and restart policy support. Exit-code metadata for `service_completed_successfully` is implemented only on the local fork until [apple/container#1562](https://github.com/apple/container/pull/1562) or an equivalent upstream API lands.
- **container-compose status:** Health, job modes, non-file-backed config/secrets, and restart policies are rejected before resources are created. `depends_on.condition: service_completed_successfully` is supported on the fork-backed integration branch.
...
- <img alt="APPLE GAP" src="https://img.shields.io/badge/APPLE%20GAP-C62828?style=flat-square"> [A4: Apple Gap, Health, Config And Secret Stores, And Restart](#a4-apple-gap-health-config-and-secret-stores-and-restart): Demonstrates healthchecks, healthy dependency gates, deploy job modes, external/content/environment-backed service secrets/configs, and restart policies. The `service_completed_successfully` gate in that example is supported on the fork-backed branch, but the file still rejects because of the other apple/container gaps.
...
Expected result: `container compose up` rejects this because [`apple/container`][apple-container] needs health status, job lifecycle/scheduler semantics, first-class config/secret stores or materialization, and restart policies for both service `restart` and `deploy.restart_policy`. File-backed service configs/secrets are supported in [S1](#s1-supported-local-web-stack), and `service_completed_successfully` is supported on the fork-backed integration branch through [apple/container#1562](https://github.com/apple/container/pull/1562). This example uses inline and external definitions to show the remaining runtime store gap.
...
- [`apple/container`][apple-container]: missing health status, job lifecycle/scheduler semantics, first-class config/secret store or materialization primitives, and restart policy support; exit metadata is available only on the local fork until #1562 or an equivalent API lands.
- `container-compose`: maps `service_completed_successfully` on the fork-backed branch and detects the remaining fields before reporting the apple/container runtime gap.
