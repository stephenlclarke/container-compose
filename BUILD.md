...
| `Homebrew / Formula Syntax` | Pushes to `main`, `release/*`, or `snapshot/*`, PRs to `main`, and manual runs | Validates the branch-appropriate Homebrew formula Ruby syntax and inspects it through `brew info`: `container-compose` on `main` and `release/*`, `container-compose-snapshot` on `snapshot/*`. |
