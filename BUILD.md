...
| `Prebuilt Binaries / Resolve Publish Context` and `Package` | Successful `CI` workflow runs for `main`, `develop/VERSION`, or semantic tags, and manual runs with a `main`, `develop/VERSION`, or `MAJOR.MINOR.PATCH` input | Resolves publish eligibility on Ubuntu first, so only refs with a successful matching `CI / Validate` job start the macOS packaging runner. The package job builds the plugin archive once, publishes the matching GitHub release asset, updates `stephenlclarke/homebrew-tap` only for installable pre-release or stable lanes, and prunes older release assets after their notes contain source-install instructions. Manual runs fall back to `make ci` only when no successful CI result exists for the selected SHA. `main` publishes immutable `homebrew-main-RUN-SHA` assets without changing the stable formula, `develop/VERSION` publishes immutable `VERSION-pre.RUN.SHA` assets and updates `container-compose-pre`, and semantic tags publish stable assets and update `container-compose`. |
