...
| --- | --- | --- |
| `CI / Validate` | Pushes to `main` and `develop/*`, PRs to `main`, semantic release tags, and manual runs | Runs `make ci-fast` for the required quick gate. On `main`, `develop/VERSION`, semantic tags, and manual runs it also runs the coverage gate. SonarQube runs only on `main`, matching the free-tier single-branch setup. |
| `Prebuilt Binaries / Package` | Successful `CI` workflow runs for `develop/VERSION` or semantic tags, and manual runs | Publishes only after the matching CI `Validate` job succeeds. It builds the plugin archive once, publishes the matching GitHub release asset, updates `stephenlclarke/homebrew-tap`, and prunes older release assets after their notes contain source-install instructions. Manual runs fall back to `make ci` only when no successful CI result exists for the selected SHA. `develop/VERSION` publishes `VERSION-pre` and updates `container-compose-pre`; semantic tags publish stable assets and update `container-compose`. |
