...
static `ipv4_address` or `ipv6_address`. A Compose-managed IPv4 network can
also choose its IPAM gateway. Static addresses on a Compose-managed network
must belong to its declared matching IPAM subnet and cannot use that gateway;
external networks are validated by the runtime. Each `link_local_ips` value becomes an additional
