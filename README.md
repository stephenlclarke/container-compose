...
The recording shows a complete matched-runtime execution of the portable services in the real [`examples/monitoring-stack/docker-compose.yaml`](examples/monitoring-stack/docker-compose.yaml): startup output, `stats --no-stream`, `ps`, nginx `/healthz`, Alertmanager readiness, a marker written into the named `nginx_cache` volume, a retained-volume shutdown, a JSON listing that identifies that retained Compose asset, a second identical startup that reads the same marker, and final removal with `down --volumes --remove-orphans`. The second `up` and final `down` stay on screen as complete real command/output transcripts, making it clear that the existing named volume is reused rather than causing a create error. Before VHS starts, the release workflow runs every displayed command against the matched packaged runtime, captures its fresh output, and fails on any command error; the tape then replays that verified transcript at a readable pace. This avoids unreliable nested-virtualization behavior in terminal recording while ensuring the visible commands and results are from the exact build being published. A failed verification also publishes its partial transcript as a workflow artifact, so any future failure retains the exact runtime command and output that caused it. Every successful Current build regenerates the recording on the hardware-virtualization-capable release runner, then publishes it with the mutable `current` release.
