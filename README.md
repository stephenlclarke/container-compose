...
The unreleased CLI now auto-detects one-off terminal allocation from host input/output instead of inheriting the service's `tty` value. `-T` disables allocation; explicit `--tty` and `--no-tty` values retain their presence and cannot be combined, even when both are false. Swift callers supply the resolved choice using `ComposeRunOptions.noTty`; `false` now explicitly requests a terminal and `true` disables it. Packaged stock E11 and the Docker reference pass the redirected-input regression with zero owned residue. Full terminal-size, invalid-input and signal parity remain under development.
