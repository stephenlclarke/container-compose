...
The unreleased stock Engine adapter also preserves digest-qualified image spelling for descriptor-verified devcontainer inspection and records logical network keys independently of custom native names. Focused components pass; matched live C01 proof is still required. These changes add no Docker dependency and are not present in the currently published stable binary. See the [Bazel guide](docs/guides/BAZEL.md).
