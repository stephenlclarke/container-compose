...
The recording shows a complete matched-runtime execution of the portable services in the real [`examples/monitoring-stack/docker-compose.yaml`](examples/monitoring-stack/docker-compose.yaml): startup output, `stats --no-stream`, `ps`, nginx `/healthz`, Alertmanager readiness, a retained-volume shutdown, a second identical startup, and final removal with `down --volumes --remove-orphans`. Before VHS starts, the release workflow runs every displayed command against the matched packaged runtime, captures its fresh output, and fails on any command error; the tape then replays that verified transcript at a readable pace. This avoids unreliable nested-virtualization behavior in terminal recording while ensuring the visible commands and results are from the exact build being published. Every successful Current build regenerates the recording on the hardware-virtualization-capable release runner, then publishes it with the mutable `current` release.
