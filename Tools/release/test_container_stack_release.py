  ...
  case "$*" in
    ...
    *) return 2 ;;
    ...
    def test_current_demo_is_recoverable_and_not_release_critical(self) -> None:
        ...
        self.assertIn("CONTAINER_COMPOSE_DEMO_DEADLINE_RUNNER", workflow)
        self.assertIn("--seconds 10 --grace-seconds 2", tape)
        self.assertIn("container system status --format json", tape)
        self.assertIn(
            "/usr/bin/plutil -extract engineStatus raw -o - -",
            tape,
        )
        ...
        self.assertIn(
            "Wait+Screen@300s /container-compose-system-(ready|failed)/",
            tape,
        )
        up_commands = [
            shlex.split(line)[1]
    ...
    def test_release_sonar_step_preserves_and_restores_canonical_main(self) -> None:
        ...
        deadline_runner = "python3 Tools/ci/run-command-with-deadline.py"
        self.assertEqual(sonar.count(deadline_runner), 1)
        self.assertIn("--seconds 1500", sonar)
        self.assertIn("--grace-seconds 30", sonar)
        self.assertNotIn("gtimeout", sonar)
        ...
        self.assertEqual(restore.count(deadline_runner), 1)
        self.assertIn("--seconds 1500", restore)
        self.assertIn("--grace-seconds 30", restore)
        self.assertNotIn("gtimeout", restore)
