...
VOLUME_REUSE_MARKER_PATH = "/var/cache/nginx/.container-compose-volume-reuse"
VOLUME_REUSE_MARKER_VALUE = "container-compose-volume-reuse-ok"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--compose",
        type=Path,
        help="Optional Compose executable for local matched-runtime verification.",
    )
...
def display_curl_command(command: Sequence[str]) -> str:
    return "curl " + shlex.join(command[1:])
...
def steps(
    ...
    compose_version = (
        (str(compose_binary), "version")
        if compose_binary is not None
        else (str(container), "compose", "version")
    )
    compose = lambda *arguments: compose_command(
        container,
        compose_file,
        *arguments,
    ...
    write_volume_marker = compose(
        "exec",
        "--no-tty",
        "nginx",
        "sh",
        "-c",
        f"printf '%s\\n' {VOLUME_REUSE_MARKER_VALUE} > {VOLUME_REUSE_MARKER_PATH}; cat {VOLUME_REUSE_MARKER_PATH}",
    )
    read_volume_marker = compose("exec", "--no-tty", "nginx", "cat", VOLUME_REUSE_MARKER_PATH)
    return (
        TranscriptStep(
        ...
        TranscriptStep(
            "06-write-volume-marker.log",
            "TAPE_TRANSCRIPT_VOLUME_MARKER_WRITTEN_OK",
            (write_volume_marker,),
        ),
        TranscriptStep(
            "07-retained-down.log",
        ...
        TranscriptStep(
            "12-second-health.log",
            "TAPE_TRANSCRIPT_SECOND_HEALTH_OK",
            (nginx_health, alertmanager_health),
        ),
        TranscriptStep(
            "13-verify-volume-marker.log",
            "TAPE_TRANSCRIPT_VOLUME_MARKER_REUSED_OK",
            (read_volume_marker,),
        ),
        TranscriptStep(
            "14-final-down.log",
...
def rendered_command(compose_file: Path, command: Sequence[str]) -> str:
    ...
    if (len(command) >= 2 and command[1] == "compose") or Path(command[0]).name == "compose":
...
def record(args: argparse.Namespace) -> None:
    ...
    compose_binary = args.compose.resolve() if args.compose is not None else None
    ...
    if compose_binary is not None and not compose_binary.is_file():
        raise TranscriptFailure(f"Compose binary does not exist: {compose_binary}")
    ...
    for transcript in output_directory.glob("*.log"):
        transcript.unlink()
    try:
        ...
        cleanup(container, compose_file, working_directory, compose_binary)
        for step in steps(container, compose_file, compose_binary):
            run_step(
                step,
                container=container,
