...
#
# USAGE:
#   check-compose-cgroup-namespace.sh [options]
#
# OPTIONS:
...
#
# This script verifies that Docker Compose V2 accepts `cgroup: host`, then
...
FIXTURE_DIR=""
PROJECT_NAME="cc-cgroupns-$RANDOM"
...
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" | sed "s/check-compose-cgroup-namespace.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker Compose cgroup namespace parity check"
    exit 0
}
...
create_fixture() {
    FIXTURE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/compose-cgroupns.XXXXXX")"
...
assert_config_preserves_host_cgroup() {
    python3 - "$1" "$2" <<'PY'
import json
import pathlib
import sys
...
if service.get("cgroup") != "host":
    raise SystemExit(f"{tool} cgroup = {service.get('cgroup')!r}, want 'host'")
PY
}
...
expect_docker_behavior() {
    ...
    assert_config_preserves_host_cgroup 'Docker Compose' "$config_output"
    ...
    if ! grep -F "Container $PROJECT_NAME-api-1 Created" "$dry_run_output" >/dev/null; then
        error 'Docker Compose did not accept cgroup: host in local dry-run up'
...
expect_container_behavior() {
    ...
    if ! grep -F -- '--cgroupns host' "$dry_run_output" >/dev/null; then
        error 'container-compose did not project cgroup: host to --cgroupns host in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
}
...
main() {
    ...
    expect_docker_behavior
    expect_container_behavior
    info 'Docker Compose config and container-compose cgroup namespace parity passed.'
}
