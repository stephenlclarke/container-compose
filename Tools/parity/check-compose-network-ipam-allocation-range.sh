...
#
# USAGE:
#   check-compose-network-ipam-allocation-range.sh [options]
#
# OPTIONS:
...
#   CONTAINER_COMPOSE_INIT_IMAGE   Exact loaded guest-init image for live proof.
...
#
# This local-only certificate compares Docker Compose IPv4 allocation-range and
# auxiliary-address behavior with the candidate. It proves model projection,
# dynamic allocation from ip_range (including its lower address), gateway and
# auxiliary reservations, service restart retention, and network cleanup. One
# timing sample per operation is diagnostic only; it is not performance proof.
...
readonly ROOT_MARKER=".container-compose-network-ipam-allocation-range-root"
readonly ROOT_MARKER_VALUE="container-compose IPv4 IPAM allocation-range certificate v1"
...
usage() {
    sed -n '/^# USAGE:/,/^# This local-only/ { /^# This local-only/d; s/^# //; s/^#//; p; }' "$SELF_PATH" |
        sed "s/check-compose-network-ipam-allocation-range.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    info "warning: $message; skipping IPv4 IPAM allocation-range certificate"
    exit 0
}
...
assert_live_fingerprint_inputs() {
    ...
    for source_path in "$CONTAINER_PACKAGE_PATH" "$CONTAINERIZATION_PACKAGE_PATH" "$CONTAINER_ENGINE_API_PACKAGE_PATH"; do
        [[ -n "$source_path" && -e "$source_path/.git" && -f "$source_path/Package.resolved" ]] || {
            error "live IPAM certificate requires an exact local source with Package.resolved: $source_path"
            return 2
        }
    done
    [[ -n "${CONTAINER_COMPOSE_INIT_IMAGE:-}" ]] || {
        error 'live IPAM certificate requires CONTAINER_COMPOSE_INIT_IMAGE'
        return 2
    }
    [[ -f "$FINGERPRINT_WRITER" ]] || {
        error "IPAM fingerprint writer is unavailable: $FINGERPRINT_WRITER"
        return 2
    }
    local install_root
    ...
    for required_binary in \
        ...
        [[ -x "$required_binary" ]] || {
            error "live IPAM certificate requires an exact executable: $required_binary"
            return 2
        }
    done
}
...
prepare_evidence_root() {
    if [[ -z "$EVIDENCE_ROOT" ]]; then
        EVIDENCE_ROOT="$(mktemp -d /private/tmp/container-compose-network-ipam-allocation-range.XXXXXX)"
    fi
...
write_candidate_fingerprint() {
    ...
    case "$phase" in
        ...
        complete)
            ...
            ;;
        *)
            error "unsupported IPAM fingerprint phase: $phase"
            return 2
            ;;
    esac
...
# Assert the Docker Compose request model retains all requested IPAM fields.
...
network = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))["networks"]["backend"]
expected = {
    "subnet": "192.168.200.0/24",
    "ip_range": "192.168.200.128/25",
    "gateway": "192.168.200.1",
...
if config != [expected]:
    raise SystemExit(f"Docker Compose IPAM model = {config!r}")
PY
}

# Assert the candidate request model retains all runtime-facing IPAM fields.
...
expected = {
    ...
    "ipv4AllocationRange": "192.168.200.128/25",
    "ipv4Gateway": "192.168.200.1",
    "ipv4ReservedAddresses": ["192.168.200.130", "192.168.200.10"],
}
for key, value in expected.items():
    if network.get(key) != value:
        raise SystemExit(f"candidate {key} = {network.get(key)!r}, expected {value!r}")
if network.get("unsupportedFields") is not None:
    raise SystemExit(f"candidate unsupported fields = {network['unsupportedFields']!r}")
PY
}

# Validate a Docker network plus both endpoint inspections against the oracle.
assert_docker_observed_state() {
    python3 - \
        "$EVIDENCE_ROOT/reference/network.json" \
        "$EVIDENCE_ROOT/reference/alpha-before-restart.json" \
        "$EVIDENCE_ROOT/reference/beta.json" \
        "$EVIDENCE_ROOT/reference/alpha-after-restart.json" <<'PY'
import ipaddress
import json
import pathlib
import sys
...
alpha_before = json.loads(pathlib.Path(sys.argv[2]).read_text(encoding="utf-8"))[0]
beta = json.loads(pathlib.Path(sys.argv[3]).read_text(encoding="utf-8"))[0]
alpha_after = json.loads(pathlib.Path(sys.argv[4]).read_text(encoding="utf-8"))[0]
...
if config != [expected_config]:
    raise SystemExit(f"Docker network IPAM configuration = {config!r}")

def address(container):
    attachments = container.get("NetworkSettings", {}).get("Networks", {})
    if len(attachments) != 1:
        raise SystemExit(f"Docker attachments = {attachments!r}")
    value = next(iter(attachments.values())).get("IPAddress")
    if not value:
        raise SystemExit(f"Docker endpoint address = {value!r}")
    return ipaddress.ip_address(value)

alpha_address = address(alpha_before)
beta_address = address(beta)
alpha_after_address = address(alpha_after)
if alpha_after_address != alpha_address:
    raise SystemExit(f"Docker restart changed alpha address: {alpha_address} -> {alpha_after_address}")

addresses = {alpha_address, beta_address}
expected_addresses = {ipaddress.ip_address("192.168.200.128"), ipaddress.ip_address("192.168.200.129")}
if addresses != expected_addresses:
    raise SystemExit(f"Docker dynamic endpoints = {sorted(map(str, addresses))!r}")
allocation_range = ipaddress.ip_network("192.168.200.128/25")
reserved = {
    ipaddress.ip_address("192.168.200.1"),
    ipaddress.ip_address("192.168.200.130"),
    ipaddress.ip_address("192.168.200.10"),
}
if not all(item in allocation_range and item not in reserved for item in addresses):
    raise SystemExit(f"Docker allocated outside range or into a reservation: {sorted(map(str, addresses))!r}")
PY
}

# Validate the candidate native network and endpoints against the same oracle.
assert_candidate_observed_state() {
    python3 - \
        "$EVIDENCE_ROOT/candidate/network.json" \
        "$EVIDENCE_ROOT/candidate/containers-before-restart.json" \
        "$EVIDENCE_ROOT/candidate/containers-after-restart.json" \
        "$CANDIDATE_PROJECT" <<'PY'
import ipaddress
import json
import pathlib
import sys
...
after = json.loads(pathlib.Path(sys.argv[3]).read_text(encoding="utf-8"))
project = sys.argv[4]
if len(network_items) != 1:
    raise SystemExit(f"candidate network inspect count = {len(network_items)}")
configuration = network_items[0].get("configuration", {})
...
for key, value in expected_configuration.items():
    if configuration.get(key) != value:
        raise SystemExit(f"candidate network {key} = {configuration.get(key)!r}, expected {value!r}")

def endpoint(items, service):
    container_name = f"{project}-{service}-1"
    container = next(
        (item for item in items if item.get("configuration", {}).get("id") == container_name),
        None,
    )
    if container is None:
        raise SystemExit(f"candidate container {container_name!r} is absent")
    attachments = container.get("status", {}).get("networks", [])
    if len(attachments) != 1:
        raise SystemExit(f"candidate {service} attachments = {attachments!r}")
    value = attachments[0].get("ipv4Address")
    if not value:
        raise SystemExit(f"candidate {service} IPv4 address = {value!r}")
    return ipaddress.ip_interface(value).ip

alpha_before = endpoint(before, "alpha")
beta_address = endpoint(before, "beta")
alpha_after = endpoint(after, "alpha")
if alpha_after != alpha_before:
    raise SystemExit(f"candidate restart changed alpha address: {alpha_before} -> {alpha_after}")

addresses = {alpha_before, beta_address}
expected_addresses = {ipaddress.ip_address("192.168.200.128"), ipaddress.ip_address("192.168.200.129")}
if addresses != expected_addresses:
    raise SystemExit(f"candidate dynamic endpoints = {sorted(map(str, addresses))!r}")
allocation_range = ipaddress.ip_network("192.168.200.128/25")
reserved = {
    ipaddress.ip_address("192.168.200.1"),
    ipaddress.ip_address("192.168.200.130"),
...
if not all(item in allocation_range and item not in reserved for item in addresses):
    raise SystemExit(f"candidate allocated outside range or into a reservation: {sorted(map(str, addresses))!r}")
PY
}
...
run_reference() {
    info 'Running Docker Compose IPv4 IPAM allocation-range reference lane'
    "${DOCKER_COMPOSE_COMMAND[@]}" --project-name "$DOCKER_PROJECT" --file "$FIXTURE_FILE" config --format json \
        >"$EVIDENCE_ROOT/reference/config.json"
    assert_docker_config
    ...
    local network_name="${DOCKER_PROJECT}_backend"
    local alpha_id
    local beta_id
    alpha_id="$("${DOCKER_COMPOSE_COMMAND[@]}" --project-name "$DOCKER_PROJECT" --file "$FIXTURE_FILE" ps --quiet alpha)"
    beta_id="$("${DOCKER_COMPOSE_COMMAND[@]}" --project-name "$DOCKER_PROJECT" --file "$FIXTURE_FILE" ps --quiet beta)"
    docker network inspect "$network_name" >"$EVIDENCE_ROOT/reference/network.json"
    docker inspect "$alpha_id" >"$EVIDENCE_ROOT/reference/alpha-before-restart.json"
    docker inspect "$beta_id" >"$EVIDENCE_ROOT/reference/beta.json"
    run_capture docker restart-alpha "$EVIDENCE_ROOT/reference/restart-alpha.log" \
        "${DOCKER_COMPOSE_COMMAND[@]}" --project-name "$DOCKER_PROJECT" --file "$FIXTURE_FILE" restart alpha
    docker inspect "$alpha_id" >"$EVIDENCE_ROOT/reference/alpha-after-restart.json"
    assert_docker_observed_state
    run_capture docker down "$EVIDENCE_ROOT/reference/down.log" \
        "${DOCKER_COMPOSE_COMMAND[@]}" --project-name "$DOCKER_PROJECT" --file "$FIXTURE_FILE" down --remove-orphans --volumes
...
run_candidate() {
    [[ "$CONTAINER_COMPOSE_LIVE" == "1" ]] || return 0
    info 'Running container-compose IPv4 IPAM allocation-range candidate lane'
    ...
    local network_name="${CANDIDATE_PROJECT}_backend"
    run_bounded "$CONTAINER_BINARY" network inspect "$network_name" >"$EVIDENCE_ROOT/candidate/network.json"
    run_bounded "$CONTAINER_BINARY" list --all --format json >"$EVIDENCE_ROOT/candidate/containers-before-restart.json"
    run_capture container-compose restart-alpha "$EVIDENCE_ROOT/candidate/restart-alpha.log" \
        "${compose_command[@]}" restart alpha
    run_bounded "$CONTAINER_BINARY" list --all --format json >"$EVIDENCE_ROOT/candidate/containers-after-restart.json"
    assert_candidate_observed_state
...
cleanup() {
    local status=$?
    trap - EXIT
    if [[ -n "$FIXTURE_FILE" && ${#DOCKER_COMPOSE_COMMAND[@]} -gt 0 ]]; then
        "${DOCKER_COMPOSE_COMMAND[@]}" --project-name "$DOCKER_PROJECT" --file "$FIXTURE_FILE" \
            down --remove-orphans --volumes >/dev/null 2>&1 || true
        if [[ "$CONTAINER_COMPOSE_LIVE" == "1" ]]; then
            candidate_compose down --remove-orphans --volumes >/dev/null 2>&1 || true
        fi
    fi
    exit "$status"
}
...
main() {
    ...
    prepare_evidence_root
    write_fixture
    DOCKER_PROJECT="cc-ipam-reference-$RANDOM-$$"
    CANDIDATE_PROJECT="cc-ipam-candidate-$RANDOM-$$"
    trap cleanup EXIT
    run_reference
    write_candidate_fingerprint preflight
    run_candidate
    write_candidate_fingerprint complete
    info "IPv4 IPAM allocation-range certificate passed; retained evidence: $EVIDENCE_ROOT"
}
