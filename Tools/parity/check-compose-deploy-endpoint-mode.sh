...
#
# USAGE:
#   check-compose-deploy-endpoint-mode.sh [options]
#
# OPTIONS:
...
# Docker Compose V2 preserves deploy.endpoint_mode in config output while local
# dry-run orchestration accepts the service, then verifies container-compose
# mirrors that local-mode behavior without treating endpoint_mode as unsupported.
...
FIXTURE_DIR=""
PROJECT_NAME="cc-endpoint-mode-$RANDOM"
...
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" | sed "s/check-compose-deploy-endpoint-mode.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker Compose deploy.endpoint_mode parity check"
    exit 0
}
...
# Create a minimal Compose fixture with Swarm endpoint-mode metadata.
create_fixture() {
    FIXTURE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/compose-deploy-endpoint-mode.XXXXXX")"
...
# Assert Docker Compose preserves deploy.endpoint_mode in config output.
assert_docker_config_preserves_endpoint_mode() {
    local path="$1"
...
mode = doc.get("services", {}).get("api", {}).get("deploy", {}).get("endpoint_mode")
if mode != "dnsrr":
    raise SystemExit(f"Docker Compose deploy.endpoint_mode = {mode!r}, want 'dnsrr'")
PY
}

# Assert container-compose config does not reject endpoint_mode metadata.
assert_container_config_accepts_endpoint_mode() {
    local path="$1"
...
if "endpoint_mode" in fields:
    raise SystemExit(f"container-compose still reports endpoint_mode as unsupported: {fields!r}")
PY
}
...
expect_docker_behavior() {
    ...
    assert_docker_config_preserves_endpoint_mode "$config_output"
    ...
    if ! grep -F "Container $PROJECT_NAME-api-1 Created" "$dry_run_output" >/dev/null; then
        error 'Docker Compose did not accept deploy.endpoint_mode in local dry-run up'
...
expect_container_behavior() {
    ...
    "$CONTAINER_COMPOSE" \
        ...
        -f "$FIXTURE_DIR/compose.yml" \
        config --format json >"$config_output"
    assert_container_config_accepts_endpoint_mode "$config_output"
    ...
    if ! grep -F "container create --name $PROJECT_NAME-api-1" "$dry_run_output" >/dev/null; then
        error 'container-compose did not accept deploy.endpoint_mode in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
}
...
main() {
    ...
    info 'Docker Compose deploy.endpoint_mode parity passed.'
}
