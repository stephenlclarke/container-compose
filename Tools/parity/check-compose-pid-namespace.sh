...
#
# USAGE:
#   check-compose-pid-namespace.sh [options]
#
# OPTIONS:
...
#
# This script verifies that Docker Compose V2 accepts `pid: private`, then
# checks that container-compose retains the generic runtime private default.
...
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" | sed "s/check-compose-pid-namespace.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker Compose private PID namespace parity check"
    exit 0
}
...
create_fixture() {
    FIXTURE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/compose-pidns.XXXXXX")"
...
assert_config_preserves_private_pid() {
    python3 - "$1" "$2" <<'PY'
import json
import pathlib
import sys
...
if service.get("pid") != "private":
    raise SystemExit(f"{tool} pid = {service.get('pid')!r}, want 'private'")
PY
}
...
expect_docker_behavior() {
    ...
    assert_config_preserves_private_pid 'Docker Compose' "$config_output"
    ...
    if ! grep -F "Container $PROJECT_NAME-api-1 Created" "$dry_run_output" >/dev/null; then
        error 'Docker Compose did not accept pid: private in local dry-run up'
...
expect_container_behavior() {
    ...
    assert_config_preserves_private_pid 'container-compose' "$config_output"
    ...
    if ! grep -F "container create --name $PROJECT_NAME-api-1" "$dry_run_output" >/dev/null; then
        error 'container-compose did not accept pid: private in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
    if grep -F -- '--pid' "$dry_run_output" >/dev/null; then
        error 'container-compose projected pid: private despite the generic private default'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
}
...
main() {
    ...
    expect_docker_behavior
    expect_container_behavior
    info 'Docker Compose config and container-compose private PID namespace parity passed.'
}
