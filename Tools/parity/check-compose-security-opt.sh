...
#
# USAGE:
#   check-compose-security-opt.sh [options]
#
# OPTIONS:
...
# Docker Compose V2 preserves no-new-privileges in config output and accepts
# it for local orchestration. It then verifies container-compose projects the
# supported option through the generic runtime --security-opt bridge.
...
FIXTURE_DIR=""
PROJECT_NAME="cc-security-opt-$RANDOM"
...
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" | sed "s/check-compose-security-opt.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker Compose security_opt parity check"
    exit 0
}
...
# Assert Docker Compose preserves the supported security option in its public
# Compose-spec config model.
assert_docker_config_preserves_security_option() {
    local path="$1"
...
if options != ["no-new-privileges:true"]:
    raise SystemExit(f"security_opt = {options!r}, want ['no-new-privileges:true']")
PY
}

# Assert the normalized container-compose service model preserves the same
# option. Its generated JSON uses the Swift `securityOpt` property name.
assert_container_config_preserves_security_option() {
    local path="$1"
...
if options != ["no-new-privileges:true"]:
    raise SystemExit(f"securityOpt = {options!r}, want ['no-new-privileges:true']")
PY
}
...
expect_docker_behavior() {
    ...
    assert_docker_config_preserves_security_option "$config_output"
...
expect_container_behavior() {
    ...
    "$CONTAINER_COMPOSE" \
        ...
        -f "$FIXTURE_DIR/compose.yml" \
        config --format json >"$config_output"
    assert_container_config_preserves_security_option "$config_output"
...
main() {
    ...
    info 'Docker Compose config and container-compose security_opt parity passed.'
}
