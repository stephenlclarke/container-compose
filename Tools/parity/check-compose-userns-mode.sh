...
#
# This script verifies that Docker Compose V2 accepts `userns_mode: host`,
# then checks that container-compose preserves the sandbox guest's default user
# namespace rather than inventing an unsupported user-namespace flag.
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker Compose host user namespace parity check"
    exit 0
}
...
assert_config_preserves_host_user_namespace() {
    python3 - "$1" "$2" <<'PY'
import json
import pathlib
import sys
...
if service.get("userns_mode") != "host":
    raise SystemExit(f"{tool} userns_mode = {service.get('userns_mode')!r}, want 'host'")
PY
}
...
expect_docker_behavior() {
    ...
    assert_config_preserves_host_user_namespace 'Docker Compose' "$config_output"
    ...
    if ! grep -F "Container $PROJECT_NAME-api-1 Created" "$dry_run_output" >/dev/null; then
        error 'Docker Compose did not accept userns_mode: host in local dry-run up'
...
expect_container_behavior() {
    ...
    assert_config_preserves_host_user_namespace 'container-compose' "$config_output"
    ...
    if ! grep -F "container create --name $PROJECT_NAME-api-1" "$dry_run_output" >/dev/null; then
        error 'container-compose did not accept userns_mode: host in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
    if grep -F -- '--userns' "$dry_run_output" >/dev/null; then
...
main() {
    ...
    expect_docker_behavior
    expect_container_behavior
    info 'Docker Compose config and container-compose host user namespace parity passed.'
}
