...
#
# USAGE:
#   check-compose-cgroup-parent.sh [options]
#
# OPTIONS:
...
#
# This script verifies that Docker Compose V2 accepts `cgroup_parent`, then
# checks that container-compose preserves it and emits the generic guest-only
# runtime option.
...
FIXTURE_DIR=""
PROJECT_NAME="cc-cgroup-parent-$RANDOM"
...
# Print command usage derived from this script's documentation block.
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" |
        sed "s/check-compose-cgroup-parent.sh/$SCRIPT_NAME/"
}
...
# Skip an optional check or turn it into a strict-mode failure.
skip_or_fail() {
    local message="$1"
    if ((STRICT == 1)); then
        error "$message"
    ...
    warning "$message; skipping Docker Compose cgroup-parent parity check"
    exit 0
}

# Select the available Docker Compose V2 command.
...
# Confirm local dependencies needed by both parity implementations.
check_tools() {
...
# Detect whether Engine dry-run evidence can supplement config parity.
...
# Create the isolated Compose fixture.
create_fixture() {
    FIXTURE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/compose-cgroup-parent.XXXXXX")"
    cat >"$FIXTURE_DIR/compose.yml" <<'YAML'
services:
  api:
    image: alpine:3.20
    cgroup_parent: workloads/build
YAML
}
...
# Assert that the normalized configuration preserves its expected field name.
assert_config_preserves_cgroup_parent() {
    python3 - "$1" "$2" <<'PY'
import json
import pathlib
import sys
...
parent = service.get("cgroup_parent", service.get("cgroupParent"))
if parent != "workloads/build":
    raise SystemExit(f"{tool} cgroup parent = {parent!r}, want 'workloads/build'")
PY
}

# Confirm Docker Compose V2 accepts the service attribute.
expect_docker_behavior() {
    ...
    assert_config_preserves_cgroup_parent 'Docker Compose' "$config_output"
    ...
    if ! grep -F "Container $PROJECT_NAME-api-1 Created" "$dry_run_output" >/dev/null; then
        error 'Docker Compose did not accept cgroup_parent in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
}

# Confirm container-compose emits the matching generic runtime option.
expect_container_behavior() {
    ...
    assert_config_preserves_cgroup_parent 'container-compose' "$config_output"
    ...
    if ! grep -F "container create --name $PROJECT_NAME-api-1" "$dry_run_output" >/dev/null; then
        error 'container-compose did not accept cgroup_parent in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
    if ! grep -F -- '--cgroup-parent workloads/build' "$dry_run_output" >/dev/null; then
        error 'container-compose did not project cgroup_parent to --cgroup-parent in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
}

# Run the parity workflow and clean up its fixture.
main() {
    parse_args "$@"
    detect_docker_compose
    check_tools
    ...
    expect_docker_behavior
    expect_container_behavior
    info 'Docker Compose config and container-compose cgroup-parent parity passed.'
}
