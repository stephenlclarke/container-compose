...
#
# USAGE:
#   check-compose-devices.sh [options]
#
# OPTIONS:
...
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" | sed "s/check-compose-devices.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker/container-compose device parity check"
    exit 0
}
...
# Create minimal Compose fixtures for supported and invalid device forms.
create_fixture() {
    FIXTURE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/compose-devices.XXXXXX")"
    COMPOSE_FILE="$FIXTURE_DIR/compose.yml"
    INVALID_FILE="$FIXTURE_DIR/invalid.yml"
    AMBIGUOUS_FILE="$FIXTURE_DIR/ambiguous.yml"
...
# Assert a JSON Compose service preserves the expected device mappings.
assert_config_devices() {
    local path="$1"
    local source="$2"
...
expected = [
    {"source": "/dev/null", "target": "/dev/xnull", "permissions": "rw"},
    {"source": "/dev/zero", "target": "/dev/zero", "permissions": "rwm"},
]
if devices != expected:
    raise SystemExit(f"{source} devices = {devices!r}, want {expected!r}")
PY
}
...
validate_docker_behavior() {
    ...
    assert_config_devices "$config_output" "Docker Compose"
...
devices = json.loads(inspect)[0]["HostConfig"].get("Devices")
expected = [
    {"PathOnHost": "/dev/null", "PathInContainer": "/dev/xnull", "CgroupPermissions": "rw"},
    {"PathOnHost": "/dev/zero", "PathInContainer": "/dev/zero", "CgroupPermissions": "rwm"},
]
if devices != expected:
    raise SystemExit(f"Docker HostConfig.Devices = {devices!r}, want {expected!r}")
PY
}
...
validate_container_compose_behavior() {
    ...
    assert_config_devices "$config_output" "container-compose"
    ...
    [[ -n "$create_line" ]] || { error 'missing dry-run command for devices service create'; return 1; }
    [[ -n "$run_line" ]] || { error 'missing dry-run command for devices service run'; return 1; }

    for line in "$up_line" "$create_line" "$run_line"; do
        [[ "$line" == *"--device /dev/null:/dev/xnull:rw"* ]] || { error "missing explicit device mapping in dry-run line: $line"; return 1; }
        [[ "$line" == *"--device /dev/zero"* ]] || { error "missing default-target device mapping in dry-run line: $line"; return 1; }
    done

    invalid_output="$("$CONTAINER_COMPOSE" --ansi never --dry-run -p "$PROJECT_NAME-invalid" -f "$INVALID_FILE" up api 2>&1 || true)"
    [[ "$invalid_output" == *"service 'api' has invalid devices"* ]] || {
        error "invalid devices blocker changed: $invalid_output"
        return 1
    }

    ambiguous_output="$("$CONTAINER_COMPOSE" --ansi never --dry-run -p "$PROJECT_NAME-ambiguous" -f "$AMBIGUOUS_FILE" up api 2>&1 || true)"
    [[ "$ambiguous_output" == *"service 'api' has invalid devices"* ]] || {
        error "ambiguous device short-form blocker changed: $ambiguous_output"
        return 1
    }
}
...
main() {
    ...
    validate_docker_behavior
    validate_container_compose_behavior
    info "Docker Compose device parity check passed for project $PROJECT_NAME"
}
