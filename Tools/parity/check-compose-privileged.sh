...
#
# USAGE:
#   check-compose-privileged.sh [options]
#
# OPTIONS:
...
#
# This script verifies that Docker Compose V2 preserves `privileged: true`,
# then checks that container-compose retains it and projects `--privileged` to
# the generic macOS runtime.
...
FIXTURE_DIR=""
PROJECT_NAME="cc-privileged-$RANDOM"
...
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" | sed "s/check-compose-privileged.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker Compose privileged parity check"
    exit 0
}
...
create_fixture() {
    FIXTURE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/compose-privileged.XXXXXX")"
...
assert_config_preserves_privileged() {
    python3 - "$1" "$2" <<'PY'
import json
import pathlib
import sys
...
if service.get("privileged") is not True:
    raise SystemExit(f"{tool} privileged = {service.get('privileged')!r}, want true")
PY
}
...
expect_docker_behavior() {
    ...
    assert_config_preserves_privileged 'Docker Compose' "$config_output"
    ...
    if ! grep -F "Container $PROJECT_NAME-api-1 Created" "$dry_run_output" >/dev/null; then
        error 'Docker Compose did not accept privileged: true in local dry-run up'
...
expect_container_behavior() {
    ...
    assert_config_preserves_privileged 'container-compose' "$config_output"
    ...
    if ! grep -F "container create --name $PROJECT_NAME-api-1" "$dry_run_output" >/dev/null; then
        error 'container-compose did not accept privileged: true in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
    if ! grep -F -- '--privileged' "$dry_run_output" >/dev/null; then
        error 'container-compose did not project privileged: true as --privileged'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
}
...
main() {
    ...
    expect_docker_behavior
    expect_container_behavior
    info 'Docker Compose config and container-compose privileged parity passed.'
}
