...
#
# USAGE:
#   check-compose-deploy-resource-reservations.sh [options]
#
# OPTIONS:
...
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" | sed "s/check-compose-deploy-resource-reservations.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker Compose deploy resource reservation parity check"
    exit 0
}
...
# Assert Docker Compose preserves CPU/memory reservations in config output.
assert_docker_config_preserves_reservations() {
    local path="$1"
...
reservations = doc.get("services", {}).get("api", {}).get("deploy", {}).get("resources", {}).get("reservations", {})
if reservations.get("cpus") != 0.25:
    raise SystemExit(f"Docker Compose deploy.resources.reservations.cpus = {reservations.get('cpus')!r}, want 0.25")
if reservations.get("memory") != "33554432":
    raise SystemExit(f"Docker Compose deploy.resources.reservations.memory = {reservations.get('memory')!r}, want '33554432'")
PY
}
...
expect_docker_behavior() {
    ...
    if ! grep -F "Container $PROJECT_NAME-api-1 Created" "$dry_run_output" >/dev/null; then
        error 'Docker Compose did not accept deploy CPU/memory reservations in local dry-run up'
...
expect_container_behavior() {
    ...
    if ! grep -F "container create --name $PROJECT_NAME-api-1" "$dry_run_output" >/dev/null; then
        error 'container-compose did not accept deploy CPU/memory reservations in local dry-run up'
        sed -n '1,120p' "$dry_run_output" >&2
        return 1
    fi
