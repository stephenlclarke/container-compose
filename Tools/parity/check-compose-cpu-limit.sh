...
#
# USAGE:
#   check-compose-cpu-limit.sh [options]
#
# OPTIONS:
...
FIXTURE_DIR=""
PROJECT_NAME="cc-cpu-limit-$RANDOM"
...
usage() {
    sed -n '/^# USAGE:/,/^# This script/ { /^# This script/d; s/^# //; s/^#//; p; }' "$SELF_PATH" | sed "s/check-compose-cpu-limit.sh/$SCRIPT_NAME/"
}
...
skip_or_fail() {
    ...
    warning "$message; skipping Docker Compose CPU-limit parity check"
    exit 0
}
...
create_fixture() {
    FIXTURE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/compose-cpu-limit.XXXXXX")"
...
assert_docker_config_preserves_cpu_limit() {
    python3 - "$1" <<'PY'
import json
import pathlib
import sys
...
assert_container_config_preserves_cpu_limit() {
    python3 - "$1" <<'PY'
import json
import pathlib
import sys
