...
# Docker Compose V2 handling for service `pids_limit`, checks Docker Engine
# HostConfig projection, then checks the same Compose file through
# container-compose config and dry-run output.
...
# Create minimal Compose fixtures for limited and unlimited pids_limit forms.
create_fixture() {
...
if limit != 128:
    raise SystemExit(f"Docker HostConfig.PidsLimit = {limit!r}, want 128")
PY
}
...
line_has_pids_limit() {
    ...
    printf '%s\n' "$line" | grep -F -- "--pids-limit '128'" >/dev/null \
        || printf '%s\n' "$line" | grep -F -- "--pids-limit 128" >/dev/null
}
...
validate_container_compose_behavior() {
    ...
    if [[ -z "$up_line" ]] || ! line_has_pids_limit "$up_line"; then
        error "container-compose dry-run up did not render --pids-limit 128"
        printf '%s\n' "$up_output" >&2
        return 1
    fi
    if [[ -z "$create_line" ]] || ! line_has_pids_limit "$create_line"; then
        error "container-compose dry-run create did not render --pids-limit 128"
        printf '%s\n' "$create_output" >&2
        return 1
    fi
    if [[ -z "$run_line" ]] || ! line_has_pids_limit "$run_line"; then
    ...
    unlimited_line="$(dry_run_api_line "$unlimited_output")"
...
main() {
    ...
    validate_docker_behavior
    validate_container_compose_behavior
    info "pids_limit parity check passed using ${DOCKER_COMPOSE_COMMAND[*]} and $CONTAINER_COMPOSE"
}
