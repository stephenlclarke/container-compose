...
cli-smoke-built:
	...
	[[ "$$version_short_output" == "0.5.1" ]]; \
	version_pretty_output="$$(".build/debug/compose" version)"; \
	[[ "$$version_pretty_output" == *"container-compose 0.5.1"* ]]; \
	...
	[[ "$$version_json_output" == *'"version":"0.5.1"'* ]]; \
	...
	[[ "$$version_short_format_output" == *'"version":"0.5.1"'* ]]; \
	version_compact_format_output="$$(".build/debug/compose" version -fjson)"; \
	[[ "$$version_compact_format_output" == *'"version":"0.5.1"'* ]]; \
	...
	printf '%s\n' '#!/usr/bin/env bash' 'if [[ "$$*" == "system version --format json" ]]; then' '  printf '\''[{"appName":"container","buildType":"release","commit":"abc123","containerization":"apple/containerization@main","distribution":"apple","source":"apple/container","version":"0.5.1"}]\n'\''' '  exit 0' 'fi' 'exit 2' > "$$compat_tmp/container"; \
	...
	[[ "$$version_compact_global_output" == "0.5.1" ]]; \
