...
start-dev-release:
	@test -n "$(VERSION_SELECTOR)" || { \
		printf 'VERSION_SELECTOR is required, for example: make start-dev-release VERSION_SELECTOR=--+\n' >&2; \
		exit 2; \
	}
...
cli-smoke-built:
	...
	[[ "$$version_short_output" == "0.6.4" ]]; \
	version_pretty_output="$$(".build/debug/compose" version)"; \
	[[ "$$version_pretty_output" == *"container-compose 0.6.4"* ]]; \
	...
	[[ "$$version_json_output" == *'"version":"0.6.4"'* ]]; \
	...
	[[ "$$version_short_format_output" == *'"version":"0.6.4"'* ]]; \
	version_compact_format_output="$$(".build/debug/compose" version -fjson)"; \
	[[ "$$version_compact_format_output" == *'"version":"0.6.4"'* ]]; \
	...
	[[ "$$version_compact_global_output" == "0.6.4" ]]; \
