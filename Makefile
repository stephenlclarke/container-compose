...
override RELEASE_GATE_INHERITED_ENVIRONMENT_FINGERPRINT = $(shell $(PYTHON) ./Tools/ci/fingerprint-release-environment.py)
...
release-gate-environment-fingerprint-check:
	@if ! [[ "$(RELEASE_GATE_INHERITED_ENVIRONMENT_FINGERPRINT)" =~ ^[0-9a-f]{64}$$ ]]; then \
		printf 'could not fingerprint the inherited release-gate environment\n' >&2; \
		exit 2; \
	fi

release-gate: release-gate-environment-fingerprint-check
	$(PYTHON) ./Tools/ci/run-release-checkpoint.py --checkpoint-dir "$(RELEASE_GATE_CHECKPOINT_DIR)" --stage sibling-stack --fingerprint "$(RELEASE_GATE_FINGERPRINT)" --seconds "$(RELEASE_GATE_STACK_TIMEOUT_SECONDS)" -- $(MAKE) --no-print-directory container-stack-release-validation
	$(PYTHON) ./Tools/ci/run-release-checkpoint.py --checkpoint-dir "$(RELEASE_GATE_CHECKPOINT_DIR)" --stage compose-ci --fingerprint "$(RELEASE_GATE_FINGERPRINT)" --seconds "$(RELEASE_GATE_STAGE_TIMEOUT_SECONDS)" -- $(MAKE) --no-print-directory ci
	$(PYTHON) ./Tools/ci/run-release-checkpoint.py --checkpoint-dir "$(RELEASE_GATE_CHECKPOINT_DIR)" --stage swift-runtime --fingerprint "$(RELEASE_GATE_FINGERPRINT)" --seconds "$(RELEASE_GATE_STAGE_TIMEOUT_SECONDS)" -- $(MAKE) --no-print-directory swift-runtime-test
	$(PYTHON) ./Tools/ci/run-release-checkpoint.py --checkpoint-dir "$(RELEASE_GATE_CHECKPOINT_DIR)" --stage compose-parity --fingerprint "$(RELEASE_GATE_FINGERPRINT)" --seconds "$(RELEASE_GATE_PARITY_TIMEOUT_SECONDS)" -- $(MAKE) --no-print-directory docker-compose-parity

release-gate-hosted: release-gate-environment-fingerprint-check
	$(PYTHON) ./Tools/ci/run-release-checkpoint.py --checkpoint-dir "$(RELEASE_GATE_CHECKPOINT_DIR)" --stage sibling-stack-hosted --fingerprint "$(RELEASE_GATE_FINGERPRINT)" --seconds "$(RELEASE_GATE_STACK_TIMEOUT_SECONDS)" -- $(MAKE) --no-print-directory container-stack-hosted-release-validation
	$(PYTHON) ./Tools/ci/run-release-checkpoint.py --checkpoint-dir "$(RELEASE_GATE_CHECKPOINT_DIR)" --stage compose-ci-hosted --fingerprint "$(RELEASE_GATE_FINGERPRINT)" --seconds "$(RELEASE_GATE_STAGE_TIMEOUT_SECONDS)" -- $(MAKE) --no-print-directory ci
...
docker-compose-parity-stages:
	...
	for target in $(DOCKER_COMPOSE_PARITY_TARGETS); do \
		python3 ./Tools/ci/run-release-checkpoint.py \
