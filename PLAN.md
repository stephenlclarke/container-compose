...
## Current Slab: Rotation-Aware Raw Follow
...
- <img alt="PARTIAL" src="https://img.shields.io/badge/PARTIAL-B26A00?style=flat-square"> [`logs-structured-record-storage`](https://github.com/stephenlclarke/container/tree/logs-structured-record-storage) and [`logs-structured-record-api`](https://github.com/stephenlclarke/container/tree/logs-structured-record-api): PR-ready fork branches that add active structured log storage and active structured record retrieval. They expose `stdio.log`, `stdio.jsonl`, `ContainerLogRecord`, `ContainerClient.logRecords(id:options:)`, and `ContainerClient.logRecordFile(id:)`; structured rotated replay and structured rotation-aware follow remain later PR slices.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td colspan="4">Notes: implemented locally in the `stephenlclarke/container` `logs-integration-chris` branch and documented with `ISSUE-logs-rotation-aware-follow.md` and `PR-logs-rotation-aware-follow.md`. The runtime-owned raw follow behavior is `ContainerClient.followLogs(id:options:)`, XPC route `containerFollowLogs`, Docker-style initial replay for negative, zero, and positive tail values, rename-based active-file detection, final reads from the renamed active file, and reopening the recreated active path without polling merged snapshots. Keep Compose service fan-out, prefixes, colors, and replica ordering out of the runtime branch. Structured/timestamped rotation-aware follow remains a separate upstream slice.</td>
    </tr>
    <tr>
      <td><img alt="OUTSTANDING" src="https://img.shields.io/badge/OUTSTANDING-6B7280?style=flat-square"> Design and upstream structured rotation-aware follow</td>
      <td>2026-06-21 23:43:58 BST</td>
      <td></td>
      <td></td>
    </tr>
    <tr>
      <td colspan="4">Notes: replace active-record-file-only structured follow with a runtime cursor that can follow `stdio.jsonl` across rename-based rotation, preserve stdout/stderr identity, honor `--timestamps`, `--since`, `--until`, and `--tail` after logical log-line reconstruction, detect truncation/retention loss, flush final partial records after container exit, and bound memory and file reads for long-running Compose timestamped follow sessions.</td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td colspan="4">Notes: `ContainerClientLogManager` already asks the direct API for `ContainerLogOptions(tail:)` and `ContainerLogReplayOptions(includeRotated: true)` on static raw logs. Prove the runtime honors that request with bounded static rotated replay, then keep the plugin as a thin fan-out and formatting layer. The remaining plugin-side raw merged-snapshot polling path should stay marked experimental until `container-compose` retargets to the fork's `ContainerClient.followLogs(id:options:)` stream and that API is accepted upstream.</td>
    </tr>
    <tr>
    ...
    <tr>
      <td><img alt="PLUGIN GAP" src="https://img.shields.io/badge/PLUGIN%20GAP-D97706?style=flat-square"> Replace raw rotated-follow polling with runtime follow stream</td>
      <td>2026-06-21 20:24:08 BST</td>
      <td></td>
      <td></td>
    </tr>
    <tr>
      <td colspan="4">Notes: remove the plugin-side repeated merged-snapshot polling path by calling `ContainerClient.followLogs(id:options:)` when the fork or upstream dependency exposes it. Keep Compose fan-out concurrency and per-service prefixing in this repository, but let the runtime own raw rotation/truncation boundaries. This task remains a plugin gap until the adapter is switched and covered with tests.</td>
    </tr>
    <tr>
...
- Uses active `ContainerClient.logRecordFile(id:)` access with a bounded local cursor for followed structured logs. Raw rotated follow still uses plugin-side merged `ContainerClient.logs(id:options:replay:)` polling until this plugin is retargeted to the fork's `ContainerClient.followLogs(id:options:)` stream.
...
- The local `logs-integration-chris` branch renders followed `container logs --timestamps`, `--since`, and `--until` output from the active structured record file and renders plain unfiltered `container logs --follow` through a raw rotation-aware follow stream.
...
- <img alt="PARTIAL" src="https://img.shields.io/badge/PARTIAL-B26A00?style=flat-square"> Writer-level local rotation, CLI option parsing, Compose option mapping, static rotated replay, and raw rotation-aware follow work on the local integration stack for `max-size` and `max-file`, but structured/timestamped rotation-aware follow still needs an upstream apple/container cursor or stream design.
...
5. <img alt="PARTIAL" src="https://img.shields.io/badge/PARTIAL-B26A00?style=flat-square"> <img alt="IMPLEMENTATION LINK" src="https://img.shields.io/badge/IMPLEMENTATION%20LINK-2563EB?style=flat-square"> <img alt="PEER TOUCHPOINT" src="https://img.shields.io/badge/PEER%20TOUCHPOINT-DB2777?style=flat-square"> <img alt="PEER OVERLAP" src="https://img.shields.io/badge/PEER%20OVERLAP-0891B2?style=flat-square"> <img alt="PEER COMPLEMENT" src="https://img.shields.io/badge/PEER%20COMPLEMENT-7C3AED?style=flat-square"> Upstream the local apple/container timestamped structured log records, direct retrieval API, static rotated replay, raw rotation-aware follow stream, and a separate structured rotation-aware follow design.
